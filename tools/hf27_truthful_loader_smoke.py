...
async def case(browser,name,delays=None,failures=None):
    ...
    state=await page.evaluate("""() => ({release:window.__APP_BOOTSTRAP_META__.release,products:window.__PRODUCTS_ARRAY__&&window.__PRODUCTS_ARRAY__.length,checks:window.__HF27_READINESS_CHECKS__,visible:window.__APP_BOOTSTRAP_META__.loader_visible_ms,phase:document.documentElement.getAttribute('data-runtime-phase'),overlay:!!document.getElementById('runtimeBootStatusOverlay')})""")
...
def main():
    ...
    cases=asyncio.run(run(a.chromium));result={'ok':all(c['passed'] for c in cases),'cases':cases}
    out=Path(a.json_out) if a.json_out else ROOT/'reports/hf27-truthful-loader-smoke.json'
