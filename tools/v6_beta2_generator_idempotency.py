...
FILES=[
 'config/runtime-assets.v6.0.0-beta2.json','config/runtime-assets.v5.3.210-rc2.json',
...
def run():
 ...
 return subprocess.run([sys.executable,str(ROOT/'tools/generate_v6_beta2_runtime.py')],cwd=ROOT,env=env,capture_output=True,text=True)
def main():
 global ROOT
 ap=argparse.ArgumentParser(); ap.add_argument('--app-root',default=str(ROOT)); ap.add_argument('--json-out',default='reports/v6-beta2-generator-idempotency.json'); ns=ap.parse_args(); ROOT=Path(ns.app_root).resolve()
