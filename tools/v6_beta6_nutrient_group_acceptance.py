...
import v6_beta6_analysis_continuity_acceptance as analysis_test
import v6_beta6_ivory_acceptance as ivory_test
...
async def mobile_case(browser,outdir):
    ...
    await page.evaluate("() => {window.NutritionTheme.set('ivory-brass',{persist:false});window.NutritionWorkspaceEntryUXHF28&&window.NutritionWorkspaceEntryUXHF28.setLayout('sections',{persist:false});window.NavigationShellV1.navigate('analysis/overview')}")
    await page.wait_for_function("document.documentElement.dataset.navigationRoute==='analysis/overview'",timeout=10000);await page.wait_for_timeout(500)
    ...
    await ctx.close();return result('mobile-keeps-sequential-text-first-navigation',ok,{'before':before,'after':after,'errors':errors,'added':added})
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--json-out',default='reports/v6-beta6-nutrient-group-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots');a=ap.parse_args();ROOT=Path(a.app_root).resolve();outdir=ROOT/a.screenshots;outdir.mkdir(parents=True,exist_ok=True)
