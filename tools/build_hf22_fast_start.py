...
FIXED_DT=(2026,7,25,20,0,0)
HOSTING_NAME='NUTRITION_CALCULATOR_V5_3_210_RC2_HOSTING_HOTFIX22_FAST_START_PRIVATE.zip'
FULL_NAME='NUTRITION_CALCULATOR_V5_3_210_RC2_HOSTING_HOTFIX22_FAST_START_ACCEPTED_FULL_PRIVATE_AUDITED.zip'
...
EXCLUDE_NAMES={'release-manifest.json','release-sbom.spdx.json','reports/hf22-fast-start-build.json','reports/hf22-fast-start-archive-verification.json'}
...
def file_rows(files):
    return [{'path':rel,'size':len(data),'sha256':sha(data),'mode':oct(mode)} for rel,data,mode in files]
...
def manifest(kind,files):
    browser=load_report('reports/hf22-fast-start-browser.json')
    static=load_report('reports/hf22-fast-start-static.json')
    integrity=json.loads((ROOT/'config/hf22-fast-start-integrity.json').read_text(encoding='utf-8'))
    ...
    return {
      ...
      'base_release':'v5.3.210-rc2-hf21-security-hardening',
      'artifact_kind':kind,
      'created_at':CREATED,
      'file_count':len(rows),
      ...
      'architecture':{
        ...
        'previous_browser_requests':101,
        'critical_bundle':'assets/runtime/critical-shell-v5.3.210-rc2-hf22.js',
        'product_bundle':'assets/data/products.v5.3.210-p1.3.bundle.js',
        'deferred_bundle':'assets/runtime/deferred-runtime-v5.3.210-rc2-hf22.js',
        'runtime_manifest':'assets/runtime/runtime-manifest-v5.3.210-rc2-hf22.js',
        'css_bundle':'assets/css/runtime-bundle-v5.3.210-rc2-hf21.css',
        'route_guard_until_background_ready':True,
        'source_fallbacks_preserved':True,
        'calculation_and_product_source_changes':False,
        'protected_hf21_files_byte_identical':len(integrity['verified_unchanged_files']),
      ...
      'acceptance':{
        ...
        'browser_audit_ok':browser.get('ok') is True,
        'browser_scenarios':browser.get('assertions'),
        'normal_mobile_shell_seconds':next((x.get('shell_s') for x in browser.get('cases',[]) if x.get('case')=='normal' and x.get('viewport')==[390,844]),None),
        'normal_desktop_shell_seconds':next((x.get('shell_s') for x in browser.get('cases',[]) if x.get('case')=='normal' and x.get('viewport')==[1440,900]),None),
...
def spdx(name,files):
    ...
    return {
      ...
      'creationInfo':{'created':CREATED,'creators':['Tool: build_hf22_fast_start.py']},
...
def build(path,kind,files):
    m=manifest(kind,files)
    s=spdx(path.name,files)
    ...
    with zipfile.ZipFile(path,'w') as z:
        ...
        add(z,'release-manifest.json',mdata,0o644)
        add(z,'release-sbom.spdx.json',sdata,0o644)
    return m,s

def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--out-dir',default=str(OUT)); args=ap.parse_args()
    out=Path(args.out_dir); out.mkdir(parents=True,exist_ok=True)
    for cmd in ([sys.executable,'tools/generate_runtime_assets.py','--check'],[sys.executable,'tools/hf22_fast_start_audit.py','--json-out','reports/hf22-fast-start-static.json']):
        r=subprocess.run(cmd,cwd=ROOT)
        if r.returncode: raise SystemExit('pre-build check failed: '+' '.join(cmd))
    browser=load_report('reports/hf22-fast-start-browser.json')
    if browser.get('ok') is not True: raise SystemExit('browser acceptance report is missing or failed')
    host_files=runtime_files(); full=full_files()
    hp=out/HOSTING_NAME; fp=out/FULL_NAME
    hm,_=build(hp,'hosting-private',host_files)
    fm,fsbom=build(fp,'full-private-audited',full)
    # Keep the working tree metadata coherent with the audited full artifact.
    ...
    (ROOT/'reports/hf22-fast-start-build.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
