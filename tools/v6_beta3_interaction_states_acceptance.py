...
async def core_contract(browser,outdir):
    ...
    ok=state['release']==VERSION and state['controller']==VERSION and state['marker']=='v1' and state['interactive']>=state['buttons']+state['fields'] and state['undecorated']==0 and not errors
    await p.screenshot(path=str(outdir/'beta3-interaction-desktop.png'),full_page=False);await ctx.close();return case('global-interaction-contract',ok,{'state':state,'errors':errors})
...
async def mobile(browser,outdir):
    ...
    await p.screenshot(path=str(outdir/'beta3-interaction-mobile.png'),full_page=False)
...
async def run(chromium,outdir):
    async with async_playwright() as pw:
        ...
        try:
            cases=[]
            for fn in (core_contract,focus_validation,pressed_selected_disabled,details_and_choices,profile_success,synthetic_async_states,theme_matrix,mobile):
                cases.append(await fn(browser,outdir) if fn in (core_contract,mobile) else await fn(browser))
            return cases
        finally:await browser.close()
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--json-out',default='reports/v6-beta3-interaction-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots');a=ap.parse_args();ROOT=Path(a.app_root).resolve();outdir=ROOT/a.screenshots;outdir.mkdir(parents=True,exist_ok=True)
    cases=asyncio.run(run(a.chromium,outdir));payload={'ok':all(c['passed'] for c in cases),'release_version':VERSION,'assertions':len(cases),'cases':cases};out=ROOT/a.json_out;out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+'\n',encoding='utf-8');print(json.dumps(payload,ensure_ascii=False,indent=2));raise SystemExit(0 if payload['ok'] else 1)
