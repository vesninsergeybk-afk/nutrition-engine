...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--json-out',default='reports/v6-gemini-media-acceptance.json');a=ap.parse_args();ROOT=Path(a.app_root).resolve();hf.ROOT=ROOT;hf.VERSION=VERSION
