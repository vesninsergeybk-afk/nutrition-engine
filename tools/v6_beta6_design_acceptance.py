...
async def main_async(root:Path,out:Path,screenshot_dir:Path):
    ...
    async with async_playwright() as pw:
        ...
        await p.screenshot(path=str(screenshot_dir/'beta6-design-desktop-empty.png'),full_page=True)
        ...
        await p.screenshot(path=str(screenshot_dir/'beta6-design-mobile-ration.png'),full_page=True)
        ...
        await p.screenshot(path=str(screenshot_dir/'beta6-design-mobile-profile.png'),full_page=True)
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('--root',default=str(Path(__file__).resolve().parents[1]));ap.add_argument('--json-out',default='reports/v6-beta6-design-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots')
