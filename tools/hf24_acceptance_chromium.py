...
async def startup_case(browser,viewport,slow=False,failure_kind=None):
    ...
    failure_markers={'product-compressed':'products.v5.3.210-p1.3.compact.json.gz','runtime-compressed':'deferred-runtime-v5.3.210-rc2-hf24.js.gz','critical-bundle':'critical-shell-v5.3.210-rc2-hf24.js'}
    ...
    unexpected_errors=[e for e in errors if not (failure_kind and '404' in e)]
    passed=shell['bridge'] and not shell['media'] and not shell['bg'] and state['release']==VERSION and state['products']==1105 and not state['media'] and not unexpected_errors
    ...
    result={'case':('startup-slow' if slow else 'startup-normal')+(('-'+failure_kind+'-fallback') if failure_kind else ''),'viewport':viewport,'shell_s':round(shell_s,3),'full_s':round(full_s,3),'shell':shell,'state':state,'requests':len(requests),'errors':errors,'unexpected_errors':unexpected_errors,'passed':passed}
