...
import hashlib,json,stat,zipfile,shutil
...
FIXED_DT=(2026,8,6,7,30,0)
PREFIX='NUTRITION_CALCULATOR_V6_0_0_BETA7_NAVIGATION_RECOVERY'
HOSTING_NAME=PREFIX+'_HOSTING_PRIVATE.zip'
FULL_NAME=PREFIX+'_ACCEPTED_FULL_PRIVATE_AUDITED.zip'
SECRET='api/gemini-secret.php'
DOCS=['README_DEPLOY_V6_BETA7_RU.md',PREFIX+'_RELEASE_NOTES_RU.md',PREFIX+'_CHANGELOG_RU.md',PREFIX+'_TEST_REPORT_RU.md',PREFIX+'_AUDIT_RU.md',PREFIX+'_FINAL_ACCEPTANCE_RU.md']
REPORTS=[
 'reports/v6-beta7-navigation-recovery-acceptance.json','reports/v6-beta7-design-acceptance.json','reports/v6-beta7-profile-hierarchy-acceptance.json','reports/v6-beta7-profile-acceptance.json','reports/v6-beta7-interaction-acceptance.json','reports/v6-beta7-analysis-continuity.json','reports/v6-beta7-nutrient-group-acceptance.json','reports/v6-beta7-gemini-media-acceptance.json','reports/v6-beta7-runtime-inventory.json','reports/v6-beta7-protected-comparison.json','reports/v6-beta7-ui-budget.json','reports/v6-beta7-release-audit.json','reports/v6-beta7-syntax-checks.json','reports/v6-beta7-hf28-utf8.json','reports/v6-beta7-hf28-gemini-contract.json','reports/v6-beta7-hf28-media-mock-e2e.json','reports/v6-beta7-hf28-credential-loading.json','reports/v6-beta7-http-contract.json'
]
...
EXCLUDE_NAMES={'release-manifest.json','release-sbom.spdx.json','reports/v6-beta7-build.json','reports/v6-beta7-archive-verification.json','reports/v6-beta7-extracted-hosting-acceptance.json'}
...
def hosting_files():
 ...
 rels=list(files)+['config/runtime-assets.v6.0.0-beta7.json']+DOCS+REPORTS
 out=[]
...
def manifest(kind,files):
 protected=json.loads((ROOT/'reports/v6-beta7-protected-comparison.json').read_text())
 runtime=json.loads((ROOT/'reports/v6-beta7-runtime-inventory.json').read_text())
 budget=json.loads((ROOT/'reports/v6-beta7-ui-budget.json').read_text())
 rows=[row(*x) for x in files]
 return {'schema_version':7,'release_version':VERSION,'base_release':BASE,'artifact_kind':kind,'created_at':CREATED,'file_count':len(rows),'uncompressed_bytes':sum(x['size'] for x in rows),'change_scope':'Restore permanent three-theme selection, working Sections/Canvas modes on mobile and desktop, and permanent access to profile/needs with edit, recalculate and print journey; no calculation-engine changes.','architecture':{'themes':['modern','retro-2bit','ivory-brass'],'presentation_modes':['sections','canvas'],'desktop_ivory_canvas':'spatial workspace','mobile_canvas':'continuous sequential page','primary_routes':['profile','ration','analysis','correction','report'],'profile_required_fields':['needs_sex','needs_age','needs_h','needs_w','needs_activity'],'product_count':1105,'protected_files_byte_identical':protected['unchanged_files'],'runtime_files':runtime['file_count'],'incremental_ui_bytes':budget['incremental_ui_bytes'],'server_credentials':'Primary and backup Gemini keys are included only in protected server-side api/gemini-secret.php.'},'acceptance':reports_summary(),'limitations':['Physical Android/iPhone, Safari, Samsung Internet and Qt WebEngine remain external acceptance targets.','A live external Gemini HTTP 200 is not claimed from the isolated build environment.','Human usability observation remains a release-candidate activity.','Private archives contain protected server credentials and must not be stored in public download directories.'],'files':rows}
def sbom(name,files):
 fs=[]
...
def main():
 ...
 result={'ok':True,'release_version':VERSION,'hosting':h,'full':f};(ROOT/'reports/v6-beta7-build.json').write_text(json.dumps(result,ensure_ascii=False,indent=2)+'\n');print(json.dumps(result,ensure_ascii=False,indent=2))
