...
FILES=[
 'config/runtime-assets.v6.0.0-beta3.json','config/runtime-assets.v5.3.210-rc2.json',
 'assets/runtime/runtime-manifest-v6.0.0-beta3.js','assets/runtime/critical-shell-v6.0.0-beta3.js','assets/runtime/critical-shell-v6.0.0-beta3.legacy.js',
 'assets/js/00-runtime-bootstrap-v6.0.0-beta3.js','assets/js/00-runtime-selector-v6.0.0-beta3.js','assets/legacy/js/00-runtime-bootstrap-v6.0.0-beta3.legacy.js',
 'index.html','index-v5.3.210.html','tools/runtime_inventory.py']
def sha(p): return hashlib.sha256(p.read_bytes()).hexdigest()
def snap(): return {f:sha(ROOT/f) for f in FILES}
def run():
 env=dict(os.environ); env['PYTHONDONTWRITEBYTECODE']='1'
 return subprocess.run([sys.executable,str(ROOT/'tools/generate_v6_beta3_runtime.py')],cwd=ROOT,env=env,capture_output=True,text=True)
def main():
 global ROOT
 ap=argparse.ArgumentParser(); ap.add_argument('--app-root',default=str(ROOT)); ap.add_argument('--json-out',default='reports/v6-beta3-generator-idempotency.json'); ns=ap.parse_args(); ROOT=Path(ns.app_root).resolve()
