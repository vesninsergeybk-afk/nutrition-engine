...
DEFAULT_BASE=Path('/mnt/data/nutrition_v6_ivory_work')
...
def selected(root:Path):
    ...
    # Mirror files are protected where they exist.
    mirrors=[]
    for p in out:
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--base-root',default=str(DEFAULT_BASE));ap.add_argument('--json-out',default='reports/v6-beta1-protected-comparison.json');ns=ap.parse_args();ROOT=Path(ns.app_root).resolve();base=Path(ns.base_root).resolve()
    ...
    result={'ok':not missing and not changed and counts['base']==counts['current']==1105,'base_release':'v6.0.0-alpha1-ivory-brass','release_version':'v6.0.0-beta1-profile-continuity','protected_files':len(base_files),'unchanged_files':len(same),'missing':missing,'changed':changed,'product_count':counts}
