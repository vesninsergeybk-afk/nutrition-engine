...
HOSTING_NAME = 'NUTRITION_CALCULATOR_V6_0_0_BETA2_NUTRIENT_GROUP_OVERVIEW_HOSTING_PRIVATE.zip'
FULL_NAME = 'NUTRITION_CALCULATOR_V6_0_0_BETA2_NUTRIENT_GROUP_OVERVIEW_ACCEPTED_FULL_PRIVATE_AUDITED.zip'
SECRET = 'api/gemini-secret.php'
SECRET_RX = re.compile(rb'(?:AIza[0-9A-Za-z_-]{25,}|AQ\.[0-9A-Za-z_-]{25,}|sk-[0-9A-Za-z_-]{20,})')
FINAL_REPORTS = {
    'group_overview': 'v6-beta2-final-group-acceptance.json',
    'profile': 'v6-beta2-final-profile-acceptance.json',
    'analysis': 'v6-beta2-final-analysis-continuity.json',
    'ui': 'v6-beta2-final-ivory-acceptance.json',
    'extended_ux': 'v6-beta2-final-ivory-extended-acceptance.json',
    'gemini_media': 'v6-beta2-final-gemini-media-acceptance.json',
}
...
def check_zip(path: Path, kind: str, rows: list[dict]) -> None:
    with zipfile.ZipFile(path) as zf:
        ...
        bad = zf.testzip()
        add(rows, f'{path.name}: CRC', bad is None, bad)
        ...
        add(rows, f'{path.name}: group contract', arch.get('nutrient_group_overview_contract') == 'NutrientGroupOverview.v1')
        add(rows, f'{path.name}: overview not score', arch.get('nutrient_group_overview_not_a_score') is True)
        add(rows, f'{path.name}: canonical source', arch.get('nutrient_group_overview_source') == 'NutritionAnalysisWorkspaceHF7')
        add(rows, f'{path.name}: profile contract', str(arch.get('profile_storage_contract', '')).startswith('NutritionProfilePersistenceV1'))
        add(rows, f'{path.name}: analysis contract', arch.get('analysis_feature_contract') == 'NutritionFeatureContinuityV1')
        add(rows, f'{path.name}: 1105 products', arch.get('product_count') == 1105)
        add(rows, f'{path.name}: HF29/HF30 retained', all(x in arch.get('future_work', {}) for x in ('HOTFIX29', 'HOTFIX30')))
        embedded = acceptance.get('extracted_hosting_browser', {})
        add(rows, f'{path.name}: embedded extracted preflight 35/35', embedded.get('ok') is True and embedded.get('assertions') == 35, embedded)
        ...
        if SECRET in names:
            ...
            add(rows, f'{path.name}: two packaged keys', len(keys) >= 2 and len(set(keys)) >= 2, {'key_count': len(keys)})
        ...
        for name in names:
            ...
            if SECRET_RX.search(zf.read(name)):
                leaks.append(name)
        add(rows, f'{path.name}: keys isolated', not leaks, leaks)

        required = (
            'index.html',
            'config/runtime-assets.v6.0.0-beta2.json',
            'assets/runtime/runtime-manifest-v6.0.0-beta2.js',
            'assets/runtime/critical-shell-v6.0.0-beta2.js',
            'assets/runtime/critical-shell-v6.0.0-beta2.legacy.js',
            'assets/js/00-runtime-bootstrap-v6.0.0-beta2.js',
            'assets/js/00-runtime-selector-v6.0.0-beta2.js',
            ...
            'assets/css/profile-continuity-v6.css',
            'README_DEPLOY_V6_BETA2_RU.md',
        )
        for rel in required:
            add(rows, f'{path.name}: {rel}', rel in names)
        if 'assets/js/ivory-brass-shell-v6.js' in names:
            text = zf.read('assets/js/ivory-brass-shell-v6.js').decode('utf-8')
            add(rows, f'{path.name}: no pseudo balance label', not re.search(r'БАЛАНС|Баланс\s+\d+\s*/\s*\d+', text))
            add(rows, f'{path.name}: transparent overview copy', 'не новая итоговая оценка рациона' in text)
...
def direct_secret(root: Path, rows: list[dict], out_dir: Path) -> None:
    ...
    body = out_dir / 'v6-beta2-secret-body.tmp'
    try:
        time.sleep(0.8)
        result = subprocess.run(
            ['curl', '-sS', '-o', str(body), '-w', '%{http_code}', f'http://127.0.0.1:{port}/api/gemini-secret.php'],
            capture_output=True,
            text=True,
            timeout=20,
        )
...
def read_final_acceptance(report_dir: Path, hosting_sha: str, rows: list[dict]) -> dict:
    ...
    for suite, filename in FINAL_REPORTS.items():
        ...
        if ok:
            ...
            except Exception as exc:  # pragma: no cover - release safety branch
                data = {'error': str(exc)}
                ok = False
    ...
    if ok:
        try:
            ...
            ok = (
                combined.get('ok') is True
                and combined.get('release_version') == VERSION
                and combined.get('assertions') == 35
                and combined.get('archive_sha256') == hosting_sha
            )
        except Exception as exc:  # pragma: no cover
            combined = {'error': str(exc)}
            ok = False
    add(rows, 'exact final extracted hosting combined 35/35 and archive hash bound', ok, combined)
...
def main() -> None:
    ...
    parser.add_argument('--json-out', default='reports/v6-beta2-archive-verification.json')
    ...
    verify = OUT / 'v6_beta2_final_archive_verify'
    ...
    inventory = json.loads((ROOT / 'reports/v6-beta2-runtime-inventory.json').read_text(encoding='utf-8'))
    with zipfile.ZipFile(hosting) as zf:
        names = set(zf.namelist())
    missing = [rel for rel in inventory.get('files', []) if rel not in names]
    add(rows, 'hosting contains complete runtime closure', inventory.get('ok') is True and inventory.get('file_count') == 314 and not missing, {'file_count': inventory.get('file_count'), 'missing': missing[:10]})
    ...
    repro = OUT / 'v6_beta2_repro'
    shutil.rmtree(repro, ignore_errors=True)
    repro.mkdir(parents=True)
    result = subprocess.run(
        [sys.executable, str(ROOT / 'tools/build_v6_beta2.py'), '--out-dir', str(repro)],
        cwd=ROOT,
        env=env,
        capture_output=True,
        text=True,
    ...
    verification = {
        ...
        'results': rows,
        'artifacts': [
            {'path': str(path), 'sha256': sha(path.read_bytes()), 'bytes': path.stat().st_size}
            for path in (hosting, full)
        ],
