...
import v6_beta4_profile_acceptance as profile_test
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--json-out',default='reports/v6-beta4-analysis-continuity.json');ap.add_argument('--screenshots',default='reports/screenshots');ns=ap.parse_args();ROOT=Path(ns.app_root).resolve();profile_test.ROOT=ROOT;outdir=ROOT/ns.screenshots;outdir.mkdir(parents=True,exist_ok=True)
