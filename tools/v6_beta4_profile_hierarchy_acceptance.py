...
async def first_run(browser,outdir):
    ...
    state=await p.evaluate('''() => ({
      ...
      basic:[...document.querySelectorAll('#profileBasicSection input,#profileBasicSection select')].map(x=>x.id),
...
async def calculate_restore(browser,outdir):
    ...
    await p2.evaluate("() => window.NavigationShellV1.navigate('profile')");await p2.wait_for_timeout(200);await p2.screenshot(path=str(outdir/'beta4-profile-restored-desktop.png'),full_page=False)
