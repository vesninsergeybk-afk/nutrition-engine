...
def check_zip(path,kind,rows):
 with zipfile.ZipFile(path) as z:
  ...
  add(rows,path.name+': HOTFIX30 complete',a.get('profile_hierarchy_release')=='HOTFIX30 completed');add(rows,path.name+': hierarchy contract',a.get('profile_hierarchy_contract')=='NutritionProfileHierarchyV2')
  ...
  required=('index.html','config/runtime-assets.v6.0.0-beta4.json','assets/runtime/runtime-manifest-v6.0.0-beta4.js','assets/runtime/critical-shell-v6.0.0-beta4.js','assets/runtime/critical-shell-v6.0.0-beta4.legacy.js','assets/js/00-runtime-bootstrap-v6.0.0-beta4.js','assets/js/00-runtime-selector-v6.0.0-beta4.js','assets/legacy/js/00-runtime-bootstrap-v6.0.0-beta4.legacy.js','assets/css/profile-hierarchy-v6.0.0-beta4.css','assets/js/profile-hierarchy-v6.0.0-beta4.js','assets/js/profile-persistence-v6.0.0-beta4.js','assets/css/interaction-states-v6.0.0-beta3.css','assets/js/interaction-state-controller-v6.0.0-beta3.js','README_DEPLOY_V6_BETA4_RU.md')
  for rel in required:add(rows,path.name+': '+rel,rel in names)
def direct_secret(root,rows):
 port='18764';proc=subprocess.Popen(['php','-S',f'127.0.0.1:{port}','-t',str(root)],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL);body=OUT/'beta4-secret-body.tmp'
 try:
...
def main():
 global ROOT,OUT
 ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--out-dir',default=str(OUT));ap.add_argument('--extracted-report-dir',required=True);ap.add_argument('--json-out',default='reports/v6-beta4-archive-verification.json');a=ap.parse_args();ROOT=Path(a.app_root).resolve();OUT=Path(a.out_dir).resolve();rd=Path(a.extracted_report_dir).resolve();hosting=OUT/HOSTING_NAME;full=OUT/FULL_NAME;rows=[]
 ...
 verify=OUT/'v6_beta4_final_archive_verify';shutil.rmtree(verify,ignore_errors=True);verify.mkdir(parents=True)
 with zipfile.ZipFile(hosting) as z:z.extractall(verify)
 direct_secret(verify,rows)
 for rel in ('assets/js/profile-hierarchy-v6.0.0-beta4.js','assets/js/profile-persistence-v6.0.0-beta4.js','assets/js/interaction-state-controller-v6.0.0-beta3.js','assets/js/ivory-brass-search-compact-v6.js','assets/js/ivory-brass-view-model-v6.js','assets/runtime/runtime-manifest-v6.0.0-beta4.js','assets/runtime/critical-shell-v6.0.0-beta4.js','assets/runtime/critical-shell-v6.0.0-beta4.legacy.js','assets/js/00-runtime-bootstrap-v6.0.0-beta4.js','assets/js/00-runtime-selector-v6.0.0-beta4.js'):
 ...
 inv=json.loads((ROOT/'reports/v6-beta4-runtime-inventory.json').read_text());names=set(zipfile.ZipFile(hosting).namelist());missing=[x for x in inv.get('files',[]) if x not in names];add(rows,'hosting complete runtime closure',inv.get('ok') is True and inv.get('file_count')==316 and not missing,{'file_count':inv.get('file_count'),'missing':missing[:10]})
 ...
 env=dict(os.environ);env['PYTHONDONTWRITEBYTECODE']='1';repro=OUT/'v6_beta4_repro';shutil.rmtree(repro,ignore_errors=True);repro.mkdir(parents=True);r=subprocess.run([sys.executable,str(ROOT/'tools/build_v6_beta4.py'),'--out-dir',str(repro)],cwd=ROOT,env=env,capture_output=True,text=True,timeout=500);add(rows,'rebuild command',r.returncode==0,(r.stderr or r.stdout)[-500:])
 if r.returncode==0:
