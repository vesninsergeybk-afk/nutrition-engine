...
BASE = 'v6.0.0-alpha1-ivory-brass'
CREATED = '2026-08-04T18:30:00Z'
FIXED_DT = (2026, 8, 4, 18, 30, 0)
...
EXCLUDE_NAMES = {
    ...
    'reports/v6-beta1-build.json', 'reports/v6-beta1-archive-verification.json',
}
HOSTING_DOCS = [
    'config/runtime-assets.v6.0.0-beta1.json',
    'README_DEPLOY_V6_BETA1_RU.md',
    'NUTRITION_CALCULATOR_V6_0_0_BETA1_PROFILE_CONTINUITY_RELEASE_NOTES_RU.md',
    'NUTRITION_CALCULATOR_V6_0_0_BETA1_PROFILE_CONTINUITY_CHANGELOG_RU.md',
    'NUTRITION_CALCULATOR_V6_0_0_BETA1_PROFILE_CONTINUITY_TEST_REPORT_RU.md',
    'NUTRITION_CALCULATOR_V6_0_0_BETA1_PROFILE_CONTINUITY_AUDIT_RU.md',
]
REQUIRED_REPORTS = [
    'reports/v6-beta1-release-audit.json',
    'reports/v6-beta1-profile-acceptance.json',
    'reports/v6-beta1-analysis-continuity.json',
    'reports/v6-beta1-extracted-hosting-acceptance.json',
    'reports/v6_ivory_acceptance.json',
    'reports/v6_ivory_extended_acceptance.json',
    'reports/v6-gemini-media-acceptance.json',
    'reports/v6-beta1-syntax-checks.json',
    'reports/v6-beta1-protected-comparison.json',
    'reports/v6-beta1-generator-idempotency.json',
    'reports/v6-beta1-ui-budget.json',
...
def manifest(kind: str, files):
    audit = read_json('reports/v6-beta1-release-audit.json')
    ...
    protected = read_json('reports/v6-beta1-protected-comparison.json')
    profile = read_json('reports/v6-beta1-profile-acceptance.json')
    analysis = read_json('reports/v6-beta1-analysis-continuity.json')
    extracted = read_json('reports/v6-beta1-extracted-hosting-acceptance.json')
    budget = read_json('reports/v6-beta1-ui-budget.json')
    syntax = read_json('reports/v6-beta1-syntax-checks.json')
    ...
    return {
        ...
        'change_scope': (
            'Profile-first journey with persistent anthropometry, draft/applied safety, '
            'restored personal targets, full analysis continuity and preserved V6 adaptive layout.'
        ),
        'architecture': {
            ...
            'product_count': 1105,
            'protected_files_byte_identical': protected['unchanged_files'],
            'runtime_files': runtime['file_count'],
            'profile_storage_contract': 'NutritionProfilePersistenceV1 / nutritionCalculator.profile.v1',
            'profile_first_required_fields': ['sex','age','height','weight','activity'],
            'draft_applied_separation': True,
            'analysis_feature_contract': 'NutritionFeatureContinuityV1',
            'server_credentials': (
        ...
        'acceptance': {
            ...
            'profile_continuity_ok': profile['ok'],
            'profile_continuity_assertions': profile['assertions'],
            'analysis_continuity_ok': analysis['ok'],
            'analysis_continuity_assertions': analysis['assertions'],
            'extracted_hosting_acceptance_ok': extracted['ok'],
            'extracted_hosting_acceptance_assertions': extracted['assertions'],
            ...
            'protected_core_changes': len(protected['changed']),
            'beta_ui_css_bytes': budget['beta_increment']['css']['bytes'],
            'beta_ui_js_bytes': budget['beta_increment']['js']['bytes'],
            'v6_ui_css_bytes': budget['v6_ui_total']['css']['bytes'],
            'v6_ui_js_bytes': budget['v6_ui_total']['js']['bytes'],
        ...
        'limitations': [
            ...
            'Human usability testing with ordinary and professional users has not yet been performed; release status is beta 1.',
...
def spdx(name: str, files):
    ...
    return {
        ...
        'creationInfo': {'created': CREATED, 'creators': ['Tool: build_v6_beta1.py']},
        'packages': [{
...
def main() -> None:
    ...
    generated = subprocess.run(
        [sys.executable, str(ROOT / 'tools/generate_v6_beta1_runtime.py')],
    ...
    (ROOT / 'reports/v6-beta1-build.json').write_text(json.dumps(result, ensure_ascii=False, indent=2) + '\n', encoding='utf-8')
