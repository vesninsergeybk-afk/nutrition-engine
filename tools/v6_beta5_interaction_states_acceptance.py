...
async def core_contract(browser,outdir):
    ...
    await p.screenshot(path=str(outdir/'beta5-interaction-desktop.png'),full_page=False);await ctx.close();return case('global-interaction-contract',ok,{'state':state,'errors':errors})
...
async def mobile(browser,outdir):
    ...
    await p.screenshot(path=str(outdir/'beta5-interaction-mobile.png'),full_page=False)
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--json-out',default='reports/v6-beta5-interaction-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots');ap.add_argument('--case',action='append',choices=list(CASE_FUNCTIONS));a=ap.parse_args();ROOT=Path(a.app_root).resolve();outdir=ROOT/a.screenshots;outdir.mkdir(parents=True,exist_ok=True)
