...
async def journey(browser,outdir):
    ...
    await page2.screenshot(path=str(outdir/'beta1-profile-restored-desktop.png'),full_page=False)
    ...
    await page3.click('#needs_reset_btn');await page3.wait_for_timeout(350)
...
async def mobile(browser,outdir):
    ...
    await page.screenshot(path=str(outdir/'beta1-profile-mobile-390x844.png'),full_page=False)
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--json-out',default='reports/v6-beta3-profile-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots');ns=ap.parse_args();ROOT=Path(ns.app_root).resolve();outdir=ROOT/ns.screenshots;outdir.mkdir(parents=True,exist_ok=True)
