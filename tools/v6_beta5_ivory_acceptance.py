...
async def desktop_canvas(browser,outdir:Path):
    ...
    await page.wait_for_selector('#ivoryInsightRail',state='attached')
    profile_state=await page.evaluate("() => ({rail:getComputedStyle(ivoryInsightRail).display,metric:!!document.querySelector('.ivory-metric-strip')?.offsetParent,dock:!!document.querySelector('.ivory-quick-dock')?.offsetParent})")
    ...
    ok=profile_state=={'rail':'none','metric':False,'dock':False} and state['theme']=='ivory-brass' and state['layout']=='canvas' and state['railDisplay']!='none' and state['heroDisplay']!='none' and state['overflow']<=1 and state['contract']=='NutritionUIViewModel.v1' and rectok and not errors
    await ctx.close();return passed('desktop-ivory-spatial-canvas',ok,{'profile':profile_state,'state':state,'errors':errors,'requests':len(req)})
...
async def mobile(browser,outdir:Path):
    ...
    await page.wait_for_selector('#ivoryMobileSummary',state='attached')
    profile=await page.evaluate("""() => ({route:document.documentElement.dataset.navigationRoute,mobile:getComputedStyle(ivoryMobileSummary).display,firstTop:needs_sex.getBoundingClientRect().top,viewport:innerHeight})""")
    await go_ration(page)
    ...
    ok=profile['route']=='profile' and profile['mobile']=='none' and 0<=profile['firstTop']<profile['viewport'] and state['theme']=='ivory-brass' and state['layout']=='sections' and state['canvasDisabled'] and state['canvasAria']=='true' and state['rail']=='none' and state['hero']=='none' and state['mobile']!='none' and state['overflow']<=1 and not errors
    await ctx.close();return passed('mobile-sequential-architecture',ok,{'profile':profile,'state':state,'errors':errors})
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--json-out',default='reports/v6-beta5-ivory-acceptance.json');ap.add_argument('--screenshots',default='reports/screenshots');a=ap.parse_args();ROOT=Path(a.app_root).resolve();outdir=ROOT/a.screenshots;outdir.mkdir(parents=True,exist_ok=True)
