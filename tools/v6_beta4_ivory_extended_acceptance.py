...
import v6_beta4_ivory_acceptance as base
...
def main():
    global ROOT
    ap=argparse.ArgumentParser();ap.add_argument('--chromium',default='/usr/bin/chromium');ap.add_argument('--app-root',default=str(ROOT));ap.add_argument('--json-out',default='reports/v6-beta4-ivory-extended-acceptance.json');ap.add_argument('--case',action='append',choices=list(CASE_FUNCTIONS));a=ap.parse_args();ROOT=Path(a.app_root).resolve();base.ROOT=ROOT
