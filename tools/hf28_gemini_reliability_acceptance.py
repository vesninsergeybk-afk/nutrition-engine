...
async def setup_media(page,mode='success',policy_false=False):
    ...
    js="""() => {
      ...
      window.__hf28Calls=0;window.__hf28Constraints=null;
      window.__NUTRITION_MIC_SOFT_TIMEOUT_MS__=250;
      window.__NUTRITION_MIC_HARD_TIMEOUT_MS__=1500;
      var track={stop:function(){window.__hf28Stops=(window.__hf28Stops||0)+1;},onended:null};
      ...
      Object.defineProperty(devices,'getUserMedia',{configurable:true,value:function(c){window.__hf28Calls++;window.__hf28Constraints=c;__BODY__}});
...
async def photo_one_click(browser):
    ...
    fixture=Path('/tmp/hf28-media-test.png');fixture.write_bytes(PNG)
    ...
    state=await page.evaluate("""() => ({media:window.NutritionGeminiRationImport.getMedia().map(x=>({kind:x.kind,source:x.source,state:x.preparationState,size:x.preparedSize||x.file.size})),topHidden:document.getElementById('workspaceMediaEntryStatus').hidden,topText:document.getElementById('workspaceMediaEntryStatusText').textContent,lazyScripts:document.querySelectorAll('script[data-gemini-media-lazy-module]').length,recordState:document.getElementById('geminiRationRecordAudio').dataset.actionState,recordText:document.getElementById('geminiRationRecordAudio').textContent,recorder:window.NutritionGeminiRationImport.getRecorderDiagnostics()})""")
    passed=before['bridge'] and not before['media'] and state['lazyScripts']==1 and state['media'][0]['kind']=='image' and state['media'][0]['state']=='ready' and not state['topHidden'] and ('Фотография подготовлена' in state['topText'] or 'Фотографии готовы' in state['topText'] or 'Подготовлено фотографий' in state['topText']) and state['recordState']!='working' and 'Подключаем микрофон' not in state['recordText'] and not errors
...
async def success_visible(browser,policy_false=False):
    ...
    await page.wait_for_function('window.__hf28Calls===1',timeout=5000)
    ...
    state=await page.evaluate("""() => ({
      calls:window.__hf28Calls,constraints:window.__hf28Constraints,
      ...
      release:window.__APP_BOOTSTRAP_META__.release,recordState:document.getElementById('geminiRationRecordAudio').dataset.actionState,recordPressed:document.getElementById('geminiRationRecordAudio').getAttribute('aria-pressed'),recordText:document.getElementById('geminiRationRecordAudio').textContent
    })""")
    passed=(state['calls']==1 and state['constraints']=={'audio':True} and not state['topHidden'] and 'Микрофон включён' in state['topText'] and state['fallbackHidden'] and not state['pending'] and state['active'] and state['release']==VERSION and state['recordState']=='recording' and state['recordPressed']=='true' and 'Запись идёт' in state['recordText'] and not errors)
...
async def denied_visible(browser):
    ...
    state=await page.evaluate("""() => ({calls:window.__hf28Calls,topHidden:document.getElementById('workspaceMediaEntryStatus').hidden,topText:document.getElementById('workspaceMediaEntryStatusText').textContent,fallbackHidden:document.getElementById('workspaceNativeVoiceFallback').hidden,pending:window.NutritionMediaEntryBridge.isVoicePending(),recordState:document.getElementById('geminiRationRecordAudio').dataset.actionState,recordText:document.getElementById('geminiRationRecordAudio').textContent})""")
    passed=state['calls']==1 and not state['topHidden'] and 'Доступ к микрофону не разрешён' in state['topText'] and not state['fallbackHidden'] and not state['pending'] and state['recordState']=='error' and 'Повторить запись' in state['recordText'] and not errors
...
async def hanging_watchdog(browser):
    ...
    immediate=await page.evaluate("() => ({hidden:document.getElementById('workspaceMediaEntryStatus').hidden,text:document.getElementById('workspaceMediaEntryStatusText').textContent,pending:window.NutritionMediaEntryBridge.isVoicePending(),calls:window.__hf28Calls})")
    ...
    hard=await page.evaluate("() => ({text:document.getElementById('workspaceMediaEntryStatusText').textContent,fallbackHidden:document.getElementById('workspaceNativeVoiceFallback').hidden,pending:window.NutritionMediaEntryBridge.isVoicePending(),calls:window.__hf28Calls})")
    await page.locator('[data-ration-entry-method="voice"]').click();await page.wait_for_function('window.__hf28Calls===2',timeout=2000)
    retried=await page.evaluate("() => ({calls:window.__hf28Calls,pending:window.NutritionMediaEntryBridge.isVoicePending(),text:document.getElementById('workspaceMediaEntryStatusText').textContent})")
...
async def repeated_click_visible(browser):
    ...
    button=page.locator('[data-ration-entry-method="voice"]');await button.click();await page.wait_for_function('window.__hf28Calls===1',timeout=2000);await button.click()
    state=await page.evaluate("() => ({calls:window.__hf28Calls,text:document.getElementById('workspaceMediaEntryStatusText').textContent,fallbackHidden:document.getElementById('workspaceNativeVoiceFallback').hidden,pending:window.NutritionMediaEntryBridge.isVoicePending()})")
...
async def run(chromium):
    async with async_playwright() as p:
        ...
        cases=[await photo_one_click(browser),await success_visible(browser),await success_visible(browser,True),await denied_visible(browser),await hanging_watchdog(browser),await repeated_click_visible(browser),await native_fallback(browser)]
