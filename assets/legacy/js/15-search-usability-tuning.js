...
(function () {
    ...
    function autoExpandMobileResults() {
        ...
        if (familyRoot && familyRoot.dataset.preparationFamilyGrouped === 'true')
            return;
        if (!isMobile() || autoExpandLock)
            return;
        var buttons = findShowMoreButtons();
        if (!buttons.length)
            return;
        var count = cards().length;
        if (count >= MOBILE_TARGET_RESULTS)
            return;
        autoExpandLock = true;
        var guard = 0;
        function step() {
    ...
    function enhanceCards() {
        cards().forEach(function (card) {
            ...
            if (add && !looksLikeShowMore(add))
                add.setAttribute('aria-label', 'Добавить в рацион: ' + name);
            if (grams) {
    ...
    function localAddedFeedback(btn) {
        ...
        if (card)
            card.classList.add('v45-just-added', 'v45-card-added');
        window.setTimeout(function () {
            btn.classList.remove('v45-added');
            btn.textContent = old;
            if (card)
                card.classList.remove('v45-just-added');
        }, 1800);
    }
    ...
    function installObservers() {
        ...
        document.addEventListener('click', function (e) {
            ...
            if (btn && !looksLikeShowMore(btn))
                localAddedFeedback(btn);
        }, true);
        if (window.matchMedia) {
            var mq = window.matchMedia(MOBILE_QUERY);
            if (mq.addEventListener)
                mq.addEventListener('change', function () { window.setTimeout(postProcessSearch, 80); });
            else if (mq.addListener)
                mq.addListener(function () { window.setTimeout(postProcessSearch, 80); });
        }
    }
    function installTests() {
        ...
        window.runSmokeTests = function () {
            ...
            try {
                ...
                ok = false;
                window.__v45PreviousSmokeError = String(e && e.message || e);
            }
