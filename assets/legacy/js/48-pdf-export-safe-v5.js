...
(function () {
    ...
    function cleanClone(node) {
        ...
        clone.querySelectorAll('button,.toolbar,#totalsActions,.harvard-plate-actions,.hpapply-actions,.dev-only,.hosting-test-panel,.a11y-audit-panel,script').forEach(function (n) { try {
            n.remove();
        }
        catch (_) { } });
        clone.querySelectorAll('details').forEach(function (d) { try {
            d.setAttribute('open', '');
        }
        catch (_) { } });
        clone.querySelectorAll('input,select,textarea').forEach(function (el) {
            try {
                ...
                if (el.tagName === 'SELECT')
                    span.textContent = el.options && el.selectedIndex >= 0 ? el.options[el.selectedIndex].textContent : el.value;
                else
    ...
    function reportHtml(kind) {
        if (kind === 'needs')
            return needsHtml();
        if (kind === 'totals')
            return totalsHtml() || '<p>Секция итогов не найдена.</p>';
        if (kind === 'hei')
            return heiHtml() || '<p>Секция HEI не найдена.</p>';
        if (kind === 'all')
            return [needsHtml(), totalsHtml(), rationHtml(), heiHtml()].filter(Boolean).join('');
        return '<p>Раздел не найден.</p>';
    }
    function titleFor(kind) {
        if (kind === 'needs')
            return 'PDF: расчёт потребностей';
        if (kind === 'totals')
            return 'PDF: итоги и дневные нормы';
        if (kind === 'hei')
            return 'PDF: HEI-2020';
    ...
    function openNativePdf(kind) {
        ...
        if (kind === 'all' && window.NutritionReportV5 && typeof window.NutritionReportV5.getReportHtml === 'function') {
            try {
                ...
                if (/<\/body>/i.test(fullHtml))
                    fullHtml = fullHtml.replace(/<\/body>/i, auto + '</body>');
                else
                    fullHtml += auto;
                w.document.open();
                ...
                try {
                    window.__LAST_NATIVE_PDF_EXPORT__ = { kind: kind, at: new Date().toISOString(), version: VERSION, source: 'NutritionReportV5' };
                }
                catch (_) { }
                return true;
            }
                ...
                try {
                    console.warn('[NutritionPdfExportSafe] report builder fallback failed', err);
                }
                catch (_) { }
            }
        }
        ...
        try {
            window.__LAST_NATIVE_PDF_EXPORT__ = { kind: kind, at: new Date().toISOString(), version: VERSION };
        }
        catch (_) { }
        return true;
    }
    function kindFromTarget(t) {
        ...
        if (!hit)
            return '';
        if (hit.id === 'needs_pdf_btn')
            return 'needs';
        if (hit.id === 'exportTotalsPdfBtn')
            return 'totals';
        if (hit.id === 'heiPdfBtn')
            return 'hei';
        if (hit.id === 'exportAllPdfBtn')
            return 'all';
        return '';
    }
    function onClick(e) {
    ...
    function improveLabels() {
        ...
        Object.keys(labels).forEach(function (id) { var b = $(id); if (b) {
            b.textContent = labels[id];
            b.setAttribute('type', 'button');
            b.setAttribute('title', 'Открыть печатную версию и сохранить через системный диалог PDF');
        } });
    }
    function init() {
        improveLabels();
