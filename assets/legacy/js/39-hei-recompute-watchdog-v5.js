...
(function () {
    ...
    function doRecompute(reason) {
        try {
            if (!window.__heiInst && typeof window.initHEI === 'function')
                window.initHEI();
            if (window.__heiInst && typeof window.__heiInst.recompute === 'function') {
                window.__heiInst.recompute();
                try {
                    window.dispatchEvent(new CustomEvent('hei:watchdog-recomputed', { detail: { reason: reason || 'unknown' } }));
                }
                catch (_) { }
            }
        }
    ...
    document.addEventListener('input', function (e) {
        var t = e && e.target;
        if (t && t.matches && t.matches('[data-ration-key], .ration-row input, input[type="number"]'))
            setTimeout(function () { check('input'); }, 280);
    }, true);
