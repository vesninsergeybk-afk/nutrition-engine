...
(function () {
    ...
    function syncControls(theme) {
        ...
        if (toggleText)
            toggleText.textContent = isRetro ? '2-bit' : 'Современная';
        var status = document.getElementById('themeStatus');
        if (status)
            status.textContent = isRetro ? 'Двухбитное оформление включено' : 'Современное оформление включено';
    ...
    function setTheme(theme, options) {
        ...
        if (previous !== theme && options.silent !== true) {
            try {
                window.dispatchEvent(new CustomEvent('nutrition:themechange', { detail: { theme: theme, previous: previous, version: VERSION } }));
            }
            catch (_) { }
        }
        return theme;
