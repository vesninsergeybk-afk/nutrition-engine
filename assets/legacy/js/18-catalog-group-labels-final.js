...
(function () {
    ...
    function labelKey(key, mode) {
        ...
        if (mode !== 'hei' && RAW_CATALOG_LABELS[k])
            return RAW_CATALOG_LABELS[k];
        if (mode === 'hei' && RAW_CATALOG_LABELS[k])
            return RAW_CATALOG_LABELS[k];
        if (window.UI_RU_LABELS) {
            ...
            if (typeof fn === 'function') {
                var label = fn(k);
                if (label && !/^Прочее$|^Другое$/.test(label))
                    return label;
            }
        }
    ...
    function keysInCurrentDomOrder(mode, count) {
        ...
        if (fromData.length === heads.length && fromData.length)
            return fromData;
    ...
    function repairCatalogLabels() {
        ...
        heads.forEach(function (head, idx) {
            ...
            var n = 0;
            try {
                n = groupMap && groupMap.get && groupMap.get(key) ? groupMap.get(key).length : Number((head.querySelector('.cat-count') || {}).textContent || 0);
            }
            catch (_) { }
    ...
    function installObserver() {
        var categories = byId('categories');
        if (!categories || categories.dataset.v453Observed === '1')
            return;
    ...
    function init() {
        ...
        if (seg)
            seg.addEventListener('click', function () { window.setTimeout(scheduleRepair, 0); window.setTimeout(scheduleRepair, 80); }, true);
        var additional = byId('additionalToolsSection');
        if (additional)
            additional.addEventListener('toggle', function () { window.setTimeout(scheduleRepair, 0); }, true);
        var select = byId('selectSection');
        if (select)
            select.addEventListener('toggle', function () { window.setTimeout(scheduleRepair, 0); }, true);
