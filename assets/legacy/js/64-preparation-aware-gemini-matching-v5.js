...
(function () {
    ...
    function detectByAliases(value, aliases) { var n = ' ' + norm(value) + ' '; if (n.trim() === '')
        return ''; for (var i = 0; i < aliases.length; i++) {
        aliases[i][1].lastIndex = 0;
        if (aliases[i][1].test(n))
            return aliases[i][0];
    } return ''; }
    ...
    function detectSkin(value) { var e = explicitEnum('skin_state', value); if (e)
        return e; var n = norm(value); if (/(?:^|\s)(?:без кожи|skinless)(?:$|\s)/.test(' ' + n + ' '))
        return 'skinless'; if (/(?:^|\s)(?:с кожей|кожа сохранена|skin on)(?:$|\s)/.test(' ' + n + ' '))
        return 'with_skin'; return ''; }
    function detectBreading(value) { var e = explicitEnum('breading_state', value); if (e)
        return e; var n = norm(value); if (/без паниров|unbreaded/.test(n))
        return 'unbreaded'; if (/в паниров|панирован|breaded/.test(n))
        return 'breaded'; return ''; }
    function detectDrain(value) { var e = explicitEnum('drain_state', value); if (e)
        return e; var n = norm(value); if (/без заливки|жидкость слита|рассол слит|drained/.test(n))
        return 'drained'; if (/с заливкой|в рассоле|in liquid/.test(n))
        return 'not_drained'; return ''; }
    function detectDoneness(value) { var e = explicitEnum('doneness_state', value); if (e)
        return e; var n = norm(value); if (/всмятку|soft boiled/.test(n))
        return 'soft'; if (/вкрутую|hard boiled/.test(n))
        return 'hard'; if (/medium|средн.*прожарк/.test(n))
        return 'medium'; return ''; }
    function detectStorage(value) { var e = explicitEnum('storage_state', value); if (e)
        return e; var n = norm(value); if (/заморож|frozen/.test(n))
        return 'frozen'; if (/охлажден|охлаждён|chilled/.test(n))
        return 'chilled'; if (/свеж|fresh/.test(n))
        return 'fresh'; return ''; }
    function detectWeight(value) { var e = explicitEnum('weight_basis', value); if (e)
        return e; var n = norm(value); if (/готов.*масс|после приготов|cooked weight/.test(n))
        return 'cooked_edible_weight'; if (/сыр.*масс|до приготов|raw weight/.test(n))
        return 'raw_edible_weight'; if (/сух.*масс|до варки|dry weight/.test(n))
        return 'dry_weight'; if (/после слив|drained weight/.test(n))
        return 'drained_edible_weight'; return ''; }
    ...
    function parseIntent(input) {
        ...
        if (intent.preparation_method !== 'unspecified' && !intent.preparation_confidence)
            intent.preparation_confidence = input.preparation_method ? 0.9 : 0.72;
        intent.methodRequested = isRequestedMethod(intent.preparation_method);
    ...
    function familyFromRecognition(recognition, query) {
        ...
        var text = recognitionText(recognition, query);
        var direct = String(recognition.food_family_id || '').trim();
        var candidates = api && api.resolveFamilyCandidates ? api.resolveFamilyCandidates(text) : [];
        var inferred = candidates && candidates[0] || null;
        if (direct && api && api.getFamily(direct)) {
            var directFamily = api.getFamily(direct);
            if (inferred && inferred.family && inferred.family.food_family_id !== direct)
                return { conflict: true, directFamily: directFamily, inferred: inferred, text: text };
            return { family: directFamily, quality: 'direct_family_id_validated', score: 120 };
        }
        if (direct)
            return { conflict: true, directFamily: null, inferred: inferred, text: text, unknownDirect: true };
        if (recognition.product_family_hint && api && api.getFamily(recognition.product_family_hint)) {
            var hinted = api.getFamily(recognition.product_family_hint);
            if (inferred && inferred.family && inferred.family.food_family_id !== hinted.food_family_id)
                return { conflict: true, directFamily: hinted, inferred: inferred, text: text };
            return { family: hinted, quality: 'direct_family_hint_validated', score: 115 };
        }
        return inferred;
    }
    ...
    function resolveRecognition(recognition, query) {
        recognition = recognition || {};
        var intent = parseIntent(Object.assign({}, recognition, { text: [query, recognition.preparation_state].filter(Boolean).join(' ') }));
        var serverStatus = String(recognition.preparation_match_status || '');
        var serverKey = String(recognition.exact_variant_key || '');
        var hit = familyFromRecognition(recognition, query);
        if (hit && hit.conflict)
            return Object.freeze({ status: 'family_conflict', family: null, familyId: '', intent: intent, exactKey: '', requiresConfirmation: true, reason: 'Возвращённое семейство не согласуется с названием продукта.' });
        if (!hit)
            return Object.freeze({ status: 'family_unmanaged', family: null, familyId: '', intent: intent, exactKey: '', requiresConfirmation: intent.methodRequested, reason: intent.methodRequested ? 'Для точного способа приготовления нет управляемого семейства.' : '' });
        var family = hit.family || hit;
        if (serverStatus === 'not_found' || serverStatus === 'ambiguous' || serverStatus === 'conflict')
            return Object.freeze({ status: serverStatus === 'conflict' ? 'family_conflict' : serverStatus, family: family, familyId: family.food_family_id, intent: intent, exactKey: '', candidates: [], requiresConfirmation: true, reason: 'Сервер заблокировал автоматическую подстановку.' });
        ...
        if (!intent.methodRequested)
            return Object.freeze({ status: 'family_only', family: family, familyId: family.food_family_id, intent: intent, exactKey: '', requiresConfirmation: false });
    ...
    function assessProduct(productKey, recognition, query) {
        ...
        if (resolved.status === 'family_conflict')
            return { boost: -1.1, hardReject: true, exact: false, resolution: resolved, reason: 'семейство продукта противоречит распознанному названию' };
        if (!resolved.familyId || !resolved.intent.methodRequested)
            return { boost: 0, hardReject: false, exact: false, resolution: resolved, reason: resolved.requiresConfirmation ? 'способ приготовления требует ручного подтверждения' : '' };
        var meta = api.getVariantMeta(productKey);
        if (!meta)
            return { boost: -0.18, hardReject: true, exact: false, resolution: resolved, reason: 'вариант приготовления не подтверждён семейным реестром' };
        var family = api.getFamilyForProduct(productKey);
        if (!family || family.food_family_id !== resolved.familyId)
            return { boost: -1.0, hardReject: true, exact: false, resolution: resolved, reason: 'другое продуктовое семейство' };
        if (resolved.status === 'exact') {
            if (productKey === resolved.exactKey)
                return { boost: 0.72, hardReject: false, exact: true, resolution: resolved, reason: 'точно совпадает способ приготовления и нутритивный профиль' };
        ...
        if (resolved.status === 'exact_requires_confirmation') {
            if (productKey === resolved.exactKey)
                return { boost: 0.35, hardReject: false, exact: false, resolution: resolved, reason: 'профиль совместим, но содержит типовую рецептуру или расчётную модель; требуется подтверждение' };
            return { boost: -1.1, hardReject: true, exact: false, resolution: resolved, reason: 'несовместимый способ приготовления' };
        }
        if (resolved.status === 'compatible_method_class') {
            if (productKey === resolved.exactKey)
                return { boost: 0.20, hardReject: false, exact: false, resolution: resolved, reason: 'ближайший вариант; требуется подтверждение' };
            return { boost: -.9, hardReject: true, exact: false, resolution: resolved, reason: 'несовместимый способ приготовления' };
        }
        if (resolved.status === 'ambiguous') {
            var allowed = (resolved.candidates || []).some(function (x) { return x && x.product && x.product.key === productKey; });
            return { boost: allowed ? .10 : -.9, hardReject: !allowed, exact: false, resolution: resolved, reason: allowed ? 'совместимый вариант; требуется выбор' : 'несовместимый способ приготовления' };
        }
        if (resolved.status === 'not_found')
            return { boost: -1.0, hardReject: true, exact: false, resolution: resolved, reason: 'точного варианта приготовления нет в базе' };
    ...
    function validateSelection(productKey, recognition, query) {
        ...
        if (assessment.hardReject)
            return { ok: false, reason: 'preparation_incompatible', assessment: assessment };
        if (assessment.resolution && assessment.resolution.requiresConfirmation && !assessment.exact)
            return { ok: false, reason: 'preparation_confirmation_required', assessment: assessment };
    ...
    window.NutritionPreparationMatching = exported;
    try {
        window.dispatchEvent(new CustomEvent('nutrition:preparationmatchingready', { detail: { version: VERSION } }));
    }
    catch (_) { }
})();
