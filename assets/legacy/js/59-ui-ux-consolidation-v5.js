...
(function () {
    ...
    function ensureVersion() {
        ...
        if (!badge) {
            ...
            var subtitle = title.querySelector('.app-subtitle');
            if (subtitle)
                title.insertBefore(badge, subtitle);
            else
                title.appendChild(badge);
        }
        badge.classList.add('version-badge');
        if (badge.textContent !== version) badge.textContent = version;
        candidates.forEach(function (x) { if (x !== badge && /v?\d+\.\d+\.\d+/.test(x.textContent || ''))
            x.remove(); });
        document.documentElement.dataset.uiVersion = version.replace(/^v/, '');
    }
    function syncRegion() {
        var seg = byId('regionSeg');
        if (!seg)
            return;
        var current = (window.State && State.getRegion ? State.getRegion() : 'us') || 'us';
        seg.setAttribute('role', 'group');
        seg.setAttribute('aria-label', 'Региональные нормы микронутриентов');
    ...
    function createFold(id, summaryText, summaryHint, child, openDesktop) {
        ...
        fold.id = id;
        fold.className = 'ui-analysis-fold';
        if (openDesktop && matchMedia('(min-width:761px)').matches)
            fold.open = true;
        var summary = document.createElement('summary');
        summary.innerHTML = '<span><b>' + summaryText + '</b>' + (summaryHint ? '<small>' + summaryHint + '</small>' : '') + '</span><strong aria-hidden="true">Подробнее</strong>';
        child.parentNode.insertBefore(fold, child);
    ...
    function ensureHeiFolds() {
        ...
        if (mobile) {
            ...
            createFold('heiSettingsFold', 'Настройка расчёта HEI', 'По калорийности рациона, целевой или ручной энергии', settings, false);
        }
        else if (settingsFold) {
            var settingsRow = settingsFold.querySelector(':scope > .row');
            if (settingsRow) {
                settingsFold.parentNode.insertBefore(settingsRow, settingsFold);
            }
            settingsFold.remove();
        }
        ...
        if (before !== after) {
            try { window.dispatchEvent(new CustomEvent('ui:hei-folds-ready')); }
            catch (_) { }
        }
    }
    ...
    function dedupeRationPills() {
        document.querySelectorAll('#rationBody .meta').forEach(function (meta) {
            ...
            pills.forEach(function (p, index) {
                var key = normalizeLabel(p.textContent);
                var duplicate = seen.some(function (prev) { return key === prev || key.indexOf(prev) >= 0 || prev.indexOf(key) >= 0; });
                if (duplicate) {
                    p.hidden = true;
    ...
    function syncOrdinaryToggles() {
        document.querySelectorAll('#rationBody .ration-row:not(.composite-folder-row)').forEach(function (row) {
            var btn = row.querySelector('.regular-nutrition-toggle');
            if (!btn)
                return;
            var details = row.nextElementSibling;
            var open = !!(details && details.style.display !== 'none');
            var expanded = String(open);
            if (btn.getAttribute('aria-expanded') !== expanded)
                btn.setAttribute('aria-expanded', expanded);
        });
    }
    function improveDetailsLabels() {
        document.querySelectorAll('.ui-analysis-fold').forEach(function (d) { var action = d.querySelector(':scope>summary>strong'); if (action)
            action.textContent = d.open ? 'Свернуть' : 'Подробнее'; });
    }
    ...
    function ensureRecommendationLayer() {
        ...
        if (recommendationRepairBusy || !host || !model || model.valid === false || host.querySelector('.hei-p1-start'))
            return;
        var pass1 = window.HEIRecommendationPass1;
        if (!pass1 || typeof pass1.refresh !== 'function')
            return;
        ...
        setTimeout(function () {
            try {
                if (window.HEIRecommendationPass2 && typeof window.HEIRecommendationPass2.refresh === 'function')
                    window.HEIRecommendationPass2.refresh();
            }
            catch (_) { }
            try {
                if (window.HEIRecommendationLanguagePass3 && typeof window.HEIRecommendationLanguagePass3.polish === 'function')
                    window.HEIRecommendationLanguagePass3.polish();
            }
            catch (_) { }
            recommendationRepairBusy = false;
        }, 40);
    ...
    document.addEventListener('toggle', function (e) { if (e.target && e.target.classList && e.target.classList.contains('ui-analysis-fold'))
        improveDetailsLabels(); }, true);
    var printOpenState = [];
    ...
    window.addEventListener('afterprint', function () {
        printOpenState.forEach(function (x) { if (x.node && x.node.isConnected)
            x.node.open = x.open; });
        printOpenState = [];
        improveDetailsLabels();
    });
    document.addEventListener('click', function (e) { var b = e.target && e.target.closest && e.target.closest('#regionSeg button[data-region]'); if (b)
        setTimeout(syncRegion, 30); });
    var queued = false;
    function schedule() { if (queued)
        return; queued = true; setTimeout(function () { queued = false; sync(); }, 60); }
