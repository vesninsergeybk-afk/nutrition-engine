...
(function () {
    ...
    function setAttr(el, name, value) { if (el && !el.hasAttribute(name))
    ...
    function initStaticA11y() {
        ...
        if (clear)
            setAttr(clear, 'aria-label', 'Очистить строку поиска');
        ...
        if (ration)
            setAttr(ration, 'aria-label', 'Итоговый рацион');
        var totals = byId('totalsSection');
        if (totals)
            setAttr(totals, 'aria-label', 'Итоги и дневные нормы');
        var hei = byId('heiPanel');
        if (hei)
            setAttr(hei, 'aria-label', 'Индекс качества рациона HEI-2020');
        ...
        if (needsToggle) {
            ...
            needsToggle.setAttribute('aria-expanded', String(!((_a = byId('needsCompact')) === null || _a === void 0 ? void 0 : _a.classList.contains('v40-needs-collapsed'))));
        ...
        Object.keys(namedControls).forEach(function (id) { var el = byId(id); if (el)
            el.setAttribute('aria-label', namedControls[id]); });
        var totalsPdf = byId('exportTotalsPdfBtn');
        if (totalsPdf && totalsPdf.textContent.trim() === 'PDF')
            totalsPdf.textContent = 'Сохранить PDF';
    ...
    function enhanceSearchCards() {
        ...
        cards.forEach(function (card, idx) {
            ...
            if (btn)
                btn.setAttribute('aria-label', 'Добавить в рацион: ' + name);
        });
    }
    ...
    function auditA11y() {
        ...
        Object.keys(map).forEach(function (id) { if (map[id] > 1)
            dup.push(id + '×' + map[id]); });
        ...
        add('Строка поиска имеет label/aria', !!(byId('globalSearchInput') && (((_a = byId('globalSearchInput').labels) === null || _a === void 0 ? void 0 : _a.length) || byId('globalSearchInput').getAttribute('aria-label'))), 'globalSearchInput');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        try {
            ...
            if (params.has('a11y'))
                window.setTimeout(renderA11yAudit, 250);
        }
        catch (_) { }
    });
})();
