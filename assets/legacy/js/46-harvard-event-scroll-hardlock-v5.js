...
(function () {
    ...
    function harvardApis() {
        ...
        [
            ...
            if (window[name] && out.indexOf(window[name]) < 0)
        ...
        apis.forEach(function (api) {
            try {
                if (api && typeof api.scheduleRender === 'function') {
                    api.scheduleRender(reason || 'event-bridge');
                    ok = true;
                }
                else if (api && typeof api.render === 'function') {
                    api.render();
                    ok = true;
                }
            }
                ...
                try {
                    console.warn('[v5.3.36] Harvard render failed', e);
                }
                catch (_) { }
            }
        });
    ...
    function scheduleHarvard(reason){
      ...
      function patchStateMethods() {
        ...
        if (!api || api.__v5319HarvardPatched)
            return false;
    ...
    function initHarvardBridge() {
        ...
        (function waitState() {
            tries += 1;
            if (!patchStateMethods() && tries < 60)
                setTimeout(waitState, 100);
        })();
