...
(function(){
  'use strict';
  var RELEASE_GATE_VERSION = 'v5.3.210-rc2-hf27-microphone-resilience';
  var VERSION = 'v5.3.210_rc2_hosting_hotfix_27_microphone_resilience';
  ...
  // HF27: the splash follows the real full-start pipeline instead of closing at shell-ready.
  ...
  function loadCompressedRuntime(src,fallbackSrc){
    ...
    return fetch(src,{cache:'default',credentials:'same-origin',signal:controller?controller.signal:undefined}).then(function(resp){
      ...
      if(timer)clearTimeout(timer); try{console.warn('[HF27 resilient media loader] compressed runtime unavailable; using script bundle',error&&(error.message||error));}catch(_){}
  ...
  function loadCriticalShell(){
    ...
    return loadScriptsSequential(CRITICAL_SHELL_SCRIPTS).then(function(result){setBootProgress(1,1);return result;}).catch(function(err){
      ...
      try { console.warn('[HF27 resilient media loader] critical shell bundle unavailable; using source files', err && (err.message||err)); } catch(_) {}
  ...
  function loadRuntime(){
    ...
    if (DEFERRED_RUNTIME_BUNDLE) {
      mainRuntime = (DEFERRED_RUNTIME_COMPRESSED ? loadCompressedRuntime(DEFERRED_RUNTIME_COMPRESSED,DEFERRED_RUNTIME_BUNDLE) : loadScript(DEFERRED_RUNTIME_BUNDLE)).catch(function(err){
        ...
        try { console.warn('[HF27 resilient media loader] deferred bundle unavailable; using source files', err && (err.message||err)); } catch(_) {}
  ...
  function runReadinessChecks(){
    ...
    },Promise.resolve()).then(function(){window.__HF27_READINESS_CHECKS__=results;return results;});
  }
  function finish(){
