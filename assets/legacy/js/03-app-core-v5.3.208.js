...
const Logger = (() => {
    ...
    const log = (level, event, payload) => {
        ...
        logs.push(item);
        if (DEV)
            console[level === 'error' ? 'error' : 'log'](`[${level}] ${event}`, payload !== null && payload !== void 0 ? payload : '');
    };
    ...
    try {
      ...
      if (native) {
        try {
          ...
          lastError = e && e.message ? e.message : String(e || 'localStorage write test failed');
          // Read access may still work when quota or browser policy blocks a new write.
          nativeWritable = false;
        }
      }
    } catch (e) {
      lastError = e && e.message ? e.message : String(e || 'localStorage unavailable');
      native = null;
      nativeReadable = false;
      nativeWritable = false;
    }
    function own(obj, key){ return Object.prototype.hasOwnProperty.call(obj, key); }
    function readNative(key){
      if (!nativeReadable || !native) return null;
      try { return native.getItem(key); }
      catch (e) { lastError = e && e.message ? e.message : String(e || 'localStorage read failed'); return null; }
    }
    return {
      available: () => !!nativeReadable,
      writable: () => !!nativeWritable,
      status: () => ({ readable:!!nativeReadable, writable:!!nativeWritable, using_overlay:Object.keys(overlay).length>0, last_error:lastError }),
      getItem: (key) => {
        key = String(key);
        if (own(overlay, key)) return overlay[key];
        const value = readNative(key);
        if (value !== null) return value;
        return own(memory, key) ? memory[key] : null;
      },
      getNativeItem: (key) => readNative(String(key)),
      setItem: (key, value) => {
        ...
        if (native) {
          try {
            ...
            lastError = e && e.message ? e.message : String(e || 'localStorage write failed');
            nativeWritable = false;
          }
        }
        overlay[key] = value;
      ...
      removeItem: (key) => {
        ...
        delete memory[key];
        if (native) {
          try { native.removeItem(key); nativeReadable = true; return true; }
          catch (e) { lastError = e && e.message ? e.message : String(e || 'localStorage remove failed'); }
        }
        return false;
      }
    };
...
function metricMode(key) {
    key = String(key || '');
    if (INFORMATIONAL_METRICS.has(key))
...
function metricDefinition(key, normOverride) {
  ...
  // Единый нормативный реестр P1.2. Числовые ориентиры и provenance не дублируются в app-core.
  const MICRO_LIMITS = (window.NutritionNormativeRegistry && typeof window.NutritionNormativeRegistry.getMicroLimitsMap === 'function')
    ? window.NutritionNormativeRegistry.getMicroLimitsMap() : {};
  ...
  function normativeEnergyContext(){
    var kcal=null;
    try{if(window.NeedsNormSyncV53155&&typeof window.NeedsNormSyncV53155.selectedEnergy==='function')kcal=Number(window.NeedsNormSyncV53155.selectedEnergy());}catch(_){}
    if(!Number.isFinite(kcal)||kcal<=0){try{kcal=Number(window.norms&&window.norms.values&&window.norms.values.kcal&&window.norms.values.kcal.value);}catch(_){kcal=null;}}
  ...
  function resolveRegionalMicroLimits(key,region,age,sex){
    ...
    if(!registry||typeof registry.resolveMicro!=='function'){
      return {min:null,ul:null,safe:null,basis:'Нормативный реестр не загружен',profile:'fail closed',note:'Числовой ориентир намеренно не подставлен.',coverageStatus:'registry_missing',provenance:{registryVersion:null,reviewedAt:null,ruleId:'registry_missing',sourceIds:[]}};
    }
    return registry.resolveMicro(String(key||''),String(region||'us'),{age:Number(age),sex:String(sex||'male'),energyKcal:normativeEnergyContext()});
  }
...
function isSaturatedFatRouteProduct(product) {
    ...
    if (catalog === 'saturated_fats' || fatQuality === 'saturated_fat_source' || limitGroup === 'saturated_fats')
        return true;
    if (tags.includes('saturated_fat_source') || tags.includes('limit_saturated_fat'))
        return true;
...
function isProteinSupplementRouteProduct(product) {
    if (isSoyProteinWithHeiCredit(product))
        return false;
    const tags = productTagsArray(product);
    const name = catalogNameText(product);
    const catalog = String((product && product.catalog_category_key) || '').toLowerCase();
    if (catalog === 'protein_supplements')
        return true;
    if (tags.includes('protein_supplement') || tags.includes('sports_protein') || tags.includes('no_hei_protein_food_credit'))
        return true;
...
function classifySpecialCatalog(product) {
    ...
    if (CATALOG_OVERRIDES[key])
        return CATALOG_OVERRIDES[key];
    const tags = productTagsArray(product);
    const name = catalogNameText(product);
    ...
    if (category === 'Protein Drink')
        return 'protein_drinks';
    if (category === 'Sports Protein')
        return 'sports_protein';
    const isVegLike = category === 'Vegetables' || (!category && String((product && product.hei_category_key) || '') === 'vegetables');
    if (isVegLike) {
        if (tags.includes('herbs') || tags.includes('leafy') || /зелень|листов|шпинат|руккол|салат|латук|кейл|мангольд|кресс|мизуна|щавел|кинза|петрушк|укроп|базилик|лук зел[её]н/i.test(name))
            return 'greens_leafy';
        if (tags.includes('starchy-veg') || /картоф|батат|кукуруз/i.test(name))
            return 'starchy_vegetables';
        if (tags.includes('fungi') || /гриб|шампиньон|в[её]шен|шиитак|эноки|белые/i.test(name))
            return 'mushrooms';
        if (tags.includes('legumes') || /горошек|горох сахарн|фасоль стручков|эдамаме/i.test(name))
            return 'legume_vegetables';
        if (/лук|чеснок|имбир|хрен|порей/i.test(name))
            return 'allium_aromatic';
        if (tags.includes('root') || /морков|св[её]кл|редис|редьк|реп[аы]|брюкв|дайкон|пастернак|топинамбур/i.test(name))
            return 'root_vegetables';
        if (tags.includes('brassica') || /капуст|броккол|кольраби/i.test(name))
            return 'cruciferous_vegetables';
        return '';
    }
    if (isSaturatedFatRouteProduct(product))
        return 'saturated_fats';
    if (tags.includes('salty_snack') || /чипс|попкорн|картофел[ья]\s+фри/.test(name))
        return 'snacks_salty';
    ...
    if (tags.includes('ready_food') || category === 'Ready food / Culinary')
        return 'ready_food';
    if (tags.includes('prepared') && /фалафель|лапша быстрого приготовления|готов/.test(name))
        return 'ready_food';
    ...
    if ((category === 'Other' || category === '') && (tags.includes('pickled') || tags.includes('fermented') || /квашен|маринован|сол[её]н/.test(name)))
        return 'preserves_pickles';
    if (tags.includes('fruit_snack') || /смоква яблочн/.test(name))
        return 'fruits_berries';
...
function classifyCatalog(product) {
    try {
        ...
        if (st === "pickled" || st === "fermented")
            return "preserves_pickles";
        if (st === "oil" || st === "butter")
            return "oils_fats";
        if (st === "fastfood")
            return "ready_fastfood";
        const cat = (product.category || "").trim();
        if (MAP_BY_CATEGORY[cat])
            return MAP_BY_CATEGORY[cat];
        const hei = (product.hei_category_key || "").trim().toLowerCase();
        if (MAP_BY_HEI[hei])
            return MAP_BY_HEI[hei];
        const tags = productTagsArray(product);
        if (tags.includes("pickled") || tags.includes("fermented"))
            return "preserves_pickles";
        if (tags.includes("oil") || tags.includes("butter"))
            return "oils_fats";
        if (tags.some(t => ["vegetables", "leafy", "root", "brassica", "starchy-veg", "legume-veg", "fungi"].includes(t)))
            return "vegetables";
        if (tags.includes("drink"))
            return "drinks";
        if (tags.some(t => ["nuts", "seeds", "legume"].includes(t)))
            return "nuts_seeds";
        ...
        CatalogDiag.otherCount++;
        CatalogDiag.unknowns.push({ key: (product === null || product === void 0 ? void 0 : product.key) || "", error: String(e) });
        return "other";
    }
}
...
window.upsertFood = function upsertFood(product) {
    if (!product || !product.key || !window.DB || !Array.isArray(window.DB.items) || !window.DB.byKey)
        return null;
    ...
    if (typeof classifyCatalog === 'function')
        item.catalog_category_key = classifyCatalog(item);
    ...
    for (const o of window.DB.items) {
        const hei = o.hei_category_key || 'other';
        if (!window.DB.byCatHEI.has(hei))
            window.DB.byCatHEI.set(hei, []);
        window.DB.byCatHEI.get(hei).push(o);
        const cat = o.catalog_category_key || 'other';
        if (!window.DB.byCatCatalog.has(cat))
            window.DB.byCatCatalog.set(cat, []);
        window.DB.byCatCatalog.get(cat).push(o);
    }
    const byName = (a, b) => (a.name_ru || '').localeCompare(b.name_ru || '', 'ru');
    for (const arr of window.DB.byCatHEI.values())
        arr.sort(byName);
    for (const arr of window.DB.byCatCatalog.values())
        arr.sort(byName);
    return item;
};
...
try {
    ...
    const existingProtein = Number(existingPrunes && ((_b = (_a = existingPrunes.protein_per_100g) !== null && _a !== void 0 ? _a : existingPrunes.protein_g) !== null && _b !== void 0 ? _b : existingPrunes.protein)) || 0;
    const existingFat = Number(existingPrunes && ((_d = (_c = existingPrunes.fat_per_100g) !== null && _c !== void 0 ? _c : existingPrunes.fat_g) !== null && _d !== void 0 ? _d : existingPrunes.fat)) || 0;
    const existingCarbs = Number(existingPrunes && ((_f = (_e = existingPrunes.carbs_per_100g) !== null && _e !== void 0 ? _e : existingPrunes.carbohydrate_g) !== null && _f !== void 0 ? _f : existingPrunes.carbs)) || 0;
    ...
    console.warn('Prunes legacy rescue failed', e);
}
window.validateFoodDb = function validateFoodDb(items) {
    ...
    for (const p of items) {
        if (!p || typeof p !== 'object') {
            critical.push({ type: 'bad-record', item: p });
            continue;
        }
        for (const k of required) {
            if (!p[k])
                warnings.push({ type: 'missing-field', key: p.key || '', field: k });
        }
        if (p.key) {
            if (seen.has(p.key))
                critical.push({ type: 'duplicate-key', key: p.key });
            seen.add(p.key);
        }
        else {
        ...
        const protein = Number((_b = (_a = p.protein_per_100g) !== null && _a !== void 0 ? _a : p.protein_g) !== null && _b !== void 0 ? _b : p.protein) || 0;
        const fat = Number((_d = (_c = p.fat_per_100g) !== null && _c !== void 0 ? _c : p.fat_g) !== null && _d !== void 0 ? _d : p.fat) || 0;
        const carbs = Number((_g = (_f = (_e = p.carbs_per_100g) !== null && _e !== void 0 ? _e : p.carbohydrate_g) !== null && _f !== void 0 ? _f : p.carb_g) !== null && _g !== void 0 ? _g : p.carbs) || 0;
        ...
        if (kcal > 20 && (protein > 0 || fat > 0 || carbs > 0) && !p.kcal_macro_validation_exempt) {
            const fiber = Number((_j = (_h = p.fiber_per_100g) !== null && _h !== void 0 ? _h : p.fiber_g) !== null && _j !== void 0 ? _j : p.fiber) || 0;
            ...
            if (rel > 0.40)
                warnings.push({ type: 'kcal-macro-mismatch', key: p.key, kcal: kcal, estimated_kcal: Math.round(estimated), relative_delta: Number(rel.toFixed(2)) });
        }
        ...
        for (const bad of ['fruit', 'vegetable', 'whole_grain', 'refined_grain', 'legume', 'fish', 'shellfish', 'nut', 'seed']) {
            if (tagsRaw.includes(bad))
                warnings.push({ type: 'legacy-tag-normalized', key: p.key, tag: bad, normalized: norm(bad) });
        }
...
try {
    window.validateFoodDb();
}
catch (e) {
    console.warn('DB validation failed', e);
}
...
function rationForCalculationV5377(ration) {
    try {
        ...
        if (api && typeof api.flattenForCalculation === 'function')
            return api.flattenForCalculation(ration || []);
        if (api && typeof api.flattenCompositeForCalculation === 'function')
            return api.flattenCompositeForCalculation(ration || []);
    }
    catch (_) { }
...
function isCustomizedCompositeV53107(entry) {
    return !!(entry && (entry.recipe_instance_type === 'customized_recipe_instance' ||
...
window.initHEI = function initHEI() {
    ...
    if (!container || !window.HEI || !window.DB || !window.DB.byKey)
        return null;
    if (window.__heiInst && typeof window.__heiInst.recompute === 'function')
        return window.__heiInst;
    function getRation() {
        ...
        try {
            if (window.State && typeof window.State.get === 'function')
                return rationForCalculationV5377(window.State.get());
            if (typeof State !== 'undefined' && State && typeof State.get === 'function')
                return rationForCalculationV5377(State.get());
        }
        catch (_) { }
        if (window.Ration && Array.isArray(window.Ration.current))
            return window.Ration.current;
        if (window.STATE && Array.isArray(window.STATE.ration))
            return window.STATE.ration;
        return [];
    }
    function calcKcalFromRation() {
        const r = rationForNutrientCalculationV53107((window.State && typeof window.State.get === 'function') ? window.State.get() : getRation());
        let kcal = 0;
        if (!window.DB || !window.DB.byKey)
            return 0;
        for (const { key, grams } of (r || [])) {
            const p = window.DB.byKey.get(key);
            if (!p)
                continue;
    ...
    function getEnergyRef() {
        ...
        if (targetRadio && targetRadio.checked) {
            ...
            if (Number.isFinite(v) && v >= 800 && v <= 5000)
                return v;
            if (window.Needs && Number.isFinite(window.Needs.targetKcal))
                return window.Needs.targetKcal;
            return 2000;
        }
        if (window.Needs && Number.isFinite(window.Needs.targetKcal))
            return window.Needs.targetKcal;
    ...
    const recomputeNow = () => { try {
        inst.recompute();
    }
    catch (e) {
        console.error('HEI recompute error', e);
    } };
    const recompute = () => {
...
const Schema = (() => {
    function validate() {
        ...
        for (const o of DB.items) {
            for (const k of req)
                if (!(k in o))
                    issues.push({ key: o.key, type: 'missing', field: k });
            if (seen.has(o.key))
                issues.push({ key: o.key, type: 'duplicate', field: 'key' });
            seen.add(o.key);
            for (const [k, v] of Object.entries(o))
                if (typeof v === 'number' && !Number.isFinite(v))
                    issues.push({ key: o.key, type: 'nan', field: k });
        }
        return issues;
    }
    return { validate };
...
const State = (() => {
    ...
    function boundedPush(list, value, max) {
        ...
        if (!list.some(item => JSON.stringify(item) === signature) && list.length < (max || 80))
            list.push(value);
    }
    function publishPreparationMigrationReport() {
        try {
            window.__PREPARATION_RATION_MIGRATION_REPORT_V53190__ = JSON.parse(JSON.stringify(preparationMigrationReport));
        }
        catch (_) { }
    }
    function preparationKeyDecision(rawKey) {
        ...
        if (!key)
            return { status: 'missing', requested_key: key, key: '' };
        if (hasOwn(PREPARATION_REDIRECTS, key)) {
            const target = String(PREPARATION_REDIRECTS[key] || '');
            if (target && target !== key && DB.byKey.has(target))
                return { status: 'redirect', requested_key: key, key: target };
            return { status: 'blocked', requested_key: key, key: '', reason: 'redirect_target_missing' };
        }
        if (PREPARATION_BLOCKED.has(key))
            return { status: 'blocked', requested_key: key, key: '', reason: 'unsupported_profile' };
        return { status: 'exact', requested_key: key, key: key };
    }
    function applyCanonicalProductIdentity(entry, decision, context) {
        if (!entry || !decision || decision.status !== 'redirect')
            return entry;
        ...
        if (!entry.id || entry.id === oldKey)
            entry.id = product.key;
    ...
    function mergeStoredAiImport(target, source) {
        if (!target || !source || !source.ai_import || typeof source.ai_import !== 'object')
            return target;
        if (!target.ai_import || typeof target.ai_import !== 'object') {
            target.ai_import = source.ai_import;
            return target;
        }
        ...
        if (Number.isFinite(a) && Number.isFinite(b))
            target.ai_import.confidence = Math.min(a, b);
        else if (!Number.isFinite(a) && Number.isFinite(b))
            target.ai_import.confidence = b;
        if (target.ai_import.source_type && source.ai_import.source_type && target.ai_import.source_type !== source.ai_import.source_type)
            target.ai_import.source_type = 'mixed';
        return target;
    }
    function normalizeStoredPreparationEntry(entry, context) {
        ...
        if (decision.status === 'redirect') {
            ...
            if (!applyCanonicalProductIdentity(entry, decision, context))
                return null;
        }
    ...
    function normalizeStoredList(value, maxItems, maxChars) {
        ...
        src.forEach(v => {
            ...
            if (text && !out.includes(text) && out.length < (maxItems || 8))
                out.push(text);
        });
        return out;
    }
    function normalizeStoredAiImport(raw) {
        ...
        Object.keys(listFields).forEach(key => {
            ...
            let value = ai[key];
            if (key === 'meal_labels' && (value == null || value === ''))
                value = ai.meal_label || ai.meal || '';
            if (key === 'observed_names' && (value == null || value === ''))
                value = ai.observed_name || ai.original_name || '';
            if (key === 'source_transcripts' && (value == null || value === ''))
                value = ai.source_transcript || '';
        ...
        function fallbackStoredEntry(entry, context) {
          if (!entry || typeof entry !== 'object') return null;
          const key = String(entry.key || entry.id || '');
          const grams = Math.max(0, Math.min(5000, Number(entry.grams) || 0));
          if (!key || !(grams > 0)) return null;
          const decision = preparationKeyDecision(key);
          if (decision.status === 'blocked') return null;
          const resolvedKey = decision.status === 'redirect' && decision.key ? decision.key : key;
          const product = DB.byKey.get(resolvedKey) || DB.byKey.get(key);
          if (!product && !entry.name_ru && !entry.name) return null;
          const out = Object.assign({}, entry, {
            id: entry.id || resolvedKey,
            key: resolvedKey,
            name_ru: (product && product.name_ru) || entry.name_ru || entry.name || resolvedKey,
            hei_category_key: (product && product.hei_category_key) || entry.hei_category_key || 'other',
            catalog_category_key: (product && product.catalog_category_key) || entry.catalog_category_key || entry.category || 'other',
            grams: grams
          });
          if (Array.isArray(entry.children)) {
            out.children = entry.children.map(child => fallbackStoredEntry(child, 'composite_child')).filter(Boolean);
          }
          boundedPush(preparationMigrationReport.load_recovered_entries, { key:key, resolved_key:resolvedKey, context:String(context || 'saved_ration') });
          preparationMigrationReport.changed = true;
          return out;
        }
        function parseStoredRationRaw(raw) {
          if (raw == null || raw === '') return [];
          const parsed = JSON.parse(String(raw));
          if (Array.isArray(parsed)) return parsed;
          if (parsed && Array.isArray(parsed.items)) return parsed.items;
          if (parsed && Array.isArray(parsed.ration)) return parsed.ration;
          return [];
        }
        function load() {
          let raw = null;
          let parsed = [];
          let parseError = null;
          try { raw = SafeStorage.getItem(LS); } catch (e) { parseError = e; }
          try { parsed = parseStoredRationRaw(raw); }
          catch (e) { parseError = e; parsed = []; }
          // If the safe wrapper had to fall back after a failed write probe, still try
          // the browser's readable native value before declaring the ration empty.
          if (!parsed.length && SafeStorage.getNativeItem) {
            try {
              const nativeRaw = SafeStorage.getNativeItem(LS);
              if (nativeRaw && nativeRaw !== raw) parsed = parseStoredRationRaw(nativeRaw);
            } catch (e) { if (!parseError) parseError = e; }
          }
          const normalized = [];
          (Array.isArray(parsed) ? parsed : []).forEach((entry, index) => {
            try {
              const value = normalizeStoredRationEntry(entry);
              if (value) normalized.push(value);
            } catch (e) {
              boundedPush(preparationMigrationReport.load_errors, { index:index, key:String(entry && (entry.key || entry.id) || ''), message:String(e && e.message || e) });
              const fallback = fallbackStoredEntry(entry, 'load_error_fallback');
              if (fallback) normalized.push(fallback);
            }
          });
          ration = consolidateStoredRation(normalized);
          if (!ration.length && parsed.length) {
            const recovered = parsed.map((entry, index) => {
              try { return fallbackStoredEntry(entry, 'empty_normalization_fallback'); }
              catch (e) { boundedPush(preparationMigrationReport.load_errors, { index:index, key:String(entry && (entry.key || entry.id) || ''), message:String(e && e.message || e) }); return null; }
            }).filter(Boolean);
            if (recovered.length) ration = consolidateStoredRation(recovered);
          }
          if (parseError) {
            preparationMigrationReport.load_parse_error = String(parseError && parseError.message || parseError);
          }
          try { region = SafeStorage.getItem(LS+'_region') || 'us'; } catch (_) { region = 'us'; }
          // Save only a successfully recovered or intentionally migrated ration.
          // Never overwrite a non-empty native ration with an empty array after a load error.
          if (preparationMigrationReport.changed && (ration.length || !parsed.length)) {
            try { save(); } catch(_) {}
          }
          publishPreparationMigrationReport();
        }
    ...
    function forceHEIRecompute(reason) {
        ...
        setTimeout(function () {
            try {
                if ((Number(window.__lastHEIRenderedAt) || 0) >= requestedAt)
                    return;
                if (window.__heiInst && typeof window.__heiInst.recompute === 'function') {
                    ...
                    window.initHEI();
                    if ((Number(window.__lastHEIRenderedAt) || 0) < requestedAt && window.__heiInst && typeof window.__heiInst.recompute === 'function')
                        window.__heiInst.recompute();
                }
                try {
                    window.dispatchEvent(new CustomEvent('hei:forced-recompute', { detail: { reason: reason, fallback: true } }));
                }
                catch (_) { }
            }
            catch (e) {
                console.error('HEI forced recompute failed', e);
            }
        }, 220);
    }
    function emitRationChanged(action, detail) {
        ...
        try {
            window.dispatchEvent(new CustomEvent('ration:changed', { detail: payload }));
        }
        catch (_a) { }
    ...
    function buildCompositeEntryIfNeeded(p, grams) {
        try {
            ...
            if (api && typeof api.createEntryFromProduct === 'function')
                return api.createEntryFromProduct(p, grams);
        }
        catch (e) {
            console.warn('Composite entry creation failed', e);
        }
        return null;
    }
    ...
    function mergeImportMeta(target, rawMeta) {
        ...
        function add(key, grams, importMeta) {
          const requestedKey = String(key == null ? '' : key);
          const decision = resolveRuntimePreparationKey(requestedKey, 'State.add');
          if (decision.status === 'blocked' || !decision.key) {
            Logger.warn('PREPARATION_PROFILE_BLOCKED', { key:requestedKey, context:'State.add', reason:decision.reason || 'blocked' });
            try { window.dispatchEvent(new CustomEvent('ration:add-failed', { detail:{ key:requestedKey, reason:decision.reason || 'blocked' } })); } catch(_) {}
            return null;
          }
          key = decision.key;
          let p = DB.byKey.get(key);
          if (!p && requestedKey && DB.byKey.has(requestedKey)) {
            key = requestedKey;
            p = DB.byKey.get(requestedKey);
          }
          if (!p && Array.isArray(DB.items)) {
            p = DB.items.find(item => item && item.key === key) || DB.items.find(item => item && item.key === requestedKey) || null;
            if (p && p.key) key = p.key;
          }
          if (!p) {
            Logger.warn('ADD_PRODUCT_NOT_FOUND', { requested_key:requestedKey, resolved_key:key });
            try { window.dispatchEvent(new CustomEvent('ration:add-failed', { detail:{ key:requestedKey, resolved_key:key, reason:'product_not_found' } })); } catch(_) {}
            return null;
          }
          const g = Math.max(0, Math.min(5000, Number(grams)||0));
          if (!(g > 0)) {
            try { window.dispatchEvent(new CustomEvent('ration:add-failed', { detail:{ key:key, reason:'invalid_grams', grams:grams } })); } catch(_) {}
            return null;
          }
    ...
    function update(ref, grams) {
        ...
        save();
        Events.emit('change');
        Logger.info('UPDATE_ITEM', { ref, grams: nextGrams });
        emitRationChanged('update', { ref, grams: nextGrams });
    }
    function markCompositeCustomized(parent, event) {
        if (!parent)
            return parent;
        if (window.CompositeFoodFolderV5377 && typeof window.CompositeFoodFolderV5377.markCustomized === 'function')
            return window.CompositeFoodFolderV5377.markCustomized(parent, event);
        parent.edited = true;
        parent.customized = true;
        parent.recipe_instance_type = 'customized_recipe_instance';
        if (!Array.isArray(parent.customization_events))
            parent.customization_events = [];
        if (event)
            parent.customization_events.push(Object.assign({ at: new Date().toISOString(), pass: 'v5.3.88' }, event));
        return parent;
    }
    function updateCompositeChild(parentRef, childRef, grams) {
        const idx = findIndex(parentRef);
        if (idx < 0)
            return;
        const parent = ration[idx];
        if (!isCompositeEntryV5377(parent) || !Array.isArray(parent.children))
            return;
        const child = parent.children.find(c => c && (c.id === childRef || c.key === childRef));
        if (!child)
            return;
        ...
        if (window.CompositeFoodFolderV5377 && typeof window.CompositeFoodFolderV5377.recalculateRootWeight === 'function')
            parent.grams = window.CompositeFoodFolderV5377.recalculateRootWeight(parent.children);
        save();
        Events.emit('change');
        Logger.info('UPDATE_COMPOSITE_CHILD', { parentRef, childRef, grams: child.grams });
        emitRationChanged('updateCompositeChild', { parentRef, childRef, grams: child.grams });
    }
    function removeCompositeChild(parentRef, childRef) {
        ...
        save();
        Events.emit('change');
        Logger.info('REMOVE_COMPOSITE_CHILD', { parentRef, childRef });
        emitRationChanged('removeCompositeChild', { parentRef, childRef });
    }
    function addCompositeChild(parentRef, productKey, grams) {
        ...
        save();
        Events.emit('change');
        Logger.info('ADD_COMPOSITE_CHILD', { parentRef, productKey, grams: g });
        emitRationChanged('addCompositeChild', { parentRef, productKey, grams: g });
    }
    function replaceCompositeChild(parentRef, childRef, productKey, grams) {
        ...
        if (!old)
            return;
        const g = Math.max(0, Math.min(5000, Number(grams) || Number(old.grams) || 0));
        if (!(g > 0))
            return;
        ...
        save();
        Events.emit('change');
        Logger.info('REPLACE_COMPOSITE_CHILD', { parentRef, childRef, productKey, grams: g });
        emitRationChanged('replaceCompositeChild', { parentRef, childRef, productKey, grams: g });
    }
    function resetCompositeEntry(parentRef) {
        ...
        if (!isCompositeEntryV5377(parent))
            return;
        const p = DB.byKey.get(parent.key);
        if (!p)
            return;
        const grams = Number(parent.grams) || Number(p.serving_weight_g) || 100;
        let reset = null;
        if (window.CompositeFoodFolderV5377 && typeof window.CompositeFoodFolderV5377.resetEntryFromTemplate === 'function')
            reset = window.CompositeFoodFolderV5377.resetEntryFromTemplate(parent, grams);
        if (!reset && window.CompositeFoodFolderV5377 && typeof window.CompositeFoodFolderV5377.createEntryFromProduct === 'function')
            reset = window.CompositeFoodFolderV5377.createEntryFromProduct(p, grams);
        if (!reset)
            return;
        if (parent.id) {
            reset.id = parent.id;
            if (Array.isArray(reset.children))
                reset.children.forEach(c => { c.parent_composite_id = parent.id; });
        }
        if (parent.ai_import)
            reset.ai_import = parent.ai_import;
        ration[idx] = reset;
        save();
        Events.emit('change');
        Logger.info('RESET_COMPOSITE_ENTRY', { parentRef });
        emitRationChanged('resetCompositeEntry', { parentRef });
    }
    function replaceAll(entries, source) {
      const input = Array.isArray(entries) ? entries : [];
      const previous = ration.slice();
      const previousBatch = { depth:addBatchDepth, changed:addBatchChanged, count:addBatchMutationCount, label:addBatchLabel };
      const skipped = [];
      ration = [];
      addBatchDepth = 1;
      addBatchChanged = false;
      addBatchMutationCount = 0;
      addBatchLabel = String(source || 'replaceAll');
      input.forEach((entry, index) => {
        if (!entry || typeof entry !== 'object') { skipped.push({ index:index, reason:'invalid_entry' }); return; }
        const key = String(entry.key || entry.id || '');
        const grams = Math.max(0, Math.min(5000, Number(entry.grams) || 0));
        if (!key || !(grams > 0)) { skipped.push({ index:index, key:key, reason:'invalid_key_or_grams' }); return; }
        const added = add(key, grams, entry.ai_import || null);
        if (!added) skipped.push({ index:index, key:key, reason:'not_added' });
      });
      const imported = ration.length;
      addBatchDepth = previousBatch.depth;
      addBatchChanged = previousBatch.changed;
      addBatchMutationCount = previousBatch.count;
      addBatchLabel = previousBatch.label;
      if (!imported && input.length) {
        ration = previous;
        Logger.warn('RATION_REPLACE_ALL_REJECTED', { source:String(source || ''), requested:input.length, skipped:skipped });
        return { ok:false, imported:0, skipped:skipped, preserved_previous:true };
      }
      save();
      Events.emit('change');
      emitRationChanged('replaceAll', { source:String(source || 'replaceAll'), imported:imported, skipped:skipped.length });
      Logger.info('RATION_REPLACE_ALL', { source:String(source || ''), imported:imported, skipped:skipped.length });
      return { ok:true, imported:imported, skipped:skipped, preserved_previous:false };
    }
    ...
    function getPreparationMigrationReport() { try {
        return JSON.parse(JSON.stringify(preparationMigrationReport));
    }
    catch (_) {
        return Object.assign({}, preparationMigrationReport);
    } }
    load();
    const api = { add, beginAddBatch, endAddBatch, update, updateCompositeChild, removeCompositeChild, addCompositeChild, replaceCompositeChild, resetCompositeEntry, remove, clear, get, getForCalculation, getForFoodPattern, getForNutrients, replaceAll, setRegion, getRegion, getPreparationMigrationReport, getPersistenceStatus: () => SafeStorage.status ? SafeStorage.status() : { readable: SafeStorage.available(), writable: SafeStorage.available() } };
    window.State = api;
    return api;
})();
const Events = (() => {
    ...
    return {
        on: (e, fn) => { if (!h.has(e))
            h.set(e, []); h.get(e).push(fn); },
        off: (e, fn) => { if (!h.has(e))
            return; h.set(e, (h.get(e) || []).filter(x => x !== fn)); },
        emit: (e, payload) => {
            (h.get(e) || []).slice().forEach(fn => {
                try {
                    ...
                    try {
                        Logger.error('EVENT_LISTENER_ERROR', { event: e, message: error && error.message ? error.message : String(error) });
                    }
                    catch (_) { }
                    try {
                        console.error('Event listener failed:', e, error);
                    }
                    catch (_) { }
                }
            });
...
function proteinDensity(p) {
    if (!p)
        return 0;
    const kcal = kcal100(p);
    const prot = protein100(p);
    return (kcal > 0) ? (prot / kcal) : prot;
}
function fitByKcal() {
    var _a, _b, _c;
    const totals = computeTotalsLocal();
    const Ktarget = ((_c = (_b = (_a = window.norms) === null || _a === void 0 ? void 0 : _a.values) === null || _b === void 0 ? void 0 : _b.kcal) === null || _c === void 0 ? void 0 : _c.value) || 0;
    const Kcurr = totals.t.kcal || 0;
    if (Ktarget <= 0 || Kcurr <= 0)
        return;
...
function fitByProtein() {
    var _a, _b, _c;
    const ration = State.get().map(it => (Object.assign({}, it)));
    const Ptarget = ((_c = (_b = (_a = window.norms) === null || _a === void 0 ? void 0 : _a.values) === null || _b === void 0 ? void 0 : _b.protein_g) === null || _c === void 0 ? void 0 : _c.value) || 0;
    if (Ptarget <= 0)
        return;
    ...
    for (const it of ration) {
        ...
        if (!p)
            continue;
        if (proteinHEI.has(p.hei_category_key))
            vari.push(it);
        else
            fixed.push(it);
    }
    ...
    if (Math.abs(left) < 1.0)
        return;
    const sorted = vari.map(v => (Object.assign(Object.assign({}, v), { p: DB.byKey.get(v.key) }))).filter(x => x.p);
    ...
    while (Math.abs(left) >= 1.0 && iter++ < maxIters) {
        for (const it of sorted) {
            ...
            let gNew = it.grams;
            if (left > 0) {
                gNew = clip(it.grams + step, 1, 5000);
                left -= deltaProtPerStep;
            }
            else {
                gNew = clip(it.grams - step, 1, 5000);
                left += deltaProtPerStep;
...
(function initPresetsUI() {
    ...
    if (seg)
        seg.addEventListener('click', (e) => {
            ...
            fitMode = b.getAttribute('data-fit') || 'kcal';
        });
    btn.addEventListener('click', () => { const id = sel.value; if (!id)
        return; applyPreset(id, fitMode); });
...
(function () {
    ...
    function addNutrients(a, b) { for (const k of Object.keys(emptyNutrients()))
        a[k] = (a[k] || 0) + (b[k] || 0); return a; }
    ...
    function classifyMealProduct(p) {
        ...
        if ((roles.sweet || roles.mixed) && !explicitSugarAddon)
            roles.condiment = false;
        if (roles.sweet)
            roles.fatAddon = false;
    ...
    function isLikelyUndividable(p, g) {
        const text = productText(p);
        if (servingUnitLooksPieceLike(p))
            return true;
        if (/яйц|egg|банан|яблок|мандарин|апельсин|киви|груш|сырок|булоч|круассан|хлебец|сосиск|сардельк|печень|батончик/.test(text))
            return true;
        if ((Number(g) || 0) < 60)
            return true;
        return false;
    ...
    function canAutoSplit(p, g) {
        ...
        if (!p || (Number(g) || 0) <= 0)
            return false;
        if (c.roles.condiment)
            return true;
        if (isLikelyUndividable(p, g))
            return false;
        if (c.roles.sweet)
            return false;
        if (c.roles.dairy && /творог|йогурт|кефир|yogurt|cottage/.test(productText(p)))
            return true;
        if (c.roles.vegetable || c.roles.wholeGrain || c.roles.refinedGrain)
            return true;
        if (c.roles.mixed && (Number(g) || 0) >= 350)
            return true;
        return (Number(g) || 0) >= 280 && !c.roles.drink;
    }
    function canManualSplit(p, g) { if (!p || (Number(g) || 0) < 20)
        return false; return canAutoSplit(p, g); }
    function deriveMealRole(p) {
        ...
        if (c.roles.condiment) {
            id = 'addon';
            label = 'добавка к блюду';
            addon = true;
        }
        else if (c.roles.mixed) {
            id = 'mixed_dish';
            label = 'сложное блюдо';
            anchor = true;
        }
        else if (c.roles.sweet) {
            id = 'sweet';
            label = 'сладкий продукт';
        }
        else if (c.roles.drink) {
            id = 'drink';
            label = 'напиток';
        }
        else if (c.roles.fruit) {
            id = 'fruit';
            label = 'фруктовая часть';
        }
        else if (c.roles.vegetable) {
            id = 'vegetable_side';
            label = 'овощная часть';
        }
        else if (c.roles.fatAddon) {
            id = 'fat_addon';
            label = 'полезный жир / добавка';
            addon = true;
        }
        else if (c.roles.dairy && /творог|йогурт|кефир|молок|cottage|yogurt|milk/.test(text)) {
            id = 'dairy_snack';
            label = 'молочная основа';
            anchor = true;
        }
        else if (c.roles.protein) {
            id = 'anchor_protein';
            label = 'белковая основа';
            anchor = true;
        }
        else if (c.roles.wholeGrain || c.roles.refinedGrain) {
            id = 'anchor_carb';
            label = c.roles.wholeGrain ? 'углеводная основа' : 'злаковая основа';
            anchor = true;
        }
    ...
    function getTargets(total, scheme) {
        ...
        if (!targetKcal || targetKcal < 1)
            targetKcal = total.kcal || 0;
        let targetProtein = hasNeeds && needsTargets.protein > 0 ? needsTargets.protein : total.protein_g;
        if (!targetProtein || targetProtein < 1)
            targetProtein = total.protein_g || 0;
        const perMeal = {};
    ...
    function addAllocation(model, mealId, allocation) {
        ...
        if (mealId === 'unassigned')
            model.unassigned.push(a);
        else {
            const meal = mealById(model, mealId) || model.meals[0];
            if (meal)
                meal.allocations.push(a);
            else
                model.unassigned.push(a);
        }
        return a;
    ...
    function mealHasRole(model, mealId, role) {
        const meal = mealById(model, mealId);
        if (!meal)
            return false;
        return (meal.allocations || []).some(a => {
            const p = byKey(a.key);
            const c = classifyMealProduct(p);
            const r = deriveMealRole(p);
    ...
    function baseAffinityForMeal(p, mealId) {
        ...
        if (mealId === 'breakfast') {
            if (c.roles.dairy)
                s += 1.8;
            if (c.roles.wholeGrain || /овсян|каша|хлоп|тост|хлеб/.test(text))
                s += 2.0;
            if (/яйц|egg/.test(text))
                s += 2.5;
            if (c.roles.fruit)
                s += 1.1;
            if (c.roles.drink)
                s += 0.7;
            if (c.roles.vegetable)
                s += 0.6;
            if (c.roles.processedMeat)
                s += 0.4;
        }
        if (mealId === 'second_breakfast' || mealId === 'snack' || mealId === 'afternoon') {
            ...
            if (c.roles.sweet)
                s += 1.7;
            if (c.roles.fatAddon && !c.roles.condiment)
                s += 1.6;
            if (c.roles.drink)
                s += 0.8;
            if (c.roles.protein && !c.roles.mixed && !/мяс|куриц|говяд|рыб|fish|chicken|meat/.test(text))
                s += 0.7;
            if (c.roles.vegetable)
                s += 0.3;
        }
        if (mealId === 'lunch') {
            ...
            if (c.roles.vegetable)
                s += 2.2;
            if (c.roles.wholeGrain || c.roles.refinedGrain)
                s += 1.8;
            if (c.roles.mixed)
                s += 1.7;
            if (c.roles.condiment || c.roles.fatAddon)
                s += 1.0;
            if (c.roles.processedMeat)
                s += 0.5;
        }
        ...
        if (mealId === 'late_snack') {
            ...
            if (c.roles.sweet)
                s -= 0.1;
            if (c.roles.mixed || (c.roles.protein && /мяс|куриц|говяд|рыб|fish|chicken|meat/.test(text)))
                s -= 1.2;
        }
        if (c.roles.sweet && !isSnackMealId(mealId))
            s -= 0.25;
        if ((c.roles.condiment || c.roles.fatAddon) && mealId === 'late_snack')
            s -= 0.9;
        if (c.roles.mixed && mealId === 'late_snack')
            s -= 1.5;
        return Math.max(0.01, s);
    }
    function scoreProductForMeal(model, scheme, item, mealId) {
        const p = byKey(item.key);
        if (!p)
            return -Infinity;
        const c = classifyMealProduct(p);
        const r = deriveMealRole(p);
        const n = nutrientOf(item.key, item.grams || item.g || 0);
        ...
        if (targetK > 0) {
            ...
            if (ratioAfter <= 1.05)
                score += 0.65 * (1.05 - ratioAfter);
            if (ratioAfter > 1.18)
                score -= Math.min(1.4, (ratioAfter - 1.18) * 1.8);
            if (current.kcal < targetK * 0.55 && n.kcal > 80)
                score += 0.25;
        }
        if (r.id === 'anchor_protein') {
            if (!mealHasRole(model, mealId, 'protein'))
                score += 0.7;
            if (target.protein && current.protein_g > target.protein * 1.15)
                score -= 0.6;
        }
        if (r.id === 'anchor_carb') {
            if (mealHasRole(model, mealId, 'protein'))
                score += 0.35;
            if (mealHasRole(model, mealId, 'vegetable'))
                score += 0.25;
        }
        if (r.id === 'vegetable_side') {
            if (mealHasRole(model, mealId, 'protein') || mealHasRole(model, mealId, 'whole_grain') || mealHasRole(model, mealId, 'refined_grain') || mealHasRole(model, mealId, 'mixed_dish'))
                score += 0.75;
            if (mealId === 'lunch' || mealId === 'dinner')
                score += mealRoleCount(model, mealId, 'vegetable') ? 0.1 : 0.45;
        }
        if (r.id === 'fruit') {
            if (isSnackMealId(mealId) && !mealHasRole(model, mealId, 'fruit'))
                score += 0.55;
            if (mealHasRole(model, mealId, 'dairy'))
                score += 0.3;
        }
        if (r.id === 'dairy_snack') {
            if (isSnackMealId(mealId) || mealId === 'breakfast')
                score += 0.35;
            if (mealHasRole(model, mealId, 'fruit'))
                score += 0.25;
        }
        if (r.id === 'sweet') {
            if (isSnackMealId(mealId))
                score += 1.15;
            else
                score -= 0.85;
            if (mealHasRole(model, mealId, 'dairy') || mealHasRole(model, mealId, 'fruit'))
                score += 0.35;
            if (current.added_sugars_g > 10)
                score -= 0.55;
        }
        if (r.addon) {
            if (mealHasRole(model, mealId, 'vegetable') || mealHasRole(model, mealId, 'protein') || mealHasRole(model, mealId, 'whole_grain') || mealHasRole(model, mealId, 'refined_grain') || mealHasRole(model, mealId, 'mixed_dish'))
                score += 1.0;
            if (!(mealHasRole(model, mealId, 'vegetable') || mealHasRole(model, mealId, 'protein') || mealHasRole(model, mealId, 'whole_grain') || mealHasRole(model, mealId, 'refined_grain') || mealHasRole(model, mealId, 'mixed_dish')))
                score -= 0.9;
        }
        if (mealId === 'late_snack' && n.kcal > 180)
            score -= 1.0;
        return score;
    }
    ...
    function allocationReason(p, mealId) {
        const c = classifyMealProduct(p);
        const r = deriveMealRole(p);
        const where = mealId && mealId !== 'unassigned' ? ' в этот приём' : '';
        if (r.id === 'addon')
            return 'добавка: прикреплена к блюду или приёму, где есть основа; сама по себе не образует приём пищи';
        if (r.id === 'mixed_dish')
            return 'сложное блюдо: размещено как самостоятельная основа, состав оценивается приблизительно';
        if (r.id === 'sweet')
            return 'сладкий продукт: чаще попадает в перекус и требует проверки добавленного сахара';
        if (r.id === 'fruit')
            return 'фруктовая часть: чаще подходит к завтраку или перекусу';
        if (r.id === 'vegetable_side')
            return 'овощная часть: алгоритм старается присоединить её к обеду или ужину';
        if (r.id === 'dairy_snack')
            return 'молочная основа: подходит к завтраку, перекусу или позднему перекусу';
        if (r.id === 'anchor_protein')
            return 'белковая основа: алгоритм распределяет такие продукты между основными приёмами';
        if (r.id === 'anchor_carb')
            return 'углеводная основа: размещена с учётом калорийной квоты и возможного сочетания с белком/овощами';
        if (r.id === 'fat_addon')
            return 'жировая добавка: лучше сочетать с салатом, гарниром или основным блюдом';
        if (c.roles.drink)
            return 'напиток: размещён отдельно от основной еды, проверьте фактический режим';
    ...
    function placeSmartItem(model, scheme, item, opts) {
        ...
        if (allowSplit && canAutoSplit(p, g) && g >= 240 && n.kcal >= 220 && r.id !== 'mixed_dish') {
            ...
            if (secondG > 0)
                addAllocation(model, top[1] || top[0] || chooseBestMeal(model, scheme, item), { key: item.key, g: secondG, productKind: classifyMealProduct(p).mainRole, mealRole: r.id, split: true, reason: allocationReason(p, top[1] || top[0]), roleLabel: r.label });
        }
        else {
    ...
    function collectManualAllocations(previousModel, ration) {
        if (!previousModel || !Array.isArray(previousModel.meals))
            return null;
        const rationMap = new Map(ration.map(x => [x.key, x.grams]));
        const grouped = new Map();
        function take(list, mealId) {
            (list || []).forEach(a => {
                if (!a || !a.key || !a.manual || !rationMap.has(a.key))
                    return;
                if (!grouped.has(a.key))
                    grouped.set(a.key, []);
        ...
        grouped.forEach((list, key) => {
            ...
            if (scaled.length)
                out.push({ key, allocations: scaled });
        });
        return out;
    }
    function placeCondiment(model, scheme, item) {
        ...
        if ((/сахар|мед|мёд|варень/.test(text) || /(^|[_\s-])sugar([_\s-]|$)/.test(text))) {
            ...
            if (!candidates.length)
                candidates = scheme.meals.filter(m => ['snack', 'afternoon', 'second_breakfast', 'breakfast'].includes(m.id)).map(m => m.id);
        }
            ...
            if (!candidates.length)
                candidates = scheme.meals.filter(m => ['lunch', 'dinner'].includes(m.id)).map(m => m.id);
        }
    ...
    function mealsWithRole(model, role) {
        const out = [];
        (model.meals || []).forEach(m => { if (mealHasRole(model, m.id, role))
            out.push(m.id); });
        return out;
    }
    function normalizeAllocations(model) {
        function normalizeList(list) { const out = []; (list || []).forEach(a => { a.g = Math.max(0, roundStep(a.g, 1)); if (a.g > 0)
    ...
    function analyzeModel(model) {
        ...
        model.meals.forEach(meal => {
            ...
            if (mealHasItems && targetK > 0 && ratio < 0.70)
                warn.push('Калорийность заметно ниже ориентира для этой схемы дня.');
            if (mealHasItems && targetK > 0 && ratio > 1.30)
                warn.push('Калорийность заметно выше ориентира для этой схемы дня.');
            if (meal.main && mealHasItems) {
                ...
                if (t.protein_g < proteinThreshold)
                    warn.push('Белка маловато для основного приёма пищи.');
            }
            if (lunchDinner.has(meal.id) && mealHasItems && total.veg_cup_eq > 0.15 && !hasVeg)
                warn.push('Овощей нет или почти нет в этом основном приёме.');
            if (t.added_sugars_g >= 15 || (t.kcal > 0 && t.added_sugars_g * 4 / t.kcal > 0.20))
                warn.push('Высокая доля добавленного сахара в этом приёме.');
            if (isSnackMealId(meal.id) && mealHasItems && sweetItems.length && (!hasProtein && !hasFruit) && t.kcal > 120)
                warn.push('Перекус почти полностью сладкий: лучше соединить его с молочным, белковым или фруктовым продуктом, если это соответствует реальному приёму пищи.');
            if (daySodium > 0 && t.sodium_mg > Math.max(800, daySodium * 0.40))
                warn.push('Этот приём даёт много натрия. Проверьте сыр, колбасу, соусы или готовые продукты.');
            if (daySfa > 0 && t.sfa_g > Math.max(8, daySfa * 0.40))
                warn.push('Высокая доля насыщенных жиров в этом приёме.');
            if (meal.late && t.kcal > Math.max(180, targetK * 1.3))
                warn.push('Поздний перекус получился тяжёлым. Проверьте, не лучше ли перенести часть продуктов в ужин или полдник.');
            if ((meal.allocations || []).some(a => classifyMealProduct(byKey(a.key)).roles.mixed))
                info.push('Есть сложное блюдо: оценка состава приёма приблизительная.');
            if ((meal.allocations || []).some(a => a.noAutoSplit))
                info.push('Штучные или порционные продукты не дробились автоматически.');
            if (mealHasItems && (hasProtein || hasVeg || hasFruit))
                info.push(['структура:', hasProtein ? 'есть белковая часть' : null, hasVeg ? 'есть овощная часть' : null, hasFruit ? 'есть фруктовая часть' : null].filter(Boolean).join(' '));
            if (!mealHasItems)
                info.push('Приём пока пустой. При необходимости переместите сюда продукты вручную.');
            meal.totals = t;
            meal.targetKcal = targetK;
            meal.kcalRatio = ratio;
            meal.warnings = warn;
            meal.info = info;
            meal.status = warn.length >= 2 ? 'bad' : warn.length ? 'warn' : 'ok';
        });
        ...
        else if (dayProtein > 0)
            summary.push('Белок распределён без выраженной концентрации в одном приёме.');
        ...
        if (total.veg_cup_eq > 0.15) {
            ...
            if (vegMainCount < 2)
                suggestions.push('Овощная часть распределена неравномерно. Можно вручную перенести часть овощей к тому основному приёму, где их нет.');
        }
        ...
        if (dayAddedSugar > 0 && sweetMeals.length) {
            ...
            if (sweetMeals.some(m => isSnackMealId(m.id)))
                suggestions.push('Сладкий перекус лучше проверить вручную: при наличии йогурта, творога или фрукта его можно соединить с ними вместо отдельного сладкого блока.');
        }
        ...
        if (condimentUnassigned)
            suggestions.push('Часть добавок не привязалась к блюдам. Их лучше вручную прикрепить к салату, гарниру или основному продукту.');
        if (!summary.length)
            summary.push('Выраженных перекосов по распределению дня не найдено.');
        if (!suggestions.length)
            suggestions.push('Грубых действий для исправления не найдено. Проверьте вручную, совпадает ли черновик с реальными блюдами и режимом дня.');
    ...
    function findAllocation(model, id) {
        for (const meal of model.meals || []) {
            const idx = (meal.allocations || []).findIndex(a => a.id === id);
            if (idx >= 0)
                return { container: meal, list: meal.allocations, idx, allocation: meal.allocations[idx], mealId: meal.id };
        }
        const ui = (model.unassigned || []).findIndex(a => a.id === id);
        if (ui >= 0)
            return { container: null, list: model.unassigned, idx: ui, allocation: model.unassigned[ui], mealId: 'unassigned' };
        return null;
    }
    function moveAllocation(id, targetMealId) {
        if (!currentModel)
            return;
        const found = findAllocation(currentModel, id);
        if (!found)
            return;
        const a = found.list.splice(found.idx, 1)[0];
        a.manual = true;
        a.origin = 'manual';
        a.reason = 'перемещено вручную';
    ...
    function splitAllocation(id, gramsToMove, targetMealId) {
        ...
        if (!found)
            return;
        const a = found.allocation;
        const p = byKey(a.key);
        if (!canManualSplit(p, a.g))
            return;
        let g = roundStep(Number(gramsToMove) || 0, 5);
        if (g <= 0 || g >= a.g)
            return;
        a.g = roundStep(a.g - g, 5);
        a.manual = true;
        a.origin = 'manual';
        ...
        normalizeAllocations(currentModel);
        analyzeModel(currentModel);
        saveModel(currentModel);
        renderMeals(currentModel);
    }
    function saveModel(model) { try {
        SafeStorage.setItem(STORAGE_KEY, JSON.stringify(model));
    }
    catch (_) { } }
    function loadModel() {
        try {
            ...
            if (!model || model.version !== VERSION || !Array.isArray(model.meals))
                return null;
            return model;
        }
        catch (_) {
    ...
    function resetModel() { currentModel = null; try {
        SafeStorage.removeItem(STORAGE_KEY);
    }
    catch (_) { } renderMeals(null); updateControls(); }
    function updateControls() {
        const has = currentRation().length > 0;
        ['distributeBtn', 'mealRefreshBtn', 'mealResetBtn'].forEach(id => { const el = document.getElementById(id); if (el)
            el.disabled = id === 'mealResetBtn' ? !currentModel : !has; });
        const badge = document.getElementById('mealsStaleBadge');
        if (badge)
            badge.style.display = currentModel && currentModel.rationHash !== rationHash() ? 'inline-flex' : 'none';
    }
    function scheduleAutoRefresh() {
        const auto = SafeStorage.getItem(AUTO_KEY) === '1';
        if (!auto || !currentModel)
            return;
        clearTimeout(recalcTimer);
        recalcTimer = setTimeout(() => { currentModel = generateMealDraft(currentModel.schemeId || currentSchemeId(), currentModel, true); saveModel(currentModel); renderMeals(currentModel); }, 350);
    }
    ...
    function mealStatusLabel(status) { if (status === 'ok')
        return 'близко к цели'; if (status === 'bad')
        return 'выраженный перекос'; return 'нужно проверить'; }
    ...
    function renderAllocation(a, currentMealId) {
        const p = byKey(a.key);
        const name = productName(p, a.key);
        const n = nutrientOf(a.key, a.g);
        const c = classifyMealProduct(p);
        const canSplit = canManualSplit(p, a.g);
        const r = deriveMealRole(p);
      ...
      <details class="meal-split-box">
        ...
        if (c.roles.mixed)
            badges.push('сложное блюдо');
        if (c.roles.sweet)
            badges.push('сахар');
        if (a.manual)
            badges.push('вручную');
        if (a.split)
            badges.push('разделено');
        return `
    ...
    function renderMeals(model) {
        const root = document.getElementById('mealsOut');
        if (!root)
            return;
        updateControls();
        const ration = currentRation();
        if (!ration.length) {
            root.innerHTML = '<div class="meal-empty-state">Добавьте продукты в итоговый рацион, чтобы сделать черновик распределения по приёмам пищи.</div>';
            return;
        }
        if (!model) {
            root.innerHTML = '<div class="meal-empty-state">Рацион уже собран. Нажмите «Сделать черновик», чтобы примерно разложить его по дню.</div>';
            return;
        }
        analyzeModel(model);
    ...
    function initMeals() {
        ...
        if (!section || !sel || !btn || !out)
            return;
        const savedScheme = SafeStorage.getItem(SCHEME_KEY);
        sel.value = MEAL_SCHEMES[savedScheme] ? savedScheme : '4';
        if (autoTgl)
            autoTgl.checked = SafeStorage.getItem(AUTO_KEY) === '1';
        currentModel = loadModel();
        if (currentModel) {
            if (!MEAL_SCHEMES[currentModel.schemeId])
                currentModel.schemeId = '4';
            sel.value = currentModel.schemeId;
            analyzeModel(currentModel);
        }
        renderMeals(currentModel);
        ...
        if (refreshBtn)
            refreshBtn.addEventListener('click', function () { const schemeId = currentSchemeId(); currentModel = generateMealDraft(schemeId, currentModel, true); saveModel(currentModel); renderMeals(currentModel); });
        if (resetBtn)
            resetBtn.addEventListener('click', resetModel);
        sel.addEventListener('change', function () { SafeStorage.setItem(SCHEME_KEY, currentSchemeId()); if (currentModel) {
            currentModel = generateMealDraft(currentSchemeId(), currentModel, true);
            saveModel(currentModel);
            renderMeals(currentModel);
        } });
        if (autoTgl)
            autoTgl.addEventListener('change', function () { SafeStorage.setItem(AUTO_KEY, autoTgl.checked ? '1' : '0'); if (autoTgl.checked)
                scheduleAutoRefresh(); });
        out.addEventListener('click', function (e) {
            const b = e.target.closest('[data-meal-action]');
            if (!b)
                return;
            const action = b.getAttribute('data-meal-action');
            const id = b.getAttribute('data-allocation-id');
            if (action === 'move') {
                const target = out.querySelector(`[data-move-target-for="${cssEsc(id)}"]`);
                moveAllocation(id, target ? target.value : 'unassigned');
            }
            if (action === 'split') {
                const gramsEl = out.querySelector(`[data-split-grams-for="${cssEsc(id)}"]`);
                const targetEl = out.querySelector(`[data-split-target-for="${cssEsc(id)}"]`);
                splitAllocation(id, gramsEl ? gramsEl.value : 0, targetEl ? targetEl.value : 'unassigned');
            }
            if (action === 'update-preserve') {
                currentModel = generateMealDraft(currentSchemeId(), currentModel, true);
                saveModel(currentModel);
                renderMeals(currentModel);
            }
            if (action === 'update-new') {
                currentModel = generateMealDraft(currentSchemeId(), null, false);
                saveModel(currentModel);
                renderMeals(currentModel);
            }
            if (action === 'keep-stale') {
                const box = out.querySelector('.meal-stale-box');
                if (box)
                    box.style.display = 'none';
            }
        });
        Events.on('change', function () { updateControls(); if (SafeStorage.getItem(AUTO_KEY) === '1')
            scheduleAutoRefresh();
        else
            renderMeals(currentModel); });
    }
...
const MicroIndex = (() => {
    ...
    for (const o of DB.items) {
        for (const [k, v] of Object.entries(o)) {
            ...
            if (kk.endsWith('_mg') || kk.endsWith('_mcg') || kk.startsWith('vitamin_'))
                set.add(kk);
        }
    }
    ...
    all.sort((a, b) => {
        ...
        if (ga !== gb)
            return (ga === 'vitamins' ? 0 : ga === 'minerals' ? 1 : 2) - (gb === 'vitamins' ? 0 : gb === 'minerals' ? 1 : 2);
...
const Calc = (() => {
    function isPer100FieldName(name) {
        if (name.endsWith(SCALE_SUFFIX) || KNOWN_PER100.has(name))
            return true;
    ...
    function scaledPerItem(item) {
        ...
        for (const [k, v] of Object.entries(p)) {
            if (typeof v !== 'number')
                continue;
            if (!isPer100FieldName(k))
                continue;
            let baseName = k;
            if (k.endsWith(SCALE_SUFFIX))
                baseName = k.slice(0, -SCALE_SUFFIX.length).replace('_per100g', '');
            baseName = canonicalizeKey(baseName);
            baseName = normalizeMacroName(baseName);
            const val = v * f;
            out[baseName] = (out[baseName] || 0) + val;
        }
        if (out.sodium_mg == null) {
            if (out.salt_g != null)
                out.sodium_mg = out.salt_g * 393.0;
        }
        return out;
    }
...
function productSourceClassV53155(p) {
    const engine = window.ProductDataQualityV13;
    if (engine && typeof engine.classifyProduct === 'function') {
        const tier = String((engine.classifyProduct(p) || {}).tier || '').toLowerCase();
        return tier === 'high' || tier === 'medium' || tier === 'low' ? tier : 'unknown';
    }
    return 'unknown';
}
function sourceQualityV53155(ration) {
    const clean = rationForNutrientCalculationV53107(ration || []);
    const engine = window.ProductDataQualityV13;
    if (engine && typeof engine.productSummary === 'function')
        return engine.productSummary(clean, DB);
    const out = { total: 0, high: 0, medium: 0, low: 0, unknown: 0, usable: 0, pct: 0, score: 0, tier: 'LOW', label: 'нет продуктов' };
    for (const it of clean) {
        const p = DB.byKey.get(it && it.key);
        if (!p)
            continue;
        out.total += 1;
        const cls = productSourceClassV53155(p);
        out[cls] += 1;
    }
    out.usable = out.high + out.medium;
    out.pct = out.total ? out.usable / out.total * 100 : 0;
    out.label = !out.total ? 'нет продуктов' : (out.low ? 'есть источники, требующие проверки' : (out.medium ? 'часть данных модельные или proxy' : 'высокая достоверность'));
    return out;
}
...
const CalculationCoreV53155 = Object.freeze({
    ...
    snapshot: function (ration) {
        ...
        return Object.freeze({
            ...
            dataQuality: (window.ProductDataQualityV13 && typeof window.ProductDataQualityV13.analyzeRation === 'function') ? window.ProductDataQualityV13.analyzeRation(rationForNutrientCalculationV53107(raw), DB) : null
        });
    }
});
...
const UI = (() => {
          ...
          <div class="accordion-body">
            ...
            function fillOptions(filter = '') {
                ...
                if (!list.length) {
                    const opt = document.createElement('option');
                    opt.value = '';
                    opt.textContent = 'В этой группе ничего не найдено';
                    sel.appendChild(opt);
                    return;
                }
                for (const p of list) {
                    const opt = document.createElement('option');
                    opt.value = p.key;
                    opt.textContent = p.name_ru || p.name || p.key;
                    sel.appendChild(opt);
                }
            }
            fillOptions();
    ...
    function renderRation() {
        ...
        if (clearRationBtn)
            clearRationBtn.disabled = (ration.length === 0);
        ...
        function confidenceLabel(entry) {
            ...
            if (c === 'HIGH')
                return 'точность высокая';
            if (c === 'MEDIUM')
                return 'точность средняя';
            if (c === 'MEDIUM_LOW')
                return 'точность средне-низкая';
            if (c === 'LOW')
        ...
        function currentRationTotalsForInsights() {
            try {
                return (Calc && Calc.totals) ? (Calc.totals(State.get()).t || {}) : {};
            }
            catch (_) {
                return {};
            }
        ...
        function entryNutrientsForInsights(entry) {
            try {
                ...
                if (hasLimiter || !isCompositeEntryV5377(entry))
                    return direct;
                const sum = {};
        ...
        function limiterInsightsHtml(nutrients, totals, maxItems) {
            ...
            if (rows.length > visible.length)
                chips.push(`<span class="ration-impact-more">+${rows.length - visible.length}</span>`);
            return chips.join('');
        }
        function compositeImpactSummary(entry, totals) {
            ...
            if (!html)
                return '<div class="composite-impact-summary is-quiet"><strong>По текущей порции:</strong> выраженного вклада в показатели умеренности не выявлено.</div>';
        ...
        function renderCompositeDetails(host, entry) {
            ...
            const rows = children.map(child => {
                ...
                const flags = [];
                if (child.custom_added)
                    flags.push('добавлено');
                if (child.replaces_key)
                    flags.push('замена');
                if (child.manual_locked)
                    flags.push('ручная масса');
      ...
      <div class="mini-nutrients" id="${miniId}"></div>
            ...
            function productSearchForComposite(filter) {
                ...
                return DB.items.filter(p => p && p.hidden_from_search !== true).map(p => {
                    ...
                    if (terms.length && !terms.every(t => searchTermMatches(text, t)))
                        return null;
                    let score = 0;
                    ...
                    for (const t of terms) {
                        ...
                        if (name.includes(t))
                            score += 50;
                        if (searchTermMatches(productSearchMetaText(p), t))
                            score += 3;
                    }
                    return { p, score };
            ...
            function openInnerEditor(mode, childRef, grams) {
                ...
                if (title)
                    title.textContent = editorMode === 'replace' ? 'Заменить ингредиент внутри блюда' : 'Добавить ингредиент внутрь блюда';
                if (help)
                    help.textContent = editorMode === 'replace' ? 'Выбранный продукт заменит текущий ингредиент и останется внутри этой же папки.' : 'Продукт будет добавлен внутрь этой папки, а не в общий список дня.';
                if (confirm)
                    confirm.textContent = editorMode === 'replace' ? 'Заменить' : 'Добавить';
                if (gramsInput)
                    gramsInput.value = Math.max(0, Number(grams) || 50);
                if (search)
                    search.value = '';
                fillInnerOptions('');
                editor.hidden = false;
            ...
            if (addBtn)
                addBtn.addEventListener('click', () => openInnerEditor('add', '', 50));
            const resetBtn = host.querySelector('.composite-reset-recipe');
            if (resetBtn)
                resetBtn.addEventListener('click', () => {
                    if (confirm('Вернуть типовую рецептуру блюда и удалить пользовательские изменения?'))
                        State.resetCompositeEntry(rootRef);
                });
            if (editor) {
                const search = editor.querySelector('input[data-role="search"]');
                const gramsInput = editor.querySelector('input[data-role="grams"]');
                ...
                let st = null;
                if (search)
                    search.addEventListener('input', () => { clearTimeout(st); st = setTimeout(() => fillInnerOptions(search.value), 120); });
                if (confirmBtn)
                    confirmBtn.addEventListener('click', () => {
                        ...
                        if (editorMode === 'replace')
                            State.replaceCompositeChild(rootRef, editorChildRef, key, g);
                        else
                            State.addCompositeChild(rootRef, key, g);
                    });
                if (cancelBtn)
    ...
    `;
            ...
            nameCell.addEventListener('click', (ev) => {
                if (ev.target && ev.target.closest && ev.target.closest('input'))
                    return;
                ...
                if (!isOpen) {
                    if (isComposite)
                        renderCompositeDetails(document.getElementById(detailsId), it);
                    else
                        renderMiniNutrients(it.key, parseFloat(gramsInput.value) || 0);
                }
            });
    ...
    function titleOfKey(key) {
        if (norms && norms.values && norms.values[key] && norms.values[key].title)
            return norms.values[key].title;
    ...
    function unitOfKey(key) {
        if (norms && norms.values && norms.values[key] && norms.values[key].unit)
            return norms.values[key].unit;
    ...
    function renderMiniNutrients(key, grams) {
        ...
        if (!host || !Calc || !Calc.scaledPerItem)
            return;
    ...
    function percentile(val, norm) { if (!norm || norm.value <= 0)
        return 0; return (val / norm.value) * 100; }
    function fmt(val, unit) {
        ...
        if (unit === 'г')
            return (Math.round(val * 10) / 10).toString() + ' г';
        if (unit === 'ккал')
            return Math.round(val).toString() + ' ккал';
    ...
    function buildNormCaption(key, norm) {
        ...
        if (metric.mode === 'informational')
            return 'информационный показатель; целевой минимум не задан';
        if (!metric.target)
            return 'ориентир не задан';
        if (metric.mode === 'upper_limit')
            return 'верхний предел: ' + fmt(metric.target, metric.unit);
    ...
    function microGetLimits(key) {
        ...
        return {
            ...
            risk: lim.risk || null,
            provenance: lifeActive ? (lifeRef.provenance || null) : (resolved.provenance || (resolved.min && resolved.min.provenance) || null)
        };
    }
    function capMin(minObj) {
        if (!minObj) return '—';
        if (Number.isFinite(Number(minObj.value))) return `Мин. рекомендация (${minObj.type}): ${minObj.value}`;
        if (minObj.formula_label) return `Ориентир (${minObj.type || 'формула'}): ${minObj.formula_label}; требуется энергетическая цель`;
        return '—';
    }
    function capUL(ulObj, safeObj) {
        if (!ulObj)
            return safeObj ? ('UL не установлен; ' + (safeObj.label || 'безопасный уровень') + ': ' + safeObj.value) : 'UL не установлен';
        const scopeMap = {
    ...
    function microCard(key, val, meta) {
        ...
        const minVal = minObj && Number.isFinite(Number(minObj.value)) ? Number(minObj.value) : null;
        ...
        capRight.textContent = (minObj && Number.isFinite(Number(minObj.value)) ? `${minObj.type}: ${minObj.value} ${unit}` : (minObj && minObj.formula_label ? `${minObj.type || 'формула'}: ${minObj.formula_label}` : 'ориентир не задан')) + ' • ' + (ulObj ? `UL: ${ulObj.value} ${unit}` : (safeObj ? `UL не установлен · ${safeObj.label || 'безопасный уровень'}: ${safeObj.value} ${unit}` : 'UL не установлен'));
        head.appendChild(titleEl);
        head.appendChild(capRight);
    ...
    function renderTotals() {
        ...
        if (!totalsGrid) {
            try {
                console.warn('renderTotals: #totalsGrid not found');
            }
            catch (_) { }
            return;
        }
        ...
        var totalsSignature = '';
        try {
            var totalPairs = Object.keys(t || {}).sort().map(function (k) {
                var v = Number(t[k]);
                return [k, Number.isFinite(v) ? Number(v.toFixed(4)) : null];
            });
            var normPairs = Object.keys((norms && norms.values) || {}).sort().map(function (k) {
                var entry = norms.values[k] || {};
                return [k, Number(entry.value) || 0, entry.unit || '', entry.type || ''];
            });
            totalsSignature = JSON.stringify({ totals: totalPairs, norms: normPairs, region: (State.getRegion ? State.getRegion() : ''), count: ration.length });
        }
        catch (_) { totalsSignature = ''; }
        if (totalsSignature && totalsSignature === window.__lastTotalsRenderSignature && totalsGrid.childElementCount) {
            window.__TOTALS_RENDER_STATS__ = window.__TOTALS_RENDER_STATS__ || { domWrites: 0, skips: 0 };
            window.__TOTALS_RENDER_STATS__.skips += 1;
            return;
        }
        window.__lastTotalsRenderSignature = totalsSignature;
        window.__TOTALS_RENDER_STATS__ = window.__TOTALS_RENDER_STATS__ || { domWrites: 0, skips: 0 };
        window.__TOTALS_RENDER_STATS__.domWrites += 1;
        ...
        for (const [key, _title, unit] of KEY_CARDS) {
            ...
            if (EDITABLE.has(key)) {
                ...
                wrap.className = 'norm-input-wrap';
                const inp = document.createElement('input');
                inp.type = 'number';
                inp.className = 'norm-input';
                inp.min = '0';
                inp.step = '1';
                inp.id = 'normInput-' + key;
                inp.value = (target || 0);
                const u = document.createElement('span');
                u.className = 'unit-tag';
                u.textContent = unit;
                ...
                const applyVal = (v) => {
                    ...
                    if (!isFinite(n) || n < 0)
                        return;
                    if (!norms.values[key])
                        norms.values[key] = { value: 0, unit: unit };
            ...
            bar.style.width = (isInformational ? 0 : pct) + '%';
            prog.appendChild(bar);
            const valDiv = document.createElement('div');
            valDiv.innerHTML = `<strong>${fmt(val, unit)}</strong>`;
            progRow.appendChild(prog);
            progRow.appendChild(valDiv);
            const foot = document.createElement('div');
            foot.className = 'muted';
            foot.style.cssText = 'font-size:12px;margin-top:4px;';
    ...
    // Product Correction PC1: generic one-word queries prefer canonical base foods.
    function genericSearchPriorityBonus(product, query) {
        const q = normalizeSearchText(query || '');
        const key = String(product && product.key || '');
        const name = normalizeSearchText(product && (product.name_ru || product.name) || '');
        const record = normalizeSearchText([product && product.record_type, product && product.source_exactness_class, Array.isArray(product && product.tags) ? product.tags.join(' ') : ''].filter(Boolean).join(' '));
        const priorities = {
            'молоко': ['milk_1_5_pct', 'milk_3_2_pct', 'milk_0_5_pct', 'milk_lactose_free_1_8_pct'],
            'йогурт': ['yogurt_plain_1_5_pct', 'yogurt_plain_3_2_pct', 'yogurt_greek_plain', 'soy_yogurt_plain_spp'],
            'творог': ['tvorog_5_pct', 'curd_5_pct', 'tvorog_9_pct', 'curd_9_pct', 'tvorog_lowfat_0_1_8_pct'],
            'гречка': ['buckwheat_cooked', 'buckwheat', 'green_buckwheat_cooked'],
            'курица': ['chicken_breast_boiled_skinless', 'chicken_breast_raw_skinless', 'chicken_breast_grilled_skinless', 'chicken_thigh_baked_with_skin', 'chicken_drumstick_baked_with_skin'],
            'масло': ['butter_82_5_pct', 'olive_oil', 'sunflower_oil'],
            'банан': ['banana']
        };
        let score = 0;
        const list = priorities[q] || [];
        const index = list.indexOf(key);
        if (index >= 0) score += 900 - index * 45;
        if (q && q.indexOf(' ') < 0) {
            if (/branded|brand_specific/.test(record)) score -= 260;
            else score += 35;
            if (/ready_meal|homemade_recipe|recipe_model|composite_food/.test(record)) score -= 180;
        }
        if (q === 'молоко' && /^молоко\s+\d/.test(name)) score += 100;
        if (q === 'йогурт' && /натуральн|без сахара/.test(name)) score += 100;
        if (q === 'творог' && /^творог\s+\d|^творог обезжир/.test(name)) score += 100;
        if (q === 'гречка' && /варен|сухая/.test(name) && !/гриб|лапш/.test(name)) score += 120;
        if (q === 'курица' && /грудка|бедро|голень/.test(name) && !/салат|плов|ветчин/.test(name)) score += 120;
        return score;
    }
    // Global search
    function renderGlobalResults(query) {
        ...
        if (!DB || !Array.isArray(DB.items) || DB.items.length < 100) {
            if (globalSearchStatus)
                globalSearchStatus.textContent = 'База продуктов не загружена. Обновите страницу после полной загрузки файлов хостинга.';
        ...
        if (!q || q.length < 2) {
            if (globalSearchStatus)
                globalSearchStatus.textContent = 'Начните вводить название продукта. Поиск работает по всей базе, без открытия категорий.';
        ...
        const scored = DB.items.filter(p => !(p && p.hidden_from_search === true)).map(p => {
            ...
            if (!terms.every(t => searchTermMatches(text, t)))
                return null;
            let score = genericSearchPriorityBonus(p, q);
            if (name === q)
                score += 300;
            ...
            for (const t of terms) {
                ...
                if (name.split(' ').some(w => w === t))
                    score += 25;
                if (name.split(' ').some(w => w.startsWith(t)))
                    score += 10;
                if (vars.some(v => name.startsWith(v)))
                    score += 80;
                if (name.split(' ').some(w => vars.some(v => w === v || w.startsWith(v))))
                    score += 18;
                if (searchTermMatches(meta, t))
                    score += 1;
            }
            // Пользователь чаще ищет по русскому названию; технические совпадения ниже.
            if (!name.includes(q) && techName.includes(q))
                score -= 20;
            return { p, score };
        ...
        `;
            ...
            gramsEl.addEventListener('input', () => {
                ...
                if (box)
                    box.innerHTML = `<span>${fmt1(g)} г</span><span>${fmt1(kcal2)} ккал</span><span>Б ${fmt1(protein2)} г</span><span>Ж ${fmt1(fat2)} г</span><span>У ${fmt1(carbs2)} г</span>`;
            });
            row.querySelector('button[data-role="add"]').addEventListener('click', () => {
                const g = Math.max(1, Math.min(5000, parseFloat(gramsEl.value) || grams));
                const added = State.add(p.key, g);
                if (!added) {
                    try {
                        window.dispatchEvent(new CustomEvent('ration:add-ui-failed', { detail: { key: p.key, grams: g, source: 'core_search' } }));
                    }
                    catch (_) { }
                    alert('Не удалось добавить продукт в рацион. Обновите страницу и повторите попытку.');
                }
            });
    ...
    function attachEvents() {
        ...
        resetBtn.addEventListener('click', () => { if (confirm('Очистить рацион и сбросить данные калькулятора потребностей?')) {
            State.clear();
            const nrb = document.getElementById('needs_reset_btn');
            if (nrb)
                nrb.click();
            else if (window.resetNeeds)
                window.resetNeeds();
        } });
        if (clearRationBtn) {
            clearRationBtn.addEventListener('click', () => {
                ...
                if (!hasItems) {
                    alert('Рацион уже пуст.');
                    return;
                }
                if (confirm('Очистить Итоговый рацион?'))
                    State.clear();
            });
        }
        ...
        normProfile.addEventListener('change', (e) => {
            if (e.target.value === 'custom') {
                ...
                norms.profile = 'adult';
                norms.values = JSON.parse(JSON.stringify(DAILY_NORMS_DEFAULT));
                renderTotals();
                Debug.snap();
            }
        });
    ...
    function init() {
        buildAccordion();
        if (uxDbCount)
            uxDbCount.textContent = DB.items.length;
        renderGlobalResults('');
        renderRation();
        renderTotals();
        attachEvents();
        ...
        if (issues.length)
            Logger.warn('SCHEMA_ISSUES', issues.slice(0, 5));
        Debug.snap();
        const region = State.getRegion();
...
const Debug = (() => {
    ...
    function perfPush(p) { perf.push(Object.assign({ ts: Date.now() }, p)); }
    function render() {
        const ration = State.get();
