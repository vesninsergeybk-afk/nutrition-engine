...
(function () {
    ...
    function getRationItems() {
        try {
            var items = JSON.parse(localStorage.getItem('nutri_ration_v1') || '[]');
    ...
    function initMiniCart() {
            ...
            if (target) {
                ...
                try {
                    ...
                    setTimeout(function () { try {
                        target.removeAttribute('data-scroll-intent');
                    }
                    catch (_) { } }, 600);
                }
            }
        ...
        document.addEventListener('input', function (e) {
            if (e.target && e.target.closest('#rationSection'))
                setTimeout(updateMiniCart, 80);
        }, true);
    ...
    function initNeedsCompact() {
        ...
        if (!section || !needs || byId('v40NeedsSummary'))
            return;
        var summary = document.createElement('div');
        summary.id = 'v40NeedsSummary';
        summary.className = 'v40-needs-summary';
        summary.innerHTML = '<div><div class="v40-needs-summary-title">Потребности и нормы</div><div class="v40-needs-summary-text" id="v40NeedsStatus">Можно сразу собирать рацион через поиск. Расчёт потребностей нужен для сравнения с дневными нормами.</div></div><button type="button" class="secondary v40-needs-toggle" id="v40NeedsToggle">Открыть калькулятор</button>';
        ...
        function setOpen(open) {
            ...
            if (toggle)
                toggle.textContent = open ? 'Свернуть калькулятор' : 'Открыть калькулятор';
            try {
                localStorage.setItem('nutri_v40_needs_open', open ? '1' : '0');
            }
            catch (_) { }
        }
        function updateStatus(calculated) {
            ...
            if (calculated || (text && !/Введите исходные данные|нажмите «Рассчитать»/i.test(text))) {
                status.textContent = 'Потребности рассчитаны или заполнены. Нормы калькулятора можно сверять с вашим целевым рационом.';
            }
            else {
                status.textContent = 'Можно сразу собирать рацион через поиск. Расчёт потребностей нужен для сравнения с дневными нормами.';
            }
        }
        var urlOpen = false;
        try {
            urlOpen = new URL(window.location.href).searchParams.get('needs') === '1';
        }
        catch (_) { }
        var savedOpen = true;
        try {
        ...
        if (toggle)
            toggle.addEventListener('click', function () { setOpen(section.classList.contains('v40-needs-collapsed')); });
        var calc = byId('needs_calc_btn');
        if (calc)
            calc.addEventListener('click', function () { setTimeout(function () { updateStatus(true); }, 220); }, true);
        updateStatus(false);
    }
    function initDevMode() {
        ...
        try {
            ...
            if (params.get('dev') === '1')
                localStorage.setItem('nutri_dev_enabled', '1');
        }
        catch (_) { }
    ...
    function initTotalsCompact() {
        ...
        if (details && !details.dataset.v40Touched) {
            ...
            if (summary)
                summary.textContent = 'Расширенные нутриенты и витамины/минералы — раскрыть';
        }
    }
    function initClearConfirm() {
        var btn = byId('clearRationBtn');
        if (!btn || btn.dataset.v40ConfirmBound)
            return;
    ...
    function initCopyText() {
        var deployVersion = String(window.__APP_DEPLOY_VERSION__ || 'v5.3.178');
        document.title = 'Калькулятор нутриентов Сергея Веснина ' + deployVersion;
        ...
        if (lead)
            lead.textContent = 'Начните вводить продукт, укажите граммы и добавьте позицию в рацион. На телефоне найденные продукты появляются сразу под строкой поиска.';
        ...
        if (pdfBtn && pdfBtn.textContent.trim() === 'PDF')
            pdfBtn.textContent = 'Сохранить PDF';
        var needsPdf = byId('needs_pdf_btn');
        if (needsPdf && needsPdf.textContent.trim() === 'PDF')
            needsPdf.textContent = 'Сохранить PDF';
    }
    function initTopNote() {
        var search = byId('globalSearchSection');
        if (!search || byId('v40TopNote'))
            return;
    ...
    function initTests() {
        ...
        window.runSmokeTests = function () {
            ...
            try {
                ...
                ok = false;
                window.__v40PreviousSmokeError = String(e && e.message || e);
            }
