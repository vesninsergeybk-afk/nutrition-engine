...
(function () {
    ...
    function statusTone(state) {
        if (state === 'target')
            return 'ok';
        if (state === 'over' || state === 'strong-over')
            return 'over';
        if (state === 'very-low' || state === 'low' || state === 'below-target')
            return 'low';
        return 'empty';
    }
    function getRation() {
        ...
        if (window.State && typeof window.State.get === 'function')
            return flat(window.State.get() || []);
        if (window.Ration && Array.isArray(window.Ration.current))
            return window.Ration.current || [];
        try {
            return JSON.parse(localStorage.getItem('nutri_ration_v1') || '[]');
        }
        catch (_) {
            return [];
        }
    }
    function saveRationFallback(ration) { try {
        localStorage.setItem('nutri_ration_v1', JSON.stringify(ration || []));
    }
    catch (_) { } }
    ...
    function tagsOf(p) {
        ...
        if (Array.isArray(raw))
            return raw.map(function (t) { return lower(t); }).filter(Boolean);
        ...
        if (typeof raw === 'string')
            return raw.split(/[|,;]+/).map(function (t) { return lower(t).trim(); }).filter(Boolean);
        return [];
    }
    ...
    function isProteinSupplement(p) {
        if (isSoyProteinWithHeiCredit(p))
            return false;
        var t = textOf(p);
    ...
    function isSaturatedFatRouteProduct(p) {
        ...
        if (catalog === 'saturated_fats' || fatQuality === 'saturated_fat_source' || limitGroup === 'saturated_fats')
            return true;
        if (tagList.indexOf('saturated_fat_source') >= 0 || tagList.indexOf('limit_saturated_fat') >= 0)
            return true;
        if (anyWord(t, /морожен|ice\s*cream|чипс|chips|соус|sauce|nut\s*butter|seed\s*butter|peanut\s*butter|almond\s*butter|орехов[а-я\s]*паст|арахисов[а-я\s]*паст|миндальн[а-я\s]*паст|паста\s+из\s+тыквен/))
            return false;
    ...
    function isExplicitDairyName(p) {
        ...
        if (isSaturatedFatRouteProduct(p))
            return false;
        if (anyWord(t, /сосиск|колбас|котлет|meatball|sausage|salami|bacon/))
            return false;
        if (anyWord(t, /молоко|йогурт|творог|кефир|айран|ряженк|простокваш|сметан|сливки|сырок|ск[иі]р|milk|yogurt|curd|cottage|kefir|ayran|sour cream|cream|cheese/))
            return true;
    ...
    function isDairyPrimary(p) {
        ...
        if (isSaturatedFatRouteProduct(p) || isProteinSupplement(p) || isProcessedMeat(p) || isFattySauceOrNonRecommendedFat(p))
            return false;
        if (hei === 'dairy' || cat === 'dairy')
            return true;
        ...
        if (tagList.indexOf('dairy') >= 0 && tagList.indexOf('ready_meal') < 0 && tagList.indexOf('ready_food') < 0 && tagList.indexOf('dairy_derived') < 0)
            return true;
    ...
    function isDessertOrSweetMainProduct(p) {
        var t = textOf(p);
        if (isDairyPrimary(p) && anyWord(t, /десерт|слад|глазирован|мёд|мед|джем|варень|шоколад|(^|[^a-z])(sweet|sweetened)($|[^a-z])|glazed|honey|jam|chocolate/i))
            return true;
    ...
    function isJuiceOrSmoothie(p) {
        ...
        if (tagList.indexOf('juice') >= 0 || tagList.indexOf('smoothie') >= 0 || tagList.indexOf('nectar') >= 0)
            return true;
        // v5.2.33: "рыба в собственном соку" must not become juice/beverage.
        if (anyWord(t, /в\s+собственн.*сок|в\s+соку|own juice|in juice/))
            return false;
        return anyWord(t, /(^|[^а-яё])сок($|[^а-яё])|смузи|нектар|\bjuice\b|smoothie|nectar/);
    }
    function isSweetDrink(p) {
        var t = productKeyName(p), tagList = tagsOf(p);
        if (tagList.indexOf('sweet_drink') >= 0 || tagList.indexOf('soda') >= 0)
            return true;
        if (anyWord(t, /газиров|лимонад|cola|soda|soft drink|sweet drink/))
            return true;
        return false;
    }
    function isUnsweetenedBeverage(p) {
        var t = productKeyName(p), tagList = tagsOf(p);
        if (isPlainWaterProduct(p) || isJuiceOrSmoothie(p) || isSweetDrink(p))
            return false;
        if (tagList.indexOf('unsweetened_beverage') >= 0 || tagList.indexOf('tea') >= 0 || tagList.indexOf('coffee') >= 0)
            return true;
        if (anyWord(t, /(^|[^a-zа-яё])(чай|кофе|tea|coffee)($|[^a-zа-яё])/i))
            return !anyWord(t, /сахар|sweet|sugar|латте|капучино|молок|milk/);
        return false;
    }
    ...
    function isWhiteRiceRefined(p) {
        var t = productKeyName(p);
        if (!anyWord(t, /рис|rice|basmati|басмати/))
            return false;
        if (anyWord(t, /бурый|коричнев|дикий|brown|wild|whole/))
            return false;
    ...
    function isHarvardExcludedPotato(p) {
        ...
        if (tagList.indexOf('potato') < 0 && !anyWord(t, /картоф|potato|(^|[^а-яё])фри($|[^а-яё])|(^|[^a-z])fries($|[^a-z])/))
            return false;
        if (anyWord(t, /батат|sweet potato/))
            return false;
        ...
        if ((lower(p && p.hei_category_key) === 'protein_foods' || lower(p && p.hei_category_key) === 'seafood_plant_protein')
            && num(p && p.protein_oz_eq_per_100g) > 0)
            return false;
        return true;
    }
    function isFruitLike(p) {
        ...
        if (isSaturatedFatRouteProduct(p) || hasTag(p, /^(not_fruit_credit|avocado|healthy_fat_source)$/) || anyWord(productKeyName(p), /авокад|avocado|масло|(^|[^a-z])oil($|[^a-z])/))
            return false;
        if (fruitEq > 0 || wholeFruitEq > 0)
            return true;
    ...
    function isVegetableLike(p) {
        ...
        if (hasTag(p, /^(healthy_fat_source|not_vegetable_credit|avocado)$/) || anyWord(productKeyName(p), /авокад|avocado/))
            return false;
        if (vegEq > 0)
            return true;
        if (fruitEq > 0)
    ...
    function isWholeGrainLike(p) {
        ...
        if (hasTag(p, /^(not_whole_grain_plate_credit|processed_snack)$/))
            return false;
        var wholeEq = num(p && p.whole_grain_oz_eq_per_100g);
        var refinedEq = num(p && p.refined_grain_oz_eq_per_100g);
        if (isWhiteRiceRefined(p))
            return false;
        if (isDessertOrSweetMainProduct(p) && wholeEq <= 0)
            return false;
        if ((hei === 'refined_grains' || refinedEq > 0) && wholeEq <= 0)
            return false;
    ...
    function isHealthyFatSource(p) {
        ...
        if (tagList.indexOf('not_healthy_fat_source') >= 0)
            return false;
        if (tagList.indexOf('healthy_fat_source') >= 0)
            return true;
        // v5.3.121: bread or a grain snack containing seeds remains a grain product, not 100 g of a fat source.
        if (num(p && p.whole_grain_oz_eq_per_100g) > 0 || num(p && p.refined_grain_oz_eq_per_100g) > 0)
            return false;
        if (isFattySauceOrNonRecommendedFat(p))
            return false;
        if (anyWord(t, /масло олив|olive oil|масло подсолнеч|sunflower oil|масло рапс|canola oil|авокад|avocado|тахини|tahini|almond|миндал|грецк|walnut|cashew|кешью|pumpkin seed|тыквен|sunflower seed|семеч|nuts|орех/))
            return true;
        if (anyWord(t, /butter|сливочн|маргарин|ghee|сало/))
            return false;
        return false;
    }
    function isProteinLike(p) {
        var t = textOf(p), hei = lower(p && p.hei_category_key), tagList = tagsOf(p);
        if (isProteinSupplement(p) || isProcessedMeat(p) || isDairyPrimary(p))
            return false;
        var explicitProteinEq = num(p && p.protein_oz_eq_per_100g) > 0 || num(p && p.seafood_plant_oz_eq_per_100g) > 0;
        if (!explicitProteinEq && anyWord(t, /масло|(^|[^a-z])oil($|[^a-z])|сливочн|маргарин|ghee/))
            return false;
        if (tagList.indexOf('nuts') >= 0 || tagList.indexOf('nut') >= 0 || tagList.indexOf('nut_butter') >= 0 || tagList.indexOf('seeds') >= 0 || tagList.indexOf('seed') >= 0 || anyWord(t, /орех|семеч|семян|миндаль|грецк|кешью|nuts?|seeds?|almond|walnut|cashew/))
            return true;
    ...
    function isProteinDominantForPlate(p) {
        if (lower(p && p.harvard_plate_group) === 'protein')
            return true;
    ...
    function buildV5Input(ration) {
        ...
        (ration || []).forEach(function (it) {
            ...
            if (plainWater) {
                input.waterPlainMl += g;
                roles.push('waterPlain');
            }
            if (unsweetened) {
                input.unsweetenedBeverageMl += g;
                roles.push('unsweetenedBeverage');
            }
            if (dairyDrink) {
                input.dairyDrinkMl += g;
                roles.push('dairyDrink');
            }
            if (juice) {
                input.juiceMl += g;
                roles.push('juice');
            ...
            if (sweetDrink) {
                input.sweetDrinkMl += g;
                roles.push('sweetDrink');
            }
            if (fattySauce) {
                input.fattySauceG += g;
                roles.push('fattySauce');
            }
            if (healthyFat) {
            ...
            if (refinedGrain) {
                input.refinedGrainsG += g;
                roles.push('refinedGrain');
            }
            if (processedMeat) {
                input.processedMeatG += g;
                roles.push('processedMeat');
            }
            if (soyProteinDairyAlt) {
                roles.push('soyProteinDairyAlternative');
            }
            if (proteinSupplement) {
                roles.push('proteinSupplement');
            }
            if (harvardPotato) {
                roles.push('harvardPotato');
            }
            var mainGroup = null;
            var reason = '';
            if (wholeGrain) {
                input.wholeGrainsG += g;
                mainGroup = 'wholeGrains';
                roles.push('wholeGrains');
            }
            else if (fruit) {
                input.fruitsG += g;
                mainGroup = 'fruits';
            ...
            else if (protein && isProteinDominantForPlate(p)) {
                input.proteinG += g;
                mainGroup = 'protein';
                roles.push('protein');
            }
            ...
            else if (refinedGrain)
                reason = 'рафинированные злаки учитываются отдельно от цельнозернового сектора';
            else if (harvardPotato)
                reason = 'картофель и картофельные блюда показаны отдельно от овощного сектора Гарвардской тарелки';
            else if (proteinSupplement)
                reason = 'протеиновые добавки показаны вне основной тарелки и не заменяют сектор здорового белка';
            else if (processedMeat)
                reason = 'обработанное мясо показано вне основной тарелки: Гарвардская тарелка рекомендует избегать таких источников белка';
            else if (dairy)
                reason = 'молочные продукты учитываются отдельной строкой и не заполняют фруктовый или белковый сектор';
            else if (mixedRetailAggregate)
                reason = 'смешанный товарный продукт имеет раздельные HEI-вклады и не относится целиком к одному сектору без декомпозиции';
            else if (mixedBlocksFruitVeg)
                reason = 'смешанный сладкий продукт не заполняет сектор фруктов или овощей';
            else if (juice)
                reason = 'соки и смузи не входят в водный индикатор';
            else if (sweetDrink)
                reason = 'сладкие напитки не входят в водный индикатор';
            else if (fattySauce)
                reason = 'соус, жирная закуска или обработанный продукт учитывается вне основной тарелки';
            else if (healthyFat && !protein)
                reason = 'источник жиров учитывается вне основной площади тарелки';
            else if (plainWater || unsweetened)
                reason = 'вода и несладкие напитки учитываются отдельным индикатором';
    ...
    function strongestDeviationSector(stage) {
        ...
        SECTOR_IDS.forEach(function (id) {
            ...
            if (!best || d > best.deviation)
                best = { id: id, deviation: d };
        });
    ...
    function getModel() {
        ...
        if (!ready)
            return { ready: false, empty: empty, message: 'База продуктов или модуль визуализации ещё загружаются.' };
        ...
        if (!activeSector || !stage.sectors[activeSector] || (!userSelectedSector && activeSector !== weakest))
            activeSector = weakest;
    ...
    function itemListHtml(items, emptyText) {
        if (!items.length)
            return '<p class="hp-mini-empty">' + esc(emptyText || 'Пока нет продуктов.') + '</p>';
    ...
    function separateItemsForSector(model, sectorId) {
        ...
        if (sectorId === 'protein')
            return details.filter(function (it) { return !it.group && (hasRole(it, 'dairy') || hasRole(it, 'processedMeat')); });
        if (sectorId === 'wholeGrains')
            return details.filter(function (it) { return hasRole(it, 'refinedGrain'); });
        if (sectorId === 'vegetables')
            return details.filter(function (it) { return hasRole(it, 'fruits') && !hasRole(it, 'vegetables'); });
        if (sectorId === 'fruits')
            return details.filter(function (it) { return hasRole(it, 'vegetables') && !hasRole(it, 'fruits'); });
        return [];
    }
    ...
    function explainHtml(model) {
        if (!model.ready)
            return '<p class="hp-mini-empty">Модуль ещё загружается.</p>';
        if (model.empty)
            return '<p class="hp-mini-empty">Добавьте продукты, чтобы увидеть распределение по группам.</p>';
    ...
    function adviceSentence(sector) {
        ...
        if (!sector || sector.state === 'empty')
            return 'Добавьте продукты, чтобы сектор появился в расчёте.';
        if (a.type === 'add')
            return 'Добавьте примерно ' + fmt(a.grams, 0) + ' г: так доля сектора приблизится к ориентиру.';
        if (a.type === 'reduce')
            return 'Сократите примерно на ' + fmt(a.grams, 0) + ' г или увеличьте группы, которые сейчас отстают.';
        return 'Доля близка к ориентиру.';
    }
    function missingSectorLabels(model) {
        if (!model || !model.stage || !model.stage.sectors)
            return [];
    ...
    function dominantSector(model) {
        ...
        SECTOR_IDS.forEach(function (id) {
            ...
            if (!s)
                return;
            if (!best || num(s.actualPercent) > num(best.actualPercent))
                best = { id: id, actualPercent: num(s.actualPercent), grams: num(s.grams) };
        });
        return best;
    }
    function plateDiagnosisText(model) {
        if (!model || !model.ready)
            return 'Ждём загрузку данных.';
        if (model.empty)
            return 'Добавьте продукты, чтобы оценить структуру тарелки.';
        if (model.coreMass <= 0)
            return 'Основная тарелка пока пуста: продукты есть, но они относятся к группам вне основного круга.';
        ...
        if (missing.length >= 2)
            return 'Структура основной тарелки заметно неполная: не хватает ' + missing.slice(0, 3).join(', ').toLowerCase() + '.';
        if (dom && dom.actualPercent >= 70)
            return 'Основная тарелка перекошена: большую часть занимает сектор «' + sectorTitle(dom.id) + '».';
        if (coreShare < 0.5)
            return 'Большая часть рациона учтена вне основной тарелки, поэтому основной круг показывает только часть питания.';
        if (model.score >= 80)
            return 'Структура основной тарелки близка к ориентиру.';
    ...
    function firstActionText(model) {
        ...
        if (items && items.length)
            return items[0].text;
        if (model && model.score >= 80)
            return 'Критичных структурных действий нет: можно смотреть качество продуктов через HEI.';
    ...
    function topSummaryHtml(model) {
        ...
        var mismatch = '';
        if (hei.total != null) {
            if (hei.total >= 70 && structure < 60)
                mismatch = 'HEI может быть приемлемым, но структура тарелки всё равно перекошена: проверьте доли основных секторов.';
            else if (hei.total < 60 && structure >= 70)
                mismatch = 'Структура похожа на тарелку, но качество продуктов по HEI требует проверки.';
            else
                mismatch = 'HEI оценивает качество, а Гарвардская тарелка — структуру. Эти два вывода нужно читать вместе.';
        }
        else {
        ...
        if (missing.length)
            cause += ' Не хватает: ' + missing.slice(0, 4).join(', ').toLowerCase() + '.';
        else if (dom && model.coreMass > 0)
            cause += ' Самый крупный сектор: ' + sectorTitle(dom.id).toLowerCase() + ' — ' + fmt(dom.actualPercent, 0) + '%.';
    ...
    function interactionGuideHtml(model) {
        if (!model || !model.ready || model.empty || model.coreMass <= 0)
            return '';
    ...
    function sectorKcalImpact(id, grams) {
        ...
        if (!grams)
            return 'энергия зависит от выбранного продукта';
    ...
    function recommendationItems(model) {
        ...
        SECTOR_IDS.forEach(function (id) {
            ...
            var addGrams = 0;
            if (s.state === 'empty')
                addGrams = sectorDefaultAddGrams(id);
            else if (s.state === 'low' && s.adjustment && s.adjustment.type === 'add')
                addGrams = Number(s.adjustment.grams) || 0;
            if (!addGrams)
                return;
            items.push({
                tone: 'need',
    ...
    function recommendationCardHtml(rec, i) {
        ...
        if (rec.sector === 'outside') {
            ...
            var actionAttr = ' data-hp-open-whatif="' + esc(rec.sector || '') + '"' +
    ...
    function qualityFlagsHtml(model) {
        ...
        if (!flags.length)
            return '<section class="hp-quality-card" data-empty="true"><strong>Качественные замечания</strong><p>Сейчас явных замечаний нет.</p></section>';
    ...
    function activePanelHtml(model) {
        if (!model.ready || model.empty || model.coreMass <= 0)
            return '<div class="hp-active-panel hp-empty-panel"><h3>Тарелка готова к расчёту</h3><p>Добавьте продукты или загрузите пример. После изменения рациона блок обновится сам.</p></div>';
    ...
    function companionIconSvg(kind) {
        ...
        if (kind === 'water')
            return '<svg class="hp-companion-inline-icon" viewBox="0 0 64 64" aria-hidden="true"><path d="M32 7C23 20 17 29 17 39a15 15 0 0 0 30 0C47 29 41 20 32 7Z" fill="' + fill + '" stroke="' + stroke + '" stroke-width="4"/><path d="M25 43c4 5 11 5 15 0" fill="none" stroke="' + stroke + '" stroke-width="3" stroke-linecap="round" opacity=".55"/></svg>';
        if (kind === 'healthyFats')
            return '<svg class="hp-companion-inline-icon" viewBox="0 0 64 64" aria-hidden="true"><ellipse cx="31" cy="35" rx="18" ry="22" fill="' + fill + '" stroke="' + stroke + '" stroke-width="4"/><circle cx="33" cy="36" r="7" fill="#fff7ed" stroke="' + stroke + '" stroke-width="3"/><path d="M41 16c7-5 13-4 16-1-2 6-8 10-16 9" fill="#dcfce7" stroke="#16a34a" stroke-width="3" stroke-linejoin="round"/></svg>';
        if (kind === 'dairy')
            return '<svg class="hp-companion-inline-icon" viewBox="0 0 64 64" aria-hidden="true"><path d="M23 10h18l-3 11 8 8v22a6 6 0 0 1-6 6H24a6 6 0 0 1-6-6V29l8-8-3-11Z" fill="' + fill + '" stroke="' + stroke + '" stroke-width="4" stroke-linejoin="round"/><path d="M21 37h22" stroke="' + stroke + '" stroke-width="3" opacity=".45"/></svg>';
    ...
    function companionStateIcon(kind, state) {
        if (kind === 'water')
            return ASSET_ROOT + 'companions/states/' + (state === 'target' ? 'water-target-v5.svg' : state === 'mid' ? 'water-mid-v5.svg' : 'water-low-v5.svg');
        if (kind === 'healthyFats')
            return ASSET_ROOT + 'companions/states/' + (state === 'target' ? 'healthy-fats-target-v5.svg' : state === 'over' ? 'healthy-fats-over-v5.svg' : 'healthy-fats-low-v5.svg');
        if (kind === 'dairy')
            return ASSET_ROOT + 'companions/states/dairy-separate-v5.svg';
    ...
    function companionsHtml(model) {
        ...
        if ((water.juiceMl || 0) > 0 || (water.sweetDrinkMl || 0) > 0)
            waterNote += ' Соки и сладкие напитки показаны отдельно.';
    ...
    function ringSectorPath(cx, cy, rOuter, rInner, start, sweep) {
        if (!Number.isFinite(sweep) || Math.abs(sweep) < 0.001 || rOuter <= rInner)
            return '';
        var end = start + sweep;
    ...
    function svgOverfillArc(id, sector) {
        if (!sector || sector.fillRatio <= 1.05)
            return '';
    ...
    function renderSvg(model) {
        if (isMobileActualMode())
            return renderMobileActualSvg(model);
        var stage = model.stage;
    ...
    function renderStatus(model) {
        ...
        if (!scoreRing || !scoreText || !metrics)
            return;
        if (!model.ready) {
            scoreRing.style.setProperty('--plate-score', '0');
        ...
        scoreRing.style.setProperty('--plate-score', String(clamp(model.score, 0, 100)));
        scoreRing.textContent = model.score + '%';
    ...
    function render() {
        var model = getModel();
        try {
            ['HarvardPlateV5337', 'HarvardPlateV5336', 'HarvardPlateV5335', 'HarvardPlateV5332', 'HarvardPlateV5331', 'HarvardPlateV5330', 'HarvardPlateV5329', 'HarvardPlateV5328', 'HarvardPlateV5221', 'HarvardPlateV5220', 'HarvardPlateV5219', 'HarvardPlateV5214', 'HarvardPlateV5213', 'HarvardPlateV5212', 'HarvardPlateV5211', 'HarvardPlateV529', 'HarvardPlateV528', 'HarvardPlateV527', 'HarvardPlateV526', 'HarvardPlateV525', 'HarvardPlateV524', 'HarvardPlateV523'].forEach(function (name) { if (window[name])
                window[name].__lastModel = model; });
        }
        catch (_) { }
        renderVisual(model);
        renderStatus(model);
    ...
    function bindDynamicClicks() {
        document.addEventListener('click', function (e) {
            ...
            if (whatif) {
                ...
                if (wid && SECTOR_IDS.indexOf(wid) >= 0)
                    selectSector(wid);
                try {
            ...
            if (outside) {
                ...
                try {
                    ...
                    if (box)
                        box.setAttribute('data-highlight', '1');
                    setTimeout(function () { if (box)
                        box.removeAttribute('data-highlight'); }, 1600);
                }
                catch (_) { }
                return;
            }
            ...
            if (sector) {
                e.preventDefault();
                selectSector(sector.getAttribute('data-hp-sector'));
            }
            else if (card) {
                e.preventDefault();
                selectSector(card.getAttribute('data-hp-sector-card'));
            }
        });
        document.addEventListener('keydown', function (e) {
            ...
            if (e.key === 'Enter' || e.key === ' ') {
                ...
                e.preventDefault();
                selectSector(SECTOR_IDS[0]);
            }
            else if (e.key === 'End') {
                e.preventDefault();
                selectSector(SECTOR_IDS[SECTOR_IDS.length - 1]);
            }
        });
    }
    ...
    function loadSample() {
        ...
        var rows = [], added = 0;
        SAMPLE.forEach(function (it) { var p = product(it.key); if (p)
            rows.push({ key: p.key, name_ru: p.name_ru, hei_category_key: p.hei_category_key, catalog_category_key: p.catalog_category_key, grams: it.grams }); });
        if (rows.length < 7) {
            FALLBACK_SAMPLE_NAMES.forEach(function (pair) { var p = findFallbackProduct(pair[0]); if (p && !rows.some(function (r) { return r.key === p.key; }))
                rows.push({ key: p.key, name_ru: p.name_ru, hei_category_key: p.hei_category_key, catalog_category_key: p.catalog_category_key, grams: pair[1] }); });
        }
        if (window.State && typeof window.State.clear === 'function' && typeof window.State.add === 'function') {
            patchStateForEvents();
            window.State.clear();
            rows.forEach(function (r) { if (product(r.key)) {
                window.State.add(r.key, r.grams);
                added++;
            } });
        }
        else {
            saveRationFallback(rows);
            added = rows.length;
        }
        userSelectedSector = false;
        if (btn) {
            btn.textContent = added ? 'Пример загружен' : 'Не удалось загрузить пример';
            window.setTimeout(function () { btn.textContent = 'Показать пример сбалансированной тарелки'; }, 1800);
        }
        try {
            ...
            if (panel) {
                ...
                try {
                    ...
                    setTimeout(function () { try {
                        panel.removeAttribute('data-scroll-intent');
                    }
                    catch (_) { } }, 600);
                }
            }
    ...
    function bind() {
        bindDynamicClicks();
        var btn = $('harvardPlateSampleBtn');
        if (btn && !btn.__harvardBound) {
            btn.__harvardBound = true;
            btn.addEventListener('click', function (e) { e.preventDefault(); loadSample(); });
        }
        var refresh = $('harvardPlateRefreshBtn');
        if (refresh && !refresh.__harvardBound) {
            refresh.__harvardBound = true;
            refresh.addEventListener('click', function (e) { e.preventDefault(); scheduleRender('manual'); });
        }
        ...
        window.addEventListener('storage', function (e) { if (!e || e.key === 'nutri_ration_v1')
            scheduleRender('ration-change'); });
        var tries = 0;
        (function waitState() { tries++; if (!patchStateForEvents() && tries < 20)
            window.setTimeout(waitState, 150); })();
    }
    ...
    function runTests() {
        ...
        try {
            var model = getModel();
            row('model computes', !!model);
            row('model uses four sectors', !!(model.stage && SECTOR_IDS.every(function (id) { return !!model.stage.sectors[id]; })));
        }
        catch (e) {
            row('model computes', false, e && (e.message || String(e)));
        }
        try {
            row('render runs', typeof render() === 'boolean');
        }
        catch (e2) {
            row('render runs', false, e2 && (e2.message || String(e2)));
        }
        if (console && console.table)
            console.table(rows);
        return rows.every(function (r) { return !!r.pass; });
