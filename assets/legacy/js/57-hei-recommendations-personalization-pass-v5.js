...
(function () {
    ...
    function joinSources(items) { items = (items || []).filter(Boolean); if (items.length < 2)
        return items[0] || ''; if (items.length === 2)
        return items[0] + ' и ' + items[1]; return items.slice(0, -1).join(', ') + ' и ' + items[items.length - 1]; }
    function productName(p) { return shortName(p && (p.name_ru || p.name) || 'Продукт'); }
    function dbByKey(db, key) {
        if (!db || !key)
            return null;
        try {
            if (db.byKey && typeof db.byKey.get === 'function')
    ...
    function rawRation() { try {
        return window.State && typeof window.State.get === 'function' ? (window.State.get() || []) : [];
    }
    catch (_) {
        return [];
    } }
    function flattenRation(raw) {
        try {
            ...
            if (api && typeof api.flattenForCalculation === 'function')
                return api.flattenForCalculation(raw || []);
            if (api && typeof api.flattenCompositeForCalculation === 'function')
                return api.flattenCompositeForCalculation(raw || []);
        }
        catch (_) { }
    ...
    function makeRawIndex(raw) {
        var out = {};
        (raw || []).forEach(function (x) { if (!x)
            return; var id = String(x.id || x.key || ''); if (id)
            out[id] = x; });
        return out;
    }
    ...
    function addContribution(row, p, g) {
        ...
        row.kcal += f('kcal');
        row.proteinG += f('protein_per_100g');
        row.fatG += f('fat_per_100g');
    ...
    function collectRationRows(flat, db, raw) {
        ...
        var rawIndex = makeRawIndex(raw || []), map = {};
        flat.forEach(function (entry) {
            if (!entry || !entry.key)
                return;
            var g = num(entry.grams, 0);
            if (g <= 0)
                return;
            var p = dbByKey(db, entry.key);
            if (!p)
                return;
            ...
            if (!map[key])
                map[key] = { id: key, key: parent && parent.key || entry.key, name: entry.parent_composite_name || (parent && (parent.name_ru || parent.name)) || productName(p), grams: 0, kcal: 0, proteinG: 0, fatG: 0, products: [], isComposite: !!parentId, confidence: compositeConfidence(parent), children: {}, contrib: blankContribution() };
            var row = map[key];
            row.grams += g;
            row.products.push(p);
            addContribution(row, p, g);
            if (parentId) {
                var childKey = String(entry.key);
                if (!row.children[childKey])
                    row.children[childKey] = { key: childKey, name: productName(p), grams: 0, kcal: 0, proteinG: 0, fatG: 0, contrib: blankContribution() };
                row.children[childKey].grams += g;
                addContribution(row.children[childKey], p, g);
            }
        });
    ...
    function currentProfile() {
        try {
            if (window.__lastNeedsProfileApplied !== true)
                return null;
            if (window.__lastPersonalNeedsProfile)
                return window.__lastPersonalNeedsProfile;
    ...
    function currentNeedsMeta() { try {
        return window.__lastNeedsProfileApplied === true ? (window.__lastNeedsMeta || null) : null;
    }
    catch (_) {
        return null;
    } }
    function buildEnergyContext(model, meta, profile) {
        ...
        var mode = 'replace';
        if (['ed_risk', 'pregnancy', 'lactation', 'medical_restriction'].indexOf(guardrail) >= 0)
            mode = 'cautious';
        else if (state === 'high' || goal === 'lose')
            mode = 'replace';
        else if (state === 'low' || goal === 'gain' || goal === 'rehab_gain' || dietStyle === 'low_appetite')
            mode = 'add';
        ...
        if (state === 'low')
            label = 'Калорийность ниже расчётного диапазона';
        if (state === 'within')
            label = 'Калорийность находится в расчётном диапазоне';
        if (state === 'high')
            label = 'Калорийность выше расчётного диапазона';
    ...
    function targetFor(action, profile, energy) {
        ...
        if (id.indexOf('fruit') === 0)
            return result('фрукты целиком или ягоды', 'около 150\u00a0г', 'возрастная порция');
        if (id === 'veg')
            return result('порцию овощей, включив в неё листовую зелень или готовые бобовые', '130–150\u00a0г', 'возрастная порция');
        if (id === 'veg-total')
            return result('некрахмалистые овощи', '130–150\u00a0г', 'возрастная порция');
        if (id === 'greens')
            return result('листовую зелень или готовые бобовые', 'порция подходящего объёма', 'возрастная порция');
        if (id === 'grains' || id === 'whole-grain' || id === 'refined')
            return result('готовую цельную крупу или цельнозерновой хлеб', '80–100\u00a0г готовой крупы', 'возрастная порция');
        if (id === 'dairy')
            return f.noDairy ? result('обогащённый кальцием растительный напиток или растительный йогурт без добавленного сахара', '200–250\u00a0мл', 'возрастная порция') : result('несладкий кисломолочный продукт или подходящую обогащённую альтернативу', '200–250\u00a0мл', 'возрастная порция');
        if (id === 'protein' || id === 'protein-quality' || id === 'protein-total') {
            if (f.renal || f.medical)
                return result('подходящий источник белка в пределах индивидуальных ограничений', 'количество, согласованное с назначениями');
            if (f.vegetarian || f.plantForward)
                return result('готовые бобовые, тофу, темпе, орехи или семена', 'одна порция');
            return result('рыбу, готовые бобовые или тофу', '80–150\u00a0г готового продукта', 'возрастная порция');
        }
        if (id === 'fats' || id === 'sat-fat' || id === 'fat-ratio')
            return (f.vegetarian || f.plantForward) ? result('орехи, семена, авокадо или растительное масло', 'количество в пределах текущей калорийности') : result('рыбу, орехи, семена или растительное масло', 'количество в пределах текущей калорийности');
        return result('подходящий продукт', 'порция, соответствующая текущему рациону');
    }
    function amountSentence(target) {
        var amount = String(target && target.amount || '').trim();
        if (!amount)
            return '';
        if (amount.indexOf('согласован') >= 0)
            return 'Количество согласуйте с индивидуальными назначениями.';
        if (amount.indexOf('пределах текущей калорийности') >= 0)
            return 'Количество подберите в пределах текущей калорийности.';
        return 'Ориентир по количеству — ' + amount + '.';
    }
    function scoreForAction(row, id) {
        var c = row.contrib || {};
        if (id === 'fruit' || id === 'fruit-total')
            return c.fruits_total;
        if (id === 'fruit-whole')
            return c.processed_fruit || c.fruits_whole;
        if (id === 'veg' || id === 'veg-total')
            return c.vegetables_total;
        if (id === 'greens')
            return c.greens_beans;
        if (id === 'grains' || id === 'refined')
            return c.grains_refined;
        if (id === 'whole-grain')
            return c.grains_whole;
        if (id === 'dairy')
            return c.dairy;
        if (id === 'protein')
            return c.protein_total + c.seafood_plant;
        if (id === 'protein-quality')
            return c.animal_protein;
        if (id === 'protein-total')
            return c.protein_total;
        if (id === 'sodium')
            return c.sodium_g;
        if (id === 'sugar')
            return c.added_sugars_pct;
        if (id === 'fats' || id === 'sat-fat' || id === 'fat-ratio')
            return c.sat_fats_pct;
        return 0;
    }
    function targetScoreForAction(row, id) {
        var c = row.contrib || {};
        if (id === 'fruit' || id === 'fruit-whole')
            return c.fruits_whole;
        if (id === 'fruit-total')
            return c.fruits_total;
        if (id === 'veg' || id === 'greens')
            return c.greens_beans;
        ...
        if (id === 'dairy')
            return c.dairy;
        if (id === 'protein' || id === 'protein-quality')
            return c.seafood_plant;
        if (id === 'protein-total')
            return c.protein_total;
        if (id === 'fats' || id === 'sat-fat' || id === 'fat-ratio')
            return c.unsat;
    ...
    function formatSource(row, id) {
        ...
        if (id === 'sodium')
            base += ' (' + fmt(c.sodium_g, 0) + '\u00a0мг натрия)';
        if (id === 'sugar')
            base += ' (' + fmt(c.added_sugars_pct, 1) + '\u00a0г добавленного сахара)';
        if (id === 'fats' || id === 'sat-fat' || id === 'fat-ratio')
            base += ' (' + fmt(c.sat_fats_pct, 1) + '\u00a0г насыщённых жиров)';
        return base;
    }
    function compositeChildText(row, id) {
        if (!row || !row.isComposite || !row.childRows || !row.childRows.length)
            return '';
    ...
    function reliabilityNote(action, rows) {
        var comps = topRows(rows, action.id, 2).filter(function (r) { return r.isComposite; });
        if (!comps.length)
            return '';
    ...
    function sourceText(action, rows) {
        var top = topRows(rows, action.id, 2);
        if (!top.length)
            return 'В текущем рационе пока нет продукта, который заметно формирует этот показатель.';
        ...
        top.forEach(function (r) { text += compositeChildText(r, action.id); });
        return text;
    }
    ...
    function donorRows(rows, action) {
        return (rows || []).filter(function (r) { return !isTargetRow(r, action); }).map(function (r) {
            var c = r.contrib || {};
            return { row: r, score: r.kcal * 0.01 + c.added_sugars_pct * 3 + c.sat_fats_pct * 2 + c.sodium_g / 450 + c.grains_refined * 3 };
        }).filter(function (x) { return x.score > 0.5; }).sort(function (a, b) { return b.score - a.score; });
    }
    function calcReduction(action, source, plan) {
        if (!source || source.grams <= 0)
            return 0;
        var key = '', metric = null, perGram = 0, c = source.contrib || {};
        if (action.id === 'sodium') {
            key = 'sodium_g';
            perGram = c.sodium_g / source.grams;
        }
        else if (action.id === 'sugar') {
            key = 'added_sugars_pct';
            perGram = c.added_sugars_pct / source.grams;
        }
        else if (action.id === 'fats' || action.id === 'sat-fat') {
            key = 'sat_fats_pct';
            perGram = c.sat_fats_pct / source.grams;
        }
        else if (action.id === 'grains' || action.id === 'refined') {
            key = 'grains_refined';
            perGram = (c.grains_refined * 85) / source.grams;
        }
        if (!key || perGram <= 0)
            return 0;
        metric = plan && plan.metrics && plan.metrics[key];
        var need = num(metric && metric.next, 0);
        if (need <= 0)
            return 0;
    ...
    function relevantProfileNote(action, profile, energy) {
        ...
        var f = profileFlags(profile, energy), id = action.id;
        if (f.protected)
            return 'В защитном сценарии рекомендация служит мягким ориентиром и не заменяет индивидуальный план питания.';
        if (id === 'dairy' && f.noDairy)
            return 'Для рациона без молочных продуктов предлагаются только обогащённые растительные альтернативы.';
        if (id.indexOf('protein') === 0 && (f.renal || f.medical))
            return 'При выбранном медицинском профиле количество и источник белка не изменяются автоматически.';
        if (id.indexOf('protein') === 0 && (f.vegetarian || f.plantForward))
            return 'При растительном пищевом стиле приоритет отдан бобовым, соевым продуктам, орехам и семенам.';
        if (f.medical)
            return 'При медицинских ограничениях конкретную замену следует сверить с допустимым перечнем продуктов.';
        if (f.lowAppetite)
            return 'При сниженном аппетите изменение лучше распределить между небольшими порциями, не увеличивая объём пищи резко.';
        if (f.older && id.indexOf('protein') === 0)
            return 'В старшей возрастной группе белковые продукты предпочтительно распределять между основными приёмами пищи.';
        if (f.child)
            return 'Для ребёнка объём следует соотнести с возрастной порцией и общей достаточностью рациона.';
        if (energy.goal === 'lose')
            return 'При цели снижения массы предпочтительна равноценная замена без увеличения общей калорийности.';
        if (energy.goal === 'gain' || energy.goal === 'rehab_gain')
            return 'При цели набора или восстановления продукт можно добавить, пока калорийность остаётся ниже расчётного диапазона.';
        return '';
    }
    function operationBadge(text, energy) {
        text = String(text || '');
        if (energy && energy.mode === 'cautious')
            return 'Нужна индивидуальная проверка';
        if (/^(?:Не увеличивайте|Не вводите|Сверьте)(?=\s|[.,;:!?—-]|$)/.test(text))
            return 'Нужна индивидуальная проверка';
        if (/^(?:Уменьшите|Сократите|Снизьте)(?=\s|[.,;:!?—-]|$)/.test(text))
            return 'Выбрано сокращение';
        if (/^(?:Замените|Перераспределите|Не увеличивая)(?=\s|[.,;:!?—-]|$)/.test(text))
            return 'Выбрана замена';
        if (/^(?:Добавьте|Увеличьте)(?=\s|[.,;:!?—-]|$)/.test(text))
            return 'Выбрано добавление';
    ...
    function personalizedPractical(action, rows, plan, profile, energy) {
        ...
        if (action.id.indexOf('protein') === 0 && (f.renal || f.medical))
            return 'Не увеличивайте белковую часть автоматически. Сверьте допустимый источник и количество белка с индивидуальными назначениями, затем пересчитайте рацион.';
        ...
        if (correction && source) {
            ...
            if (energy.mode === 'cautious')
                return 'Не вводите жёсткое ограничение автоматически. Возможную замену продукта «' + shortName(source.name) + '» согласуйте с индивидуальным планом и переносимостью.';
            if (action.id === 'grains' || action.id === 'refined')
                return 'Замените часть продукта «' + shortName(source.name) + '» на ' + target.label + '. ' + amountSentence(target);
            if (action.id === 'protein-quality')
                return 'Не увеличивая общий объём белковой пищи, замените часть продукта «' + shortName(source.name) + '» на ' + target.label + '.';
            if (action.id === 'fruit-whole')
                return 'Замените переработанную фруктовую позицию «' + shortName(source.name) + '» фруктом целиком или ягодами.';
            if (action.id === 'fats' || action.id === 'sat-fat' || action.id === 'fat-ratio')
                return 'Замените часть продукта «' + shortName(source.name) + '» на ' + target.label + '. ' + amountSentence(target);
        ...
        if (energy.mode === 'add') {
            if (existing)
                return 'Увеличьте количество уже используемого продукта «' + shortName(existing.name) + '» либо добавьте ' + target.label + '. ' + amountSentence(target);
            return 'Добавьте ' + target.label + '. ' + amountSentence(target);
        }
        if (energy.mode === 'cautious')
            return 'Не вводите автоматическое жёсткое ограничение. Подберите ' + target.label + ' с учётом переносимости и индивидуального плана.';
        var donor = (donorRows(rows, action)[0] || {}).row;
        if (donor)
            return 'Замените часть продукта «' + shortName(donor.name) + '» на ' + target.label + ', не увеличивая общую калорийность. ' + amountSentence(target);
        if (existing)
            return 'Перераспределите рацион в пользу продукта «' + shortName(existing.name) + '», сохраняя общую калорийность.';
    ...
    function profileChips(profile, energy) {
        ...
        if (profile.ageGroup && profile.ageGroup.label)
            out.push(profile.ageGroup.label);
        if (energy && energy.applied)
            out.push(energy.label);
        if (energy && energy.guardrail && energy.guardrail !== 'none')
            out.push('Защитный сценарий: ' + String(energy.guardrail).replace(/_/g, ' '));
        return out.slice(0, 6);
    }
    function personalizeAction(action, rows, plan, profile, energy) {
        var out = {};
        Object.keys(action || {}).forEach(function (k) { out[k] = action[k]; });
        ...
        if (source) {
            if (['sodium', 'sugar', 'fats', 'sat-fat', 'fat-ratio'].indexOf(out.id) >= 0)
                out.headline = 'Начните с продукта «' + shortName(source.name) + '»';
            else if (out.id === 'grains' || out.id === 'refined')
                out.headline = 'Замените часть продукта «' + shortName(source.name) + '» цельнозерновым вариантом';
            else if (out.id === 'protein-quality')
                out.headline = 'Пересмотрите источник белка — «' + shortName(source.name) + '»';
        }
        if (target && energy.mode === 'add' && ['sodium', 'sugar', 'fats', 'sat-fat', 'fat-ratio', 'grains', 'refined', 'protein-quality', 'fruit-whole'].indexOf(out.id) < 0)
            out.headline = 'Увеличьте долю продукта «' + shortName(target.name) + '»';
        if (out.id.indexOf('protein') === 0 && (flags.renal || flags.medical))
            out.headline = 'Сверьте допустимый источник и количество белка';
        else if (energy.mode === 'cautious')
            out.headline = 'Сверьте изменение рациона с индивидуальным планом';
        else if (out.id === 'dairy' && flags.noDairy)
            out.headline = out.operationBadge === 'Выбрано добавление' ? 'Добавьте обогащённую растительную альтернативу' : 'Замените часть рациона обогащённой растительной альтернативой';
        return out;
    }
    function personalizePlan(plan, model, options) {
        options = options || {};
        var rows = options.rows || [], profile = options.profile || null, meta = options.needsMeta || null;
        ...
        var byKey = {};
        (plan.allActions || []).forEach(function (a) { var pa = personalizeAction(a, rows, plan, profile, energy); (a.keys || []).forEach(function (k) { if (!byKey[k])
            byKey[k] = pa; }); });
    ...
    function applyActionCard(card, action) {
        ...
        if (title)
            setTextStable(title, action.headline);
        var reason = card.querySelector('.hei-p1-reason');
        if (reason)
            setTextStable(reason, action.reason);
        var step = card.querySelector('.hei-p1-step strong');
        if (step)
            setTextStable(step, action.personalPractical);
        var old = card.querySelector('.hei-p2-grounding');
        if (old)
            old.remove();
        var box = document.createElement('div');
        box.className = 'hei-p2-grounding';
        box.innerHTML = '<p><span>Что формирует показатель</span>' + esc(action.sourceText) + '</p><div class="hei-p2-badges"><b>' + esc(action.operationBadge) + '</b>' + (action.profileNote ? '<em>' + esc(action.profileNote) + '</em>' : '') + '</div>' + (action.reliabilityNote ? '<p class="hei-p2-reliability">' + esc(action.reliabilityNote) + '</p>' : '');
        var calc = card.querySelector('.hei-p1-calculation');
        card.insertBefore(box, calc || null);
    }
    function applyComponentCard(details, action) {
        if (!details || !action)
            return;
        var body = details.querySelector('.hei-p1-component-body');
        if (!body)
            return;
        var p = body.querySelector(':scope > p');
        if (p)
            setTextStable(p, action.personalPractical);
        var old = body.querySelector('.hei-p2-component-context');
        if (old)
            old.remove();
        var ctx = document.createElement('div');
        ctx.className = 'hei-p2-component-context';
        ctx.innerHTML = '<span>Что формирует показатель</span><p>' + esc(action.sourceText) + '</p>' + (action.profileNote ? '<em>' + esc(action.profileNote) + '</em>' : '') + (action.reliabilityNote ? '<small>' + esc(action.reliabilityNote) + '</small>' : '');
    ...
    function applyTable(plan) {
        ...
        Object.keys(plan.byKey || {}).forEach(function (key) {
            ...
            if (!row || row.children.length < 4)
                return;
            var action = plan.byKey[key], cell = row.children[3];
            var line = cell.querySelector('.hei-p2-table-personal');
            if (!line) {
                line = document.createElement('span');
                line.className = 'hei-p1-table-line hei-p2-table-personal';
    ...
    function renderPersonalized(plan) {
        ...
        if (signature && signature === window.__lastHEIRecommendationPersonalizedSignature && levers && levers.querySelector('.hei-p2-grounding')) {
            window.__lastHEIRecommendationPlanPersonalized = plan;
            return;
        }
        window.__lastHEIRecommendationPersonalizedSignature = signature;
        if (levers) {
            var cards = levers.querySelectorAll('.hei-p1-action');
            for (var i = 0; i < cards.length; i++)
                applyActionCard(cards[i], plan.actions[i]);
            var heading = levers.querySelector('.hei-p1-heading p');
            if (heading)
                heading.textContent = 'Рекомендации привязаны к продуктам текущего рациона. Калькулятор выбирает между добавлением, заменой и сокращением с учётом калорийности и заданного профиля потребностей.';
            var head = levers.querySelector('.hei-p1-heading');
            if (head) {
                var old = head.querySelector('.hei-p2-profile-context');
                if (old)
                    old.remove();
                if (plan.profileChips.length) {
                    var chips = document.createElement('div');
                    chips.className = 'hei-p2-profile-context';
                    chips.innerHTML = '<span>Что учтено</span><div>' + plan.profileChips.map(function (x) { return '<b>' + esc(x) + '</b>'; }).join('') + '</div>';
...
.hei-p2-grounding{margin:10px 0 0;padding:10px 11px;border:1px solid #dbe7ef;border-radius:10px;background:#fff}.hei-p2-grounding p{margin:0;color:#40566b;font-size:12px;line-height:1.48}.hei-p2-grounding p>span,.hei-p2-component-context>span{display:block;margin-bottom:3px;color:#356b94;font-size:10px;font-weight:850;letter-spacing:.06em;text-transform:uppercase}.hei-p2-badges{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.hei-p2-badges b,.hei-p2-badges em{border-radius:999px;padding:5px 8px;font-size:10px;line-height:1.25;font-style:normal}.hei-p2-badges b{background:#edf5fb;color:#255f87}.hei-p2-badges em{background:#f3f8f5;color:#35644f}.hei-p2-profile-context{flex:1 0 100%;border-top:1px solid #e3ebf2;padding-top:10px}.hei-p2-profile-context>span{display:block;color:#6a7c8d;font-size:10px;font-weight:800;text-transform:none;letter-spacing:.01em}.hei-p2-profile-context>div{display:flex;flex-wrap:wrap;gap:6px;margin-top:6px}.hei-p2-profile-context b{border:1px solid #dbe6ef;border-radius:999px;background:#f8fbfd;color:#3b566d;padding:5px 8px;font-size:10px}.hei-p2-component-context{margin:0 0 9px;padding:9px 10px;border-radius:9px;background:#f7fafc}.hei-p2-component-context p{margin:0;color:#40566b;font-size:12px;line-height:1.45}.hei-p2-component-context em{display:block;margin-top:6px;color:#3d6754;font-size:11px;font-style:normal}.hei-p2-reliability,.hei-p2-component-context small{display:block;margin:7px 0 0;color:#6a5b40;font-size:10px;line-height:1.4}.hei-p2-table-personal{margin-top:6px;padding-top:6px;border-top:1px dashed #dce5ec}@media(max-width:900px){.hei-p2-profile-context{margin-top:10px}.hei-p2-badges{display:grid}.hei-p2-grounding{padding:9px}.hei-p2-profile-context>div{gap:5px}}@media print{.hei-p2-grounding,.hei-p2-component-context{break-inside:avoid}}';
    ...
    function refresh() {
        if (busy)
            return;
        var p1 = window.HEIRecommendationPass1, base = window.__lastHEIRecommendationPlan, model = window.__lastHEIModel;
        if (!p1 || !base || !model || model.valid === false)
            return;
        busy = true;
        try {
            var raw = rawRation(), flat = flattenRation(raw), rows = collectRationRows(flat, window.DB, raw);
            var plan = personalizePlan(base, model, { rows: rows, profile: currentProfile(), needsMeta: currentNeedsMeta() });
            renderPersonalized(plan);
        }
        finally {
            busy = false;
        }
