...
(function () {
    ...
    function heiData() {
        ...
        var total = NaN;
        if (model && Number.isFinite(Number(model.total)))
            total = Number(model.total);
        if (!Number.isFinite(total)) {
            var t = $('heiTotalCell');
            if (t)
                total = Number(String(t.textContent || '').replace(',', '.').replace(/[^0-9.+-]/g, ''));
        }
        if (!rows.length) {
            ...
            if (tbody) {
                rows = Array.prototype.slice.call(tbody.querySelectorAll('tr')).map(function (row) {
                    ...
                    if (!Number.isFinite(score) && c[4])
                        score = Number(String(c[4].textContent || '').replace(',', '.').replace(/[^0-9.+-]/g, ''));
    ...
    function capScore(base, facts) {
        var cap = 100;
        if (facts.vegFruit > 70 || facts.vegFruit < 35)
            cap = Math.min(cap, 60);
        if (facts.wholeGrains < 10)
            cap = Math.min(cap, 55);
        if (facts.protein < 12)
            cap = Math.min(cap, 55);
        if (facts.maxShare > 65)
            cap = Math.min(cap, 55);
        if (facts.vegFruit > 70 && facts.wholeGrains < 10 && facts.protein < 12)
            cap = Math.min(cap, 40);
        if (facts.maxShare > 75)
            cap = Math.min(cap, 38);
        if (facts.vegFruit > 80 && (facts.wholeGrains < 10 || facts.protein < 12))
            cap = Math.min(cap, 42);
    ...
    function plateStructure(model) {
        ...
        var baseScore = componentScore(vegFruit, 50, 40, 50) +
        ...
        var issues = [];
        if (vegFruit > 60)
            issues.push('овощи и фрукты занимают слишком большую долю основной тарелки');
        if (vegFruit < 45)
            issues.push('овощей и фруктов меньше половины основной тарелки');
        if (wholeGrains < 18)
            issues.push('цельных злаков мало относительно ориентира');
        if (wholeGrains > 32)
            issues.push('цельных злаков больше ориентира');
        if (protein < 18)
            issues.push('белковая часть ниже ориентира');
        if (protein > 32)
            issues.push('белковая часть выше ориентира');
        if (fruits > vegetables)
            issues.push('фруктовая часть больше овощной; в модели тарелки овощи обычно преобладают');
        if (!issues.length)
            issues.push('основные сектора близки к рабочему ориентиру');
        var actions = [];
        if (protein < 18)
            actions.push('добавить белковую часть к одному приёму пищи');
        if (wholeGrains < 18)
            actions.push('добавить порцию цельных злаков');
        if (vegFruit > 60)
            actions.push('сейчас важнее выровнять остальные сектора');
        if (vegFruit < 45)
            actions.push('добавить овощную часть к одному-двум приёмам пищи');
        if (fruits > vegetables)
            actions.push('следующую растительную порцию сделать овощной');
        if (!actions.length)
            actions.push('сохранять структуру и проверять качество продуктов по HEI');
        return {
            ready: true,
            status: status,
    ...
    function heiStatus(hei) {
        ...
        if (!Number.isFinite(total))
            return { status: 'unknown', text: 'HEI ещё не рассчитан', total: NaN };
        if (total >= 80)
            return { status: 'good', text: 'HEI высокий', total: total };
        if (total >= 70)
            return { status: 'good', text: 'HEI хороший', total: total };
        if (total >= 50)
            return { status: 'medium', text: 'HEI средний', total: total };
    ...
    function mainConclusion(plate) {
        if (!plate.ready)
            return plate.reason || 'Добавьте продукты, чтобы оценить структуру тарелки.';
        if (plate.status === 'ok')
            return 'Основная тарелка близка к модели: растительная половина, цельные злаки и белковая часть представлены сбалансированно.';
        ...
        if (plate.protein < 18 && plate.wholeGrains < 18)
            return 'Белковая часть и цельные злаки ниже ориентира для основной тарелки.';
        if (plate.vegFruit > 60)
            return 'Растительная часть слишком велика относительно белковой части и цельных злаков.';
        if (plate.vegFruit < 45)
            return 'Овощей и фруктов меньше половины основной тарелки.';
        if (plate.protein < 18)
            return 'Белковая часть ниже ориентира основной тарелки.';
        if (plate.wholeGrains < 18)
            return 'Цельных злаков мало относительно ориентира основной тарелки.';
        return plate.issues[0] || 'Структуру тарелки стоит немного выровнять.';
    }
    function bridgeText(plate, hei) {
        var hs = heiStatus(hei);
        if (!plate.ready)
            return 'HEI оценивает качество компонентов, а Гарвардская тарелка начнёт оценивать структуру после добавления продуктов.';
        if (hs.status === 'unknown')
            return 'Гарвардская тарелка уже показывает структуру основной части рациона. HEI появится после расчёта качества компонентов.';
        var goodHEI = hs.status === 'good';
        var goodPlate = plate.status === 'ok';
        if (goodHEI && goodPlate)
            return 'HEI и Гарвардская тарелка согласуются: рацион выглядит качественным и структурно близким к модели тарелки.';
        if (goodHEI && !goodPlate)
            return 'HEI может быть хорошим по компонентам, но тарелка показывает перекос структуры: отдельные продукты качественные, а распределение основной тарелки стоит выровнять.';
        if (!goodHEI && goodPlate)
            return 'Тарелка выглядит близко к ориентиру, но HEI снижается качеством компонентов: проверьте натрий, добавленный сахар, насыщенные жиры, рафинированные злаки и слабые группы HEI.';
    ...
    function applyOpenState(root, state) {
        if (!root || !root.querySelectorAll || !state)
            return;
        var list = root.querySelectorAll('details');
        for (var i = 0; i < list.length; i++) {
            var d = list[i];
    ...
    function render() {
        ...
        stableSetHtml(box, '<details class="hhb-compact">' +
