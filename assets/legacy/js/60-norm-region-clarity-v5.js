...
(function () {
    ...
    function currentRegion() {
        try {
            if (window.State && typeof window.State.getRegion === 'function')
                return window.State.getRegion() === 'eu' ? 'eu' : 'us';
        }
        catch (_) { }
    ...
    function sync() {
        var seg = byId('regionSeg'), status = byId('normRegionStatus');
        if (!seg || !status)
            return;
        var region = currentRegion(), p = PROFILE[region];
        seg.dataset.activeRegion = region;
        seg.querySelectorAll('button[data-region]').forEach(function (btn) {
            var active = btn.dataset.region === region;
            btn.classList.toggle('active', active);
            btn.setAttribute('aria-pressed', String(active));
            var bp = PROFILE[btn.dataset.region] || PROFILE.us;
            btn.setAttribute('aria-label', (active ? 'Применяются: ' : 'Выбрать: ') + bp.aria);
        });
        status.innerHTML = '<span>Применяются нормы:</span> <strong>' + p.label + '</strong> <small>' + p.short + '</small>';
        status.dataset.region = region;
        var switcher = byId('normRegionSwitcher');
        if (switcher)
            switcher.setAttribute('aria-label', 'Сейчас применяются нормы микронутриентов: ' + p.aria);
    ...
    function formatSafety(res, unit) {
        if (res && res.ul && Number.isFinite(Number(res.ul.value)))
            return num(res.ul.value) + ' ' + unit + ' · UL' + (res.ul.scope ? ' · ' + scopeRu(res.ul.scope) : '');
        if (res && res.safe && Number.isFinite(Number(res.safe.value)))
            return num(res.safe.value) + ' ' + unit + ' · ' + (res.safe.label || 'безопасный уровень') + (res.safe.scope ? ' · ' + scopeRu(res.safe.scope) : '');
    ...
    function renderComparison() {
        ...
        if (typeof resolver !== 'function' || !refBody || !safeBody)
            return false;
        ...
        if (profile)
            profile.textContent = p.child ? 'Взрослый пример для ' + sexRu : sexRu + ', ' + (Number.isFinite(p.shownAge) ? Math.round(p.shownAge) + ' лет' : '19–50 лет');
        if (note) {
        ...
        if (regStatus) {
            ...
            regStatus.textContent = audit && audit.ok ? 'Нормативный реестр ' + String(reg.version) + ' · проверен ' + String(reg.reviewedAt) + ' · источников: ' + String(audit.sourceCount) : 'Нормативный реестр не загружен: числовые ориентиры должны считаться недоступными.';
    ...
    function closeHelp() { var d = byId('normRegionHelp'); if (!d)
        return; try {
    ...
    function wrapState() {
        if (!window.State || typeof window.State.setRegion !== 'function' || window.State.__normRegionClarityWrapped)
            return false;
        var original = window.State.setRegion.bind(window.State);
        window.State.setRegion = function (region) { var out = original(region); sync(); renderComparison(); try {
            window.dispatchEvent(new CustomEvent('norm:region-changed', { detail: { region: currentRegion(), version: VERSION } }));
        }
        catch (_) { } return out; };
        window.State.__normRegionClarityWrapped = true;
        return true;
    }
    function init() {
        sync();
        wrapState();
        renderComparison();
        var help = byId('normRegionHelpBtn');
        if (help && !help.__normHelpBound) {
            help.__normHelpBound = true;
            help.addEventListener('click', openHelp);
        }
        document.querySelectorAll('.norm-region-help-close').forEach(function (btn) { if (btn.__normHelpBound)
            return; btn.__normHelpBound = true; btn.addEventListener('click', closeHelp); });
        var dialog = byId('normRegionHelp');
        if (dialog && !dialog.__normBackdropBound) {
            dialog.__normBackdropBound = true;
            dialog.addEventListener('click', function (e) { if (e.target !== dialog)
                return; var r = dialog.getBoundingClientRect(); if (e.clientX < r.left || e.clientX > r.right || e.clientY < r.top || e.clientY > r.bottom)
                closeHelp(); });
        }
        document.addEventListener('click', function (e) { var btn = e.target && e.target.closest && e.target.closest('#regionSeg button[data-region]'); if (btn)
            setTimeout(function () { sync(); renderComparison(); }, 0); });
        ['needs_age', 'needs_sex'].forEach(function (id) { var el = byId(id); if (el && !el.__normComparisonBound) {
            el.__normComparisonBound = true;
            el.addEventListener('input', renderComparison);
        ...
        var tries = 0, timer = setInterval(function () { tries++; var wrapped = wrapState(), rendered = renderComparison(); if (wrapped && rendered) {
            sync();
            clearInterval(timer);
        }
        else if (tries > 120)
