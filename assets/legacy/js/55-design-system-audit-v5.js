(function () {
    ...
    function brand() {
        ...
        if (title && !title.querySelector('.app-subtitle')) {
            ...
            sub.textContent = 'Рацион · HEI · дневные нормы';
            title.appendChild(sub);
        }
        var badge = title && title.querySelector('.version-badge, .badge, span:not(.app-subtitle)');
        if (badge && /v?\d+\.\d+\.\d+/.test(badge.textContent || '')) {
            if (badge.textContent !== deploy) badge.textContent = deploy;
    ...
    function schedule() { if (queued)
        return; queued = true; setTimeout(function () { queued = false; sync(); }, 45); }
