...
window.HEI = (() => {
    ...
    const lerp = (x0, x1, v, y0, y1) => { if (x0 === x1)
        return v >= x1 ? y1 : y0; const t = clamp((v - x0) / (x1 - x0), 0, 1); return y0 + (y1 - y0) * t; };
    ...
    const isVeg = p => (p && p.category === 'Vegetables') || (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'vegetables' || hasTag(p, 'vegetables');
    ...
    const isFruit = p => (p && p.category === 'Fruits') || (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'fruits_berries' || hasTag(p, 'fruits');
    const isDriedFruit = p => isFruit(p) && (normState(p) === 'dry' || normState(p) === 'dried' || hasAnyTag(p, ['dried_fruit', 'dried', 'dry']));
    const isDairy = p => (p === null || p === void 0 ? void 0 : p.category) === 'Dairy' ||
        (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'dairy' ||
        hasAnyTag(p, ['dairy', 'cheese', 'milk', 'yogurt', 'kefir', 'curd']);
    const isDairyCheese = p => isDairy(p) && (hasTag(p, 'cheese') ||
        /\bcheese\b/i.test((p === null || p === void 0 ? void 0 : p.name) || '') ||
        /(^|[\s(«"'])сыр($|[\s),.:;»"'-])/iu.test((p === null || p === void 0 ? void 0 : p.name_ru) || ''));
    const isDairyMilkYog = p => isDairy(p) &&
        !isDairyCheese(p) &&
    ...
    const isSaturatedFatRouteProduct = p => {
        ...
        if (catalog === 'saturated_fats' || fatQuality === 'saturated_fat_source' || limitGroup === 'saturated_fats')
            return true;
        if (hasAnyTag(p, ['saturated_fat_source', 'limit_saturated_fat']))
            return true;
        if (/морожен|ice\s*cream|чипс|chips|соус|sauce|nut\s*butter|seed\s*butter|peanut\s*butter|almond\s*butter|орехов[а-я\s]*паст|арахисов[а-я\s]*паст|миндальн[а-я\s]*паст|паста\s+из\s+тыквен/i.test(text))
            return false;
    ...
    const isWholeGrain = p => hasTag(p, 'whole_grains') || (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'whole_grains';
    const isRefinedGrain = p => hasTag(p, 'refined_grains') || (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'refined_grains';
    const isEgg = p => hasAnyTag(p, ['egg', 'eggs']) || /яйц|egg/.test(textOf(p));
    const isSeafood = p => hasAnyTag(p, ['seafood', 'fish', 'shellfish']) ||
    ...
    const isSoyProduct = p => hasAnyTag(p, ['soy', 'tofu', 'tempeh', 'natto']) ||
        /соя|соев|тофу|темпе|натто|soy|tofu|tempeh|natto/.test(textOf(p));
    const isNutsSeeds = p => hasAnyTag(p, ['nuts', 'seeds', 'nuts_seeds']) ||
    ...
    const isLegume = p => {
        ...
        if (isSoyProduct(p) && !hasAnyTag(p, ['legumes', 'beans', 'peas', 'lentils', 'chickpeas']))
            return false;
    ...
    const isProteinFood = p => ((p === null || p === void 0 ? void 0 : p.hei_category_key) === 'protein_foods' ||
        (p === null || p === void 0 ? void 0 : p.hei_category_key) === 'seafood_plant_protein' ||
        (p === null || p === void 0 ? void 0 : p.category) === 'Meat' ||
        (p === null || p === void 0 ? void 0 : p.category) === 'Seafood & Plant Protein' ||
        hasAnyTag(p, [
    ...
    const isSeafoodPlantProtein = p => ((p === null || p === void 0 ? void 0 : p.hei_category_key) === 'seafood_plant_protein' ||
    ...
    const grainOzEqGrams = p => {
        ...
        if (st === 'dry' || st === 'dried' || hasAnyTag(p, ['dry', 'flour', 'cereal_dry']))
            return HEI_MAP.oz_eq['grains.dry'] || 28.35;
        if (st === 'baked' || hasAnyTag(p, ['bread', 'crispbread', 'bakery', 'cracker']))
            return HEI_MAP.oz_eq['grains.dry'] || 28.35;
    ...
    const proteinOzEqGrams = p => {
        if (isEgg(p))
            return 50;
        if (isLegume(p))
            return HEI_MAP.oz_eq['protein.legume_cooked'] || 43;
        if (isNutsSeeds(p))
            return HEI_MAP.oz_eq['protein.nuts_seeds'] || 14.0;
        return HEI_MAP.oz_eq['protein.meat_fish_eggs'] || 28.35;
    };
    const dairyCupEqGrams = p => {
        if (!isDairy(p) || isDairyExcludedFromHEI(p))
            return null;
        if (isDairyCheese(p))
            return HEI_MAP.cup_eq['dairy.cheese'] || 42;
        if (hasTag(p, 'ice_cream'))
            return 134; // Approximation until product-level FPED equivalents are added.
        if (hasTag(p, 'curd') || /творог|ricotta|рикотт/i.test(textOf(p)))
            return HEI_MAP.cup_eq['dairy.milk_yogurt'] || 244; // Approximation.
        return HEI_MAP.cup_eq['dairy.milk_yogurt'] || 244;
    };
    if (typeof window !== 'undefined') {
        window.__HEIClassifiers = {
            normalizeTag, tagsOf, hasTag, hasAnyTag, textOf,
            isDairy, isDairyCheese, isDairyMilkYog, isFruit, isVeg, isDriedFruit,
    ...
    function computeIntake(ration, DB) {
      ...
      return raw !== null && raw !== '' && typeof raw !== 'boolean'
        ...
        for (const { key, grams } of (ration || [])) {
            ...
            if (vegPer100 !== null) {
                if (legumeMatched)
                    acc.legumes_veg_total += g * vegPer100 / 100;
                else
                    acc.vegetables_total += g * vegPer100 / 100;
                vegUsed = true;
            }
            const greensPer100 = eq(p, 'greens_beans_cup_eq_per_100g');
            if (greensPer100 !== null) {
                if (legumeMatched)
                    acc.legumes_greens_beans += g * greensPer100 / 100;
                else
                    acc.greens_beans += g * greensPer100 / 100;
                greensUsed = true;
            }
            ...
            if (proteinPer100 !== null) {
                ...
                if (legumeMatched)
                    acc.legumes_as_protein += oz;
                else
                    acc.protein_excl_legumes += oz;
            }
            ...
            if (!vegUsed && vegetableMatched)
                acc.vegetables_total += g / vegetableCupEqGrams(p);
            if (!greensUsed && greensMatched)
                acc.greens_beans += g / greensBeansCupEqGrams(p);
            if (!vegUsed && legumeMatched && !vegetableMatched)
                acc.legumes_veg_total += g / legumeCupEqGrams(p);
            if (!greensUsed && legumeMatched && !greensMatched)
                acc.legumes_greens_beans += g / legumeCupEqGrams(p);
            if (!fruitUsed && isFruit(p))
                acc.fruits_total += g / fruitCupEqGrams(p);
            if (!wholeFruitUsed && isFruit(p) && isWholeFruit(p))
                acc.fruits_whole += g / fruitCupEqGrams(p);
            if (!dairyUsed) {
                const dairyEq = dairyCupEqGrams(p);
                if (dairyEq)
                    acc.dairy += g / dairyEq;
            }
            if (!wholeGrainUsed && isWholeGrain(p))
                acc.grains_whole += g / grainOzEqGrams(p);
            if (!refinedGrainUsed && isRefinedGrain(p))
                acc.grains_refined += g / grainOzEqGrams(p);
            ...
            if (!seafoodPlantUsed && isSeafoodPlantProtein(p))
                acc.seafood_plant += g / proteinOzEqGrams(p);
        }
        return acc;
    }
    function scoreFromDensity(density, intakeEnergyKcal, energyRefKcal) {
        ...
        if (!Number.isFinite(intakeEnergy) || intakeEnergy <= 0) {
            ...
            return { density, points: P, total: 0, energyRefKcal, intake: { energy_kcal: 0, sodium_mg: 0, fatty_sfa_g: 0, fatty_unsat_g: 0, added_sugars_g: 0 }, valid: false, invalidReason: 'empty_ration' };
        }
        const P = {};
        const adeq = n => P[n] = lerp(0, STD[n].min, density[n], 0, STD[n].pts);
        adeq('fruits_total');
        adeq('fruits_whole');
        adeq('vegetables_total');
        adeq('greens_beans');
        adeq('grains_whole');
        adeq('dairy');
        adeq('protein_total');
        ...
        return { density, points: P, total, energyRefKcal, intake, valid: true };
    }
    function score(intake, energyRefKcal) {
        const eRef = Math.max(1, energyRefKcal);
    ...
    const PRAISE = {
        ...
        sodium_g: 'Натрий удерживается в здоровых рамках.',
        added_sugars_pct: 'Добавленный сахар под контролем — это хорошо для метаболического здоровья.',
        sat_fats_pct: 'Насыщённые жиры в норме — держите баланс в пользу масел, рыбы и орехов.',
    ...
    function buildRows(model) {
        ...
        pushAdeq('fruits_total');
        pushAdeq('fruits_whole');
        pushAdeq('vegetables_total');
        pushAdeq('greens_beans');
        pushAdeq('grains_whole');
        pushAdeq('dairy');
        pushAdeq('protein_total');
        pushAdeq('seafood_plant');
        // ratio
        ...
        const _gr_now = 85 * (eRef / 1000) * d.grains_refined;
        const _gr_max = 85 * (eRef / 1000) * STD.grains_refined.max;
        ...
        const _na_now = d.sodium_g * eRef / 1000;
        const _na_max = STD.sodium_g.max * eRef / 1000;
        ...
        const _sug_energy = Math.max(1, model.intake.energy_kcal || eRef);
        const _sug_now = d.added_sugars_pct * _sug_energy / 100 / 4;
        const _sug_max = STD.added_sugars_pct.max * _sug_energy / 100 / 4;
        ...
        const _sfa_energy = Math.max(1, model.intake.energy_kcal || eRef);
        const _sfa_now = d.sat_fats_pct * _sfa_energy / 100 / 9;
        const _sfa_max = STD.sat_fats_pct.max * _sfa_energy / 100 / 9;
    ...
    function buildRecommendations(model) {
        ...
        const parts = [];
        for (const r of rows) {
            const maxPts = ((_a = STD[r.key]) === null || _a === void 0 ? void 0 : _a.pts) || 10;
            const pts = parseFloat(r.pts) || 0;
            let sev = 'ok';
            if (pts <= 0.01)
                sev = 'severe';
            else if (pts < maxPts * 0.6)
                sev = 'mild';
            let grams = 0;
            try {
            ...
            const rec = (_b = RECS[r.key]) === null || _b === void 0 ? void 0 : _b[sev];
            ...
            if (Array.isArray(pb.actions)) {
                const items = pb.actions.map(a => {
                    if (a.type === 'add')
                        return `<li>➕ ${a.label} <span class=\"micro\">${[a.portion || '', (a.meal ? (MEAL_RU[a.meal] || a.meal) : '')].filter(Boolean).join(', ')}</span></li>`;
                    if (a.type === 'reduce')
                        return `<li><span class=\"micro\">что можно сократить в этой категории:</span> ${a.label}</li>`;
                    if (a.type === 'reduceNa')
                        return `<li><span class=\"micro\">что можно сократить в этой категории:</span> ${a.label} <span class=\"micro\">−${a.mg} мг натрия</span></li>`;
                    return '';
                }).join('');
                if (items)
                    plays.push(`<ul class="play">${items}</ul>`);
            }
    ...
    function rowClassByPts(key, pts) {
        ...
        const max = ((_a = STD[key]) === null || _a === void 0 ? void 0 : _a.pts) || 10;
    ...
    function humanLeverText(l) {
        const key = l && l.key;
        if (key === 'dairy')
            return `1 молочная порция (200–250 мл) — <span class="micro">${STD[key].label}</span>`;
        if (key === 'grains_whole')
            return `80–100 г готовой крупы/каши — <span class="micro">${STD[key].label}</span>`;
        if (key === 'fruits_total' || key === 'fruits_whole')
            return `≈150 г фруктов — <span class="micro">${STD[key].label}</span>`;
        if (key === 'vegetables_total')
            return `≈130–150 г овощей — <span class="micro">${STD[key].label}</span>`;
        if (key === 'greens_beans')
            return `зелень/бобовые к приёму пищи — <span class="micro">${STD[key].label}</span>`;
        if (key === 'protein_total')
            return `порция белкового блюда — <span class="micro">${STD[key].label}</span>`;
        if (key === 'seafood_plant')
            return `рыба/бобовые/тофу/орехи — <span class="micro">${STD[key].label}</span>`;
    ...
    function renderLevers(model, root) {
        ...
        if (!box || window.HEIRecommendationPass1)
            return;
    ...
    function render(model, root) {
        ...
        var coreSignature = '';
        try {
            coreSignature = JSON.stringify({
                valid: model && model.valid !== false,
                total: model && Number.isFinite(model.total) ? Number(model.total.toFixed(4)) : null,
                energyRefKcal: model && Number.isFinite(model.energyRefKcal) ? Number(model.energyRefKcal.toFixed(2)) : null,
                intakeEnergy: model && model.intake && Number.isFinite(model.intake.energy_kcal) ? Number(model.intake.energy_kcal.toFixed(2)) : null,
                rows: rows.map(function (r) { return [r.key, r.value, r.norm, r.delta, r.pts]; })
            });
        }
        catch (_) { coreSignature = ''; }
        ...
        if (tbody)
            tbody.innerHTML = rows.map(r => {
                var _a, _b, _c;
                return `
      <tr class="${rowClassByPts(r.key, r.pts)}" data-hei-key="${r.key}" data-hei-score="${parseFloat(r.pts) || 0}" data-hei-max="${((_a = STD[r.key]) === null || _a === void 0 ? void 0 : _a.pts) || 10}" title="${(((_b = STD[r.key]) === null || _b === void 0 ? void 0 : _b.tip) || '').replace(/"/g, '&quot;')}">
        <td>${((_c = STD[r.key]) === null || _c === void 0 ? void 0 : _c.label) || r.key}</td>
        <td>${r.value}</td>
        <td>${r.norm}</td>
        <td>${r.delta}
    ...
    `;
        ...
        if (model.valid === false) {
            if (tbody)
                tbody.innerHTML = '<tr><td colspan="5" class="muted">Добавьте продукты в рацион, чтобы рассчитать HEI.</td></tr>';
            if (totalCell)
                totalCell.textContent = '—';
            if (summary) {
                summary.className = 'alert neutral';
                summary.innerHTML = 'HEI не рассчитывается для пустого рациона. Добавьте хотя бы один продукт с ненулевой массой.';
            }
        ...
        if (totalCell)
            totalCell.textContent = fmt(model.total, 1);
        if (summary) {
            summary.className = 'alert ' + (model.total >= 90 ? 'good' : model.total >= 70 ? 'neutral' : 'bad');
            summary.innerHTML = `Итоговый балл HEI‑2020: <b>${fmt(model.total, 1)} / 100</b>. 
      Нормировка: <b>${fmt(model.energyRefKcal, 0)} ккал</b>. 
      Энергия рациона (для %): <b>${fmt(model.intake.energy_kcal, 0)} ккал</b>. 
      ${model.total >= 90 ? 'Отличный результат!' : model.total >= 70 ? 'Хороший уровень, есть что улучшить.' : 'Нужно улучшить баланс рациона.'}`;
        }
        // Топ‑рычаги
        ...
        (function () {
            ...
            const top = bad.slice(0, 3).map(r => { var _a; return `<li><strong>${((_a = STD[r.key]) === null || _a === void 0 ? void 0 : _a.label) || r.key}</strong> — ${r.html}</li>`; }).join('');
            const list = recs.map(r => {
                var _a;
                return `
        <div class="rec ${r.sev}">
          <div class="rec-head"><strong>${((_a = STD[r.key]) === null || _a === void 0 ? void 0 : _a.label) || r.key}</strong> <span class="tag ${r.sev}">${r.sev === 'ok' ? 'ОК' : r.sev === 'mild' ? 'Нужно улучшить' : 'Приоритет'}</span></div>
      ...
      `;
            ...
            if (recsWrap && !window.HEIRecommendationPass1)
                recsWrap.innerHTML = `
    ...
    function mount({ db, container, getRation, getEnergyRef, onPrint }) {
        ...
        const recompute = () => {
            var _a, _b;
            const energyModeActual = (_a = document.getElementById('heiEnergyActual')) === null || _a === void 0 ? void 0 : _a.checked;
            const energyModeManual = (_b = document.getElementById('heiManualRadio')) === null || _b === void 0 ? void 0 : _b.checked;
            if (badge)
                badge.textContent = 'нормировка: ' + (energyModeActual ? 'ккал рациона' : (energyModeManual ? 'ручные ккал' : 'целевые ккал'));
        ...
        root.addEventListener('click', (e) => {
            ...
            if (w) {
                ...
                if (sim) {
                    ...
                    whatifHint.innerHTML = `Что если по компоненту <b>${STD[key].label}</b> ${w.textContent.trim()}? 
            Итоговый HEI станет <b>${fmt(sim.total, 1)}</b> (${delta >= 0 ? '+' : ''}${fmt(delta, 1)}).`;
                }
            }
        });
        return { recompute };
    ...
    return { mount, computeIntake, score, scoreFromDensity, helpers: { STD, HEI_MAP } };
})();
;
