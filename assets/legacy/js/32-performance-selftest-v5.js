...
(function () {
    ...
    function fmtBytes(n) { n = Number(n) || 0; if (n > 1048576)
        return (n / 1048576).toFixed(2) + ' МБ'; if (n > 1024)
        return (n / 1024).toFixed(1) + ' КБ'; return n + ' Б'; }
    ...
    function render() {
        ...
        try {
            console.info('[v5.2.23 perf]', data);
        }
        catch (_) { }
    }
    window.NutritionPerformanceV5 = { version: VERSION, collect: collect, render: render };
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', function () { setTimeout(render, 1000); }, { once: true });
    else
