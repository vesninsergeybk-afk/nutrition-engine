...
(function () {
    ...
    function productsArray() {
        try {
            if (window.DB && Array.isArray(window.DB.items))
                return window.DB.items;
            var db = byId('db');
    ...
    function primaryUnit(product) {
        ...
        if (!units.length && !product.serving_unit_hint)
            return null;
    ...
    function hintText(product) {
        if (!product)
            return '';
        if (product.serving_unit_hint)
            return String(product.serving_unit_hint).trim();
    ...
    function enhanceSearchCards() {
        ...
        cards.forEach(function (card) {
            ...
            if (!key || card.dataset.servingUnitsKey === key)
                return;
    ...
    function enhanceCatalogSelect(select) {
        ...
        if (old &&
            ...
            old.innerHTML === hint.innerHTML)
            return;
        if (old)
            old.remove();
        var inline = select.closest('.inline');
        if (inline && inline.parentNode)
            inline.parentNode.insertBefore(hint, inline.nextSibling);
        else
    ...
    function enhanceRationRows() {
        ...
        rows.forEach(function (row) {
            ...
            if (row.dataset.servingUnitsKey === key && old)
                return;
            if (old)
                old.remove();
    ...
    function init() {
        ...
        window.runSmokeTests = function () {
            ...
            try {
                ...
                ok = false;
                window.__v466PreviousSmokeError = String(e && e.message || e);
            }
