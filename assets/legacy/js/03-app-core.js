...
function childAgeNorm(key, age, sex) {
    ...
    if (!__childAgeNormCache) {
        try {
            __childAgeNormCache = JSON.parse((document.getElementById('age_rda_table') || {}).textContent || '[]');
        }
        catch (_) {
            __childAgeNormCache = [];
        }
...
function resolveAdultRegionalMicro(key, region, age, sex) {
    ...
    if (region === 'us') {
        ...
        if (usMin[key])
            min = { type: usMin[key][0], value: usMin[key][1] };
        ...
        if (usUl[key])
            ul = { value: usUl[key][0], scope: usUl[key][1] };
    }
    else {
        var euMin = {
        ...
        if (euMin[key])
            min = { type: euMin[key][0], value: euMin[key][1] };
        ...
        if (euUl[key])
            ul = { value: euUl[key][0], scope: euUl[key][1] };
        if (key === 'iron_mg')
            safe = { value: 40, scope: 'total', label: 'безопасный уровень' };
        if (key === 'manganese_mg')
            safe = { value: 8, scope: 'total', label: 'безопасный уровень' };
        if (key === 'zinc_mg')
            note = 'EFSA связывает PRI с фитатом; показано значение для низкофитатного рациона.';
        if (key === 'vitamin_b1_mg' || key === 'vitamin_b3_mg')
            note = 'EFSA задаёт энергозависимый ориентир; показано взрослое расчётное приближение.';
    }
...
function resolveRegionalMicroLimits(key, region, age, sex) {
    var child = childAgeNorm(key, age, sex);
    if (child)
        return child;
...
const State = (() => {
    ...
    function normalizeStoredRationEntry(entry) {
        if (!entry || typeof entry !== 'object')
            return null;
        if (entry.ai_import)
            entry.ai_import = normalizeStoredAiImport(entry.ai_import);
    ...
    function load() {
        ...
        try {
            region = SafeStorage.getItem(LS + '_region') || 'us';
        }
        catch (_b) {
            region = 'us';
        }
    ...
    function add(key, grams, importMeta) {
        ...
        if (decision.status === 'blocked' || !decision.key) {
            Logger.warn('PREPARATION_PROFILE_BLOCKED', { key: String(key || ''), context: 'State.add' });
            return null;
        }
        key = decision.key;
        const p = DB.byKey.get(key);
        if (!p)
            return null;
    ...
    load();
    const api = { add, beginAddBatch, endAddBatch, update, updateCompositeChild, removeCompositeChild, addCompositeChild, replaceCompositeChild, resetCompositeEntry, remove, clear, get, getForCalculation, getForFoodPattern, getForNutrients, setRegion, getRegion, getPreparationMigrationReport };
    window.State = api;
    return api;
})();
const Events = (() => {
...
function productSourceClassV53155(p) {
    ...
    if (/source_required|review_required|incomplete|needed|candidate|manual_review/.test(text))
        return 'low';
    if (/proxy|recipe|model|label/.test(text))
        return 'medium';
    if (/verified|exact|complete/.test(text))
        return 'high';
    return 'unknown';
}
