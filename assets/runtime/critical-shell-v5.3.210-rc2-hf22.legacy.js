/* HF22 critical shell bundle; generated, calculation-neutral. */
...
/* BEGIN assets/legacy/js/89-workspace-entry-ux-v5.3.210-rc2-hf21.js */
/* Nutrition Calculator v5.3.210 RC2 HF21
...
(function(w,d){
  ...
  var VERSION='v5.3.210-rc2-hf21-security-entry-ux';
  var initialized=false;
  var syncTimer=0;
  ...
  function createRationEntryMethods(){
    ...
    panel.innerHTML=''+
      '<div class="workspace-ration-entry-methods__copy"><span>Добавление продуктов</span><strong id="workspaceRationEntryMethodsTitle">Поиск или ИИ-распознавание</strong><small>ИИ может распознать продукты по фотографии, голосу или готовому аудиофайлу.</small><em>Камера, микрофон и выбор файла открываются только после отдельного нажатия в блоке ИИ. Калькулятор не запрашивает пароли и платёжные данные.</em></div>'+
  ...
  function handleEntryMethod(kind){
    ...
    /* Security hardening: never synthesize a click on a hidden file input or
       microphone control. The user confirms the sensitive browser action with
       a separate, visible click inside the AI block. */
    if(target){
      toggle(target,'workspace-entry-target',true);
      setAttr(target,'data-entry-guided','true');
      w.setTimeout(function(){toggle(target,'workspace-entry-target',false);try{target.removeAttribute('data-entry-guided');}catch(_){}},2200);
    }
  }
  ...
  function init(){
    ...
    w.NutritionWorkspaceEntryUXHF21={version:VERSION,refresh:sync,goToProfile:goToProfile,setMode:setMode,revealAiSection:revealAiSection};
...
;
/* END assets/legacy/js/89-workspace-entry-ux-v5.3.210-rc2-hf21.js */
