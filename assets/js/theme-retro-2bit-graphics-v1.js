...
(function(){
  ...
  function classify(el){
    ...
    if(/close|закрыть|✕|×/.test(all))return 'close';
    if(/справк|help|информац|подробнее/.test(all) || /(^|\s)[?i](\s|$)/.test(text))return 'info';
    if(/удал|очист|trash/.test(all))return 'remove';
    if(/сброс|reset|заново/.test(all))return 'reset';
    if(/печать|print/.test(all))return 'print';
    if(/pdf/.test(all))return 'pdf';
    if(/фото|камер|image/.test(all))return 'camera';
    if(/аудио|микроф|голос/.test(all))return 'audio';
    if(/загруз|импорт|upload|файл/.test(all))return 'upload';
    if(/gemini|ии-|искусственн|стратеги/.test(all))return 'ai';
    if(/поиск|найти|search/.test(all))return 'search';
    if(/добав|подтверд|применить|сохранить/.test(all))return 'add';
    if(/редакт|измен|настро/.test(all))return 'edit';
    if(/рассчит|сформир|вычисл/.test(all))return 'calculate';
    if(/рацион|прием пищи|приём пищи/.test(all))return 'ration';
    if(/hei|индекс качества/.test(all))return 'hei';
    return '';
  }
  ...
  function decorateButton(el){
    if(!active() || !el || el.nodeType!==1 || el.getAttribute('data-retro-decorated')==='1')return;
    var icon=classify(el);
    if(!icon)return;
    if(!el.hasAttribute('data-retro-icon')){el.setAttribute('data-retro-icon',icon);el.setAttribute('data-retro-owned-icon','1');}
    if(norm(el.textContent).length>2&&el.classList.contains('retro-icon-only')){el.classList.remove('retro-icon-only');el.removeAttribute('data-retro-owned-icon-only');}
    if(isIconOnly(el) && !el.classList.contains('retro-icon-only')){el.classList.add('retro-icon-only');el.setAttribute('data-retro-owned-icon-only','1');}
  ...
  function decorateHeading(section,icon){
    ...
    if(h && !h.hasAttribute('data-retro-section-icon')){h.setAttribute('data-retro-section-icon',icon);h.setAttribute('data-retro-owned-section-icon','1');}
  }
  function decorateStates(root){
    root.querySelectorAll&&root.querySelectorAll('.alert,[data-state],.search-status,.gemini-ai-status,.gemini-ration-import__status').forEach(function(el){
      if(el.hasAttribute('data-retro-state-glyph'))return;
  ...
  function scanSections(root){
    Object.keys(sectionMap).forEach(function(id){
      var section=null;
      if(root.nodeType===9)section=document.getElementById(id);
      else if(root.id===id)section=root;
      else if(root.querySelector)section=root.querySelector('#'+id);
      decorateHeading(section,sectionMap[id]);
    });
  }
  function scan(root){
    if(!active() || !root || (root.nodeType!==1 && root.nodeType!==9))return;
    if(root.matches&&root.matches('button,.btn,[role="button"]'))decorateButton(root);
  ...
  function flush(){
    ...
    var roots=pending.slice();pending=[];
    roots.forEach(scan);
  }
  function queue(root){
  ...
  function enable(){
    ...
    if(typeof MutationObserver!=='undefined'&&!observer){
      ...
      try{window.__RETRO_GRAPHICS_OBSERVER__=observer;}catch(_){}
    }
  }
  function apply(){if(active())enable();else cleanup();}
  function bind(){
    apply();
