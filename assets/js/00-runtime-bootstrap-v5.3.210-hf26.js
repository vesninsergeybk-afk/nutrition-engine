...
(function(){
  'use strict';
  var RELEASE_GATE_VERSION = 'v5.3.210-rc2-hf26-truthful-progress';
  var VERSION = 'v5.3.210_rc2_hosting_hotfix_26_truthful_progress';
  ...
  // HF26: the splash follows the real full-start pipeline instead of closing at shell-ready.
  ...
  function loadCompressedRuntime(src,fallbackSrc){
    ...
    return fetch(src,{cache:'default',credentials:'same-origin',signal:controller?controller.signal:undefined}).then(function(resp){
      ...
      if(timer)clearTimeout(timer); try{console.warn('[HF26 truthful loader] compressed runtime unavailable; using script bundle',error&&(error.message||error));}catch(_){}
  ...
  function loadCriticalShell(){
    ...
    return loadScriptsSequential(CRITICAL_SHELL_SCRIPTS).then(function(result){setBootProgress(1,1);return result;}).catch(function(err){
      ...
      try { console.warn('[HF26 truthful loader] critical shell bundle unavailable; using source files', err && (err.message||err)); } catch(_) {}
  ...
  function loadRuntime(){
    ...
    if (DEFERRED_RUNTIME_BUNDLE) {
      mainRuntime = (DEFERRED_RUNTIME_COMPRESSED ? loadCompressedRuntime(DEFERRED_RUNTIME_COMPRESSED,DEFERRED_RUNTIME_BUNDLE) : loadScript(DEFERRED_RUNTIME_BUNDLE)).catch(function(err){
        ...
        try { console.warn('[HF26 truthful loader] deferred bundle unavailable; using source files', err && (err.message||err)); } catch(_) {}
  ...
  function runReadinessChecks(){
    ...
    },Promise.resolve()).then(function(){window.__HF26_READINESS_CHECKS__=results;return results;});
  }
  function finish(){
