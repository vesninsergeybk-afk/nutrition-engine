// Nutrition Calculator v5.3.199 — AI decision support simplification pass
(function(){
  'use strict';
  var VERSION='v5.3.199_ai_decision_support_simplification_pass';
  ...
  function buildRationItems(model,mealCtx){
    var raw=safeArray(model&&model.rawRation),display=safeArray(model&&model.displayRationRows),items=[];
    function rawForRow(row){for(var i=0;i<raw.length;i++){var it=raw[i];if(it&&String(it.key)===String(row.key))return it;}for(var j=0;j<raw.length;j++){var ch=safeArray(raw[j]&&raw[j].children);for(var k=0;k<ch.length;k++)if(ch[k]&&String(ch[k].key)===String(row.key))return ch[k];}return null;}
    display.forEach(function(row,index){
      if(!row||!(Number(row.grams)>0))return;
      var item=rawForRow(row)||{},p=dbProduct(row.key),parent='';
      if(Number(row.level)>0){for(var r=0;r<raw.length;r++){var children=safeArray(raw[r]&&raw[r].children);if(children.some(function(c){return c&&String(c.key)===String(row.key);})){parent=productName(dbProduct(raw[r].key),raw[r].name_ru||raw[r].key);break;}}}
      var ai=item.ai_import||{},mealLabels=uniq([].concat((mealCtx.by_key&&mealCtx.by_key[row.key])||[],ai.meal_labels||[],item.meal_label||[]),6),isCompositeRoot=row.kind==='composite_root';
      items.push({item_id:'item_'+('000'+String(items.length+1)).slice(-3),order:index+1,key:text(row.key,120),name:text(row.name||productName(p,row.key),140),grams:round(row.grams,1),entry_type:text(row.kind||item.entry_type||'normal',40),parent_dish:parent,meal_labels:mealLabels.length?mealLabels:['Не распределено'],preparation:preparationContext(item,p),nutrient_contribution:isCompositeRoot?{}:contributionMap(row.nutrients||{}),contribution_basis:isCompositeRoot?'Сводная строка блюда; нутриентный вклад передан по ингредиентам, чтобы избежать двойного счёта.':'Вклад карточки продукта в текущей массе.',quality:productQuality(item,p),note:text(row.note,220)});
    });
    return items;
  }
  ...
  function renderUnexpectedFormat(meta){var out=$('geminiAiOutput');if(!out)return;out.innerHTML='<div class="gemini-ai-format-error" role="alert"><strong>Сервер вернул устаревший формат ИИ-дополнения.</strong><p>Сырой ответ скрыт. Обновите файлы сборки v5.3.199 на хостинге и повторите запрос.</p><div class="gemini-ds-meta">Модель: '+esc(meta&&meta.model||'Gemini')+'</div></div>';out.hidden=false;var clear=$('geminiAiClearBtn');if(clear)clear.hidden=false;}
  function run(){
