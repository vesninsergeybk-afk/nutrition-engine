...
(function(w,d){
  'use strict';
  var VERSION='v6.0.0-beta1-profile-continuity';
  var KEY='nutritionCalculator.profile.v1';
  var PREF_KEY='nutritionCalculator.profileRemember.v1';
  var SCHEMA=1;
  var SAVE_DELAY=220;
  var BASIC_REQUIRED=['needs_sex','needs_h','needs_w','needs_age','needs_activity'];
  var MATERIAL_IDS=[
  ...
  function restore(){
    ...
    if(restored){
      ...
      if(canRestoreApplied(p)){startupHydrating=true;d.documentElement.setAttribute('data-profile-restoring','1');}
    }
    return restored;
  }
  ...
  function renderProgress(){
    var c=completion(),count=byId('profileContinuityCount'),bar=byId('profileContinuityBar'),box=byId('profileContinuityMissing'),calc=byId('profileCalculateContinue'),focus=byId('profileFocusMissing');
    if(count)count.textContent=c.done+' из '+c.total;if(bar)bar.style.width=Math.round(c.done/c.total*100)+'%';
    if(box){box.innerHTML=c.complete?'<span class="is-complete">Основные данные заполнены. Осталось рассчитать персональные ориентиры.</span>':c.missing.map(function(id){return '<button type="button" data-profile-focus="'+id+'">'+labelFor(id)+'</button>';}).join('');}
    if(calc){calc.disabled=!c.complete;calc.setAttribute('aria-disabled',c.complete?'false':'true');}
  ...
  function clearSaved(options){options=options||{};safeRemove(KEY);lastPayload=null;d.documentElement.removeAttribute('data-profile-restored');d.documentElement.removeAttribute('data-profile-restoring');renderStatus();if(options.reset){var b=byId('needs_reset_btn');if(b)b.click();}}
  ...
  function attemptAutoCalculation(){
    ...
    function run(){
      ...
      else{startupHydrating=false;d.documentElement.removeAttribute('data-profile-restoring');renderStatus();}
    }
    run();
  }
  function markDirty(){
  ...
  function onComputed(){
    ...
    restoringCalculation=false;startupHydrating=false;restoreDestination='';restoreSnapshot=null;d.documentElement.removeAttribute('data-profile-restoring');write(true);renderProgress();
  ...
  function bind(){
    ...
    var onAppReady=function(){
      appReady=true;
      if(startupHydrating&&restoreSnapshot)reconcileStoredFields(restoreSnapshot);
