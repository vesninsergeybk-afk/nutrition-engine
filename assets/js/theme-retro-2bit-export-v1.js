...
(function(){
  ...
  function clean(node){
    if(!node)return null;var c=node.cloneNode(true);
  ...
  function fallbackBody(kind){
    ...
    var c=clean(sectionNode(kind));return c?'<section class="pdf-section">'+c.outerHTML+'</section>':'<p>Раздел не найден.</p>';
  }
  function fullDocument(kind){
    if(kind==='all'&&window.NutritionReportV5&&typeof window.NutritionReportV5.getReportHtml==='function'){
      try{return inject(window.NutritionReportV5.getReportHtml({force:true,mode:'retro-pdf-native'}));}catch(_){}
    }
  ...
  function open(kind){
    ...
    if(!w){alert('Разрешите всплывающие окна для печати или сохранения PDF.');return false;}
    ...
    w.document.open();w.document.write(html);w.document.close();try{w.focus();}catch(_){}
    try{window.__LAST_RETRO_PRINT_EXPORT__={kind:kind,at:new Date().toISOString(),version:VERSION};}catch(_){}
    return true;
  }
  document.addEventListener('click',function(e){
    if(!retro())return;var hit=e.target&&e.target.closest?e.target.closest(Object.keys(ids).map(function(id){return '#'+id;}).join(',')):null;
    if(!hit)return;e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();open(ids[hit.id]);
  },true);
