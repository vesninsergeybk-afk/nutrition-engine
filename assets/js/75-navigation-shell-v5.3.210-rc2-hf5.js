/* Nutrition Calculator v5.3.210 RC2 HF4
...
(function(w,d){
  ...
  var ROUTES={
    ration:{
      ...
      ids:['needsCompact','geminiRationImportSection','globalSearchSection','rc2AlternativeInput','rationSection','additionalToolsSection']
    },
    'analysis/overview':{
      ...
      ids:['totalsSection','dietAnalysisProfilePanel','strictHarvardPlateDetails','dataQualityPanel']
    },
    'analysis/nutrients':{
      ...
      ids:['totalsSection','dataQualityPanel']
    },
    'analysis/hei':{
      hash:'#analysis/hei',
      workspace:'analysis',
  ...
  var TARGET_ROUTE={
    needsCompact:'ration',needs:'ration',geminiRationImportSection:'ration',globalSearchSection:'ration',rationSection:'ration',additionalToolsSection:'ration',mealsSection:'ration',
    heiPanel:'analysis/hei',heiSummary:'analysis/hei',heiAssessment:'analysis/hei',heiTable:'analysis/hei',heiReference:'analysis/hei',
    totalsSection:'analysis/nutrients',dietAnalysisProfilePanel:'analysis/overview',strictHarvardPlateDetails:'analysis/overview',dataQualityPanel:'analysis/overview',
  ...
  function resolveInitialRoute(){
    ...
    var legacyRoute=routeFromLegacyHash();
    if(legacyRoute)return legacyRoute;
    var saved=normalizeRoute(safeGet(storageRef('localStorage'),ROUTE_KEY));
    return saved||'ration';
  }
  ...
  function recordOriginalState(){
    ...
    for(i=0;i<MANAGED_IDS.length;i++){
      el=byId(MANAGED_IDS[i]);
      if(!el)continue;
      el.classList.add('navshell-managed');
      if(!el.hasAttribute('data-navshell-original-hidden'))el.setAttribute('data-navshell-original-hidden',el.hidden?'1':'0');
    }
  }
  ...
  function createShell(){
    ...
    nav.innerHTML='<div class="navigation-shell__brand"><strong>Калькулятор</strong><span>Рабочие разделы</span></div><div class="navigation-shell__items">'+
      ...
      '</div><div class="navigation-shell__secondary"><button type="button" data-navshell-settings-toggle aria-expanded="false">Профиль и настройки</button><button type="button" data-navshell-mode="long">Полное полотно</button></div>';
    ...
    context.innerHTML='<div class="navigation-shell-context__copy"><span id="navigationShellEyebrow"></span><h2 id="navigationShellTitle" tabindex="-1"></h2><p id="navigationShellDescription"></p></div>'+
      '<button class="navigation-shell-context__settings" type="button" data-navshell-settings-toggle aria-expanded="false">Настройки</button>'+
  ...
  function bind(){
    ...
    w.addEventListener('app:ready',function(){w.setTimeout(refreshRationKpi,80);},false);
  ...
  function init(){
    ...
    w.NavigationShellV1={
      ...
      getState:function(){return {version:VERSION,mode:currentMode,route:currentRoute,workspace:ROUTES[currentRoute]?ROUTES[currentRoute].workspace:'',settingsOpen:settingsOpen};},
      routes:ROUTES
    };
