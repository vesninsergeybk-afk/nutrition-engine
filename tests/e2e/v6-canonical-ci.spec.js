...
function captureRuntimeFailures(page) {
  ...
  page.on('requestfailed', request => {
    ...
    if (url.includes('/assets/data/products.') && url.includes('.json.gz') &&
        /ERR_ABORTED|NS_BINDING_ABORTED|cancelled|canceled/i.test(reason)) return;
...
test('canonical V6 needs checkpoint boots into the section workspace', async ({ page, loadApp }) => {
...
test('mobile checkpoint opens in sections without horizontal overflow', async ({ page, loadApp }) => {
...
test('current needs checkpoint calculates and preserves the profile across sections', async ({ page, loadApp }) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await loadApp();
  await waitForCheckpoint(page);
  await waitForInterfacePass1(page);
  await page.waitForFunction(() => document.documentElement.getAttribute('data-navigation-shell') === 'workspace');
...
test('all three current themes remain user-selectable in sections mode', async ({ page, loadApp }) => {
  await page.setViewportSize({ width: 1440, height: 1000 });
  await loadApp();
  await waitForCheckpoint(page);
  await waitForInterfacePass1(page);
